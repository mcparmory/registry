...
async def send_custom_post_request(
    path: str = Field(..., description="The relative path of the Algolia REST API endpoint to target, starting after the base URL.", examples=['/keys']),
    ...
    # Log invocation
    _log_tool_invocation("send_custom_post_request", "POST", "/{path}", _request_id)
    ...
    _auth = _get_auth_for_operation("send_custom_post_request")
...
async def search_multiple_indices(
    ...
    # Log invocation
    _log_tool_invocation("search_multiple_indices", "POST", "/1/indexes/*/queries", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("search_multiple_indices")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_recommendations(requests: list[_models.BoughtTogetherQuery | _models.RelatedQuery | _models.TrendingItemsQuery | _models.TrendingFacetsQuery | _models.LookingSimilarQuery] = Field(..., description="List of recommendation requests to execute, each specifying the target index, AI model, and model-specific parameters. Multiple requests can be batched together; order of items determines order of results returned.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_recommendations", "POST", "/1/indexes/*/recommendations", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_recommendations")
    _http_headers.update(_auth.get("headers", {}))
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python search_api_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
