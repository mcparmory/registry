...
try:
    _auth_handlers["appId"] = _auth.APIKeyAuth(env_var="APP_ID_API_KEY", location="header", param_name="x-algolia-application-id")
    logging.info("Authentication configured: appId")
except ValueError as e:
    # Extract credential names from error message (first sentence before "Leave empty")
    error_msg = str(e).split("Leave empty")[0].strip()
    logging.warning(f"Credentials for appId not configured: {error_msg}")
    _auth_handlers["appId"] = None
try:
    _auth_handlers["apiKey"] = _auth.APIKeyAuth(env_var="API_KEY", location="header", param_name="x-algolia-api-key")
...
mcp = FastMCP("Algolia Search", middleware=[_JsonCoercionMiddleware()])

# Tags: algoliasearch
@mcp.tool(
    title="Send Custom POST Request",
    annotations=ToolAnnotations(
        openWorldHint=True
    ),
)
async def send_custom_post_request(
    path: str = Field(..., description="The relative path of the Algolia REST API endpoint to target, starting after the base URL."),
    parameters: dict[str, Any] | None = Field(None, description="Optional key-value pairs to include as URL query parameters with the request."),
    body: dict[str, Any] | None = Field(None, description="Optional JSON object to send as the POST request body payload."),
    ...
    try:
        _request = _models.CustomPostRequest(
            path=_models.CustomPostRequestPath(path=path),
            query=_models.CustomPostRequestQuery(parameters=parameters),
            body=_models.CustomPostRequestBody(body=body)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for send_custom_post_request: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/{path}", _request.path.model_dump(by_alias=True)) if _request.path else "/{path}"
    ...
    _auth = await _get_auth_for_operation("send_custom_post_request")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("send_custom_post_request", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="send_custom_post_request",
        method="POST",
        path=_http_path,
...
async def search_multiple_indices(
    requests: list[_models.SearchForHits | _models.SearchForFacets] = Field(..., description="List of search requests to execute, where each item specifies the target index, query, and any search parameters. Order determines execution sequence when using the stopIfEnoughMatches strategy."),
    strategy: Literal["none", "stopIfEnoughMatches"] | None = Field(None, description="Controls how multiple queries are executed: run all queries regardless of results (none), or stop early once any query returns at least hitsPerPage results (stopIfEnoughMatches)."),
) -> dict[str, Any] | ToolResult:
    ...
    try:
        _request = _models.SearchRequest(
            body=_models.SearchRequestBody(requests=requests, strategy=strategy)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_multiple_indices: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/1/indexes/*/queries"
    _http_body = _request.body.model_dump(by_alias=True, exclude_none=True) if _request.body else None
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("search_multiple_indices")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("search_multiple_indices", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="search_multiple_indices",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def get_recommendations(requests: list[_models.BoughtTogetherQuery | _models.RelatedQuery | _models.TrendingItemsQuery | _models.TrendingFacetsQuery | _models.LookingSimilarQuery] = Field(..., description="List of recommendation requests to execute, each specifying the target index, AI model, and model-specific parameters. Multiple requests can be batched together; order of items determines order of results returned.")) -> dict[str, Any] | ToolResult:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetRecommendationsRequest(
            body=_models.GetRecommendationsRequestBody(requests=requests)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_recommendations: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/1/indexes/*/recommendations"
    _http_body = _request.body.model_dump(by_alias=True, exclude_none=True) if _request.body else None
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_recommendations")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_recommendations", "POST", _http_path, _request_id)
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python algolia_search_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Algolia Search MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting Algolia Search MCP Server")
