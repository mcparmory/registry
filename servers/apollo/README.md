...
uvx mcparmory-apollo
```

### With pip

```bash
pip install mcparmory-apollo
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
OAUTH2_SCOPES=YOUR_OAUTH2_SCOPES \
API_KEY=YOUR_API_KEY \
...
Example (if server is at `/home/user/mcp-servers/apollo`):
```json
{
  "mcpServers": {
    "apollo": {
      "command": "python",
      "args": ["/home/user/mcp-servers/apollo/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens \
  ...
  ghcr.io/mcparmory/apollo:latest
```
...
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env apollo
```
