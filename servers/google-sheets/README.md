...
```bash
pip install mcparmory-google-sheets
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
OAUTH2_SCOPES=YOUR_OAUTH2_SCOPES \
API_KEY=YOUR_API_KEY \
...
Example (if server is at `/home/user/mcp-servers/google-sheets`):
```json
{
  "mcpServers": {
    "google-sheets": {
      "command": "python",
      "args": ["/home/user/mcp-servers/google-sheets/server.py"]
    }
  }
}
```
...
```bash
docker build -t google-sheets .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env google-sheets
```
