...
uvx mcparmory-enigma
```

### With pip

```bash
pip install mcparmory-enigma
API_KEY=YOUR_API_KEY \
mcparmory-enigma
```
...
- `API_KEY` — API Key Authentication (x-enigma-api-key)
...
Example (if server is at `/home/user/mcp-servers/enigma`):
```json
{
  "mcpServers": {
    "enigma": {
      "command": "python",
      "args": ["/home/user/mcp-servers/enigma/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 --env-file .env enigma
```
