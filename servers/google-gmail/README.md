...
<!-- mcp-name: com.mcparmory/google-gmail -->
...
uvx mcparmory-google-gmail
```

### With pip

```bash
pip install mcparmory-google-gmail
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
OAUTH2_SCOPES=YOUR_OAUTH2_SCOPES \
...
Example (if server is at `/home/user/mcp-servers/google-gmail`):
```json
{
  "mcpServers": {
    "google-gmail": {
      "command": "python",
      "args": ["/home/user/mcp-servers/google-gmail/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens \
  ...
  ghcr.io/mcparmory/google-gmail:latest
```
...
```bash
docker build -t google-gmail .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env google-gmail
```
