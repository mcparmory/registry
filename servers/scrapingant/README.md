...
uvx mcparmory-scrapingant
```

### With pip

```bash
pip install mcparmory-scrapingant
API_KEY=YOUR_API_KEY \
...
Example (if server is at `/home/user/mcp-servers/scrapingant`):
```json
{
  "mcpServers": {
    "scrapingant": {
      "command": "python",
      "args": ["/home/user/mcp-servers/scrapingant/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 \
  ...
  ghcr.io/mcparmory/scrapingant:latest
```
...
```bash
docker build -t scrapingant .
docker run -p 8000:8000 --env-file .env scrapingant
```
