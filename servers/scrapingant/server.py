...
BASE_URL = os.getenv("BASE_URL", "https://api.scrapingant.com")
SERVER_NAME = "Scrapingant"
...
try:
    _auth_handlers["ApiKeyAuth"] = _auth.APIKeyAuth(env_var="API_KEY", location="query", param_name="x-api-key")
...
mcp = FastMCP("Scrapingant", middleware=[_JsonCoercionMiddleware()])
...
async def scrape_webpage(
    url: str = Field(..., description="The target URL to scrape. Must be a valid, complete URL."),
    browser: bool | None = Field(None, description="Whether to render the page using a headless browser to execute JavaScript and load dynamic content. Enabled by default. Disable to retrieve raw server response only."),
    return_page_source: bool | None = Field(None, description="Return the unaltered HTML source from the server without browser-based JavaScript rendering. Only applies when browser rendering is enabled. Useful for comparing server-side vs. client-side rendered content."),
    cookies: str | None = Field(None, description="Session cookies to include with the scraping request, formatted as semicolon-separated key-value pairs (e.g., name1=value1;name2=value2). Useful for authenticated scraping."),
    js_snippet: str | None = Field(None, description="Base64-encoded JavaScript code to execute after the page loads in the browser. Only applies when browser rendering is enabled. Useful for custom interactions like scrolling or form submission."),
    proxy_type: str | None = Field(None, description="Proxy type for the request: datacenter (faster, shared IPs) or residential (slower, real user IPs). Defaults to datacenter."),
    proxy_country: str | None = Field(None, description="Country code for the proxy location (e.g., US, GB, DE). If not specified, a random country will be used. Only applies when using a proxy."),
    wait_for_selector: str | None = Field(None, description="CSS selector of an element to wait for before returning results. The scraper will poll until the element appears in the DOM. Only applies when browser rendering is enabled. Useful for waiting on dynamically loaded content."),
    block_resource: str | None = Field(None, description="Resource types to block from loading in the browser (e.g., image, stylesheet, script, font). Can be specified multiple times for multiple types. Only applies when browser rendering is enabled. Useful for faster scraping when certain resources aren't needed."),
    ...
    try:
        _request = _models.ScrapingantGeneralRequestV2GeneralGet4Request(
            query=_models.ScrapingantGeneralRequestV2GeneralGet4RequestQuery(url=url, browser=browser, return_page_source=return_page_source, cookies=cookies, js_snippet=js_snippet, proxy_type=proxy_type, proxy_country=proxy_country, wait_for_selector=wait_for_selector, block_resource=block_resource)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for scrape_webpage: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/v2/general"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("scrape_webpage")
    _http_query.update(_auth.get("params", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("scrape_webpage", "GET", _http_path, _request_id)
...
@mcp.tool(
    title="Scrape Webpage Submit",
    annotations=ToolAnnotations(
        openWorldHint=True
    ),
)
async def scrape_webpage_submit(
    url: str = Field(..., description="The target URL to scrape. Must be a valid, complete URL."),
    browser: bool | None = Field(None, description="Enable headless browser rendering to execute JavaScript and handle dynamic content. Defaults to true. When disabled, only raw server response is returned."),
    return_page_source: bool | None = Field(None, description="Return the unaltered page source as received from the server without JavaScript rendering. Only applies when browser is enabled. Defaults to false."),
    cookies: str | None = Field(None, description="HTTP cookies to include with the scraping request, formatted as semicolon-separated key-value pairs (e.g., name1=value1;name2=value2)."),
    js_snippet: str | None = Field(None, description="Base64-encoded JavaScript code to execute after the page loads in the browser. Only applies when browser is enabled. Useful for interactions, scrolling, or data extraction."),
    proxy_type: str | None = Field(None, description="Proxy type for the request: datacenter (faster, shared IPs) or residential (slower, real user IPs). Defaults to datacenter."),
    proxy_country: str | None = Field(None, description="Country code for proxy location (e.g., US, GB, DE). If not specified, a random country is selected. Only applies when using a proxy."),
    wait_for_selector: str | None = Field(None, description="CSS selector of an element to wait for before returning results. The scraper will poll until the element appears or timeout occurs. Only applies when browser is enabled."),
    block_resource: str | None = Field(None, description="Resource types to block from loading in the browser (e.g., image, stylesheet, script, font). Reduces bandwidth and speeds up scraping. Only applies when browser is enabled. Can be specified multiple times for multiple resource types."),
) -> dict[str, Any] | ToolResult:
    ...
    # Construct request model with validation
    try:
        _request = _models.ScrapingantGeneralRequestV2GeneralGet5Request(
            query=_models.ScrapingantGeneralRequestV2GeneralGet5RequestQuery(url=url, browser=browser, return_page_source=return_page_source, cookies=cookies, js_snippet=js_snippet, proxy_type=proxy_type, proxy_country=proxy_country, wait_for_selector=wait_for_selector, block_resource=block_resource)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for scrape_webpage_submit: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("scrape_webpage_submit")
    _http_query.update(_auth.get("params", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("scrape_webpage_submit", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="scrape_webpage_submit",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
@mcp.tool(
    title="Scrape Webpage Replace",
    annotations=ToolAnnotations(
        idempotentHint=True,
        openWorldHint=True
    ),
)
async def scrape_webpage_replace(
    url: str = Field(..., description="The target URL to scrape. Must be a valid HTTP or HTTPS URL."),
    browser: bool | None = Field(None, description="Whether to use a headless browser for scraping to render JavaScript and handle dynamic content. Enabled by default. Required for JS execution, page source capture, and element waiting."),
    return_page_source: bool | None = Field(None, description="Return the unaltered HTML returned by the server without JavaScript rendering. Only applies when browser is enabled. When true, JavaScript will not be executed."),
    cookies: str | None = Field(None, description="Cookies to include with the scraping request, formatted as semicolon-separated key-value pairs (e.g., name1=value1;name2=value2)."),
    js_snippet: str | None = Field(None, description="Base64-encoded JavaScript code to execute after the page loads in the browser. Only applies when browser is enabled. Useful for interactions, scrolling, or data extraction."),
    proxy_type: str | None = Field(None, description="Type of proxy to use for the request: datacenter (default) for speed or residential for authenticity. Datacenter proxies are faster; residential proxies better mimic real users."),
    proxy_country: str | None = Field(None, description="Country code for the proxy location (e.g., US, GB, DE). If not specified, a random country will be used. Only applies when using proxies."),
    wait_for_selector: str | None = Field(None, description="CSS selector of an element to wait for before returning results. The scraper will poll until the element appears in the DOM. Only applies when browser is enabled."),
    block_resource: str | None = Field(None, description="Resource types to block from loading in the browser (e.g., image, stylesheet, script, font). Can be specified multiple times for multiple types. Only applies when browser is enabled. Reduces bandwidth and speeds up scraping."),
) -> dict[str, Any] | ToolResult:
    ...
    # Construct request model with validation
    try:
        _request = _models.ScrapingantGeneralRequestV2GeneralGet3Request(
            query=_models.ScrapingantGeneralRequestV2GeneralGet3RequestQuery(url=url, browser=browser, return_page_source=return_page_source, cookies=cookies, js_snippet=js_snippet, proxy_type=proxy_type, proxy_country=proxy_country, wait_for_selector=wait_for_selector, block_resource=block_resource)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for scrape_webpage_replace: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("scrape_webpage_replace")
    _http_query.update(_auth.get("params", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("scrape_webpage_replace", "PUT", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="scrape_webpage_replace",
        method="PUT",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
@mcp.tool(
    title="Modify Webpage Scrape",
    annotations=ToolAnnotations(
        openWorldHint=True
    ),
)
async def scrape_webpage_modify(
    url: str = Field(..., description="The target URL to scrape. Must be a valid HTTP or HTTPS URL."),
    browser: bool | None = Field(None, description="Whether to use a headless browser for scraping to render JavaScript and handle dynamic content. Enabled by default. Required for JS execution, page waiting, and resource blocking features."),
    return_page_source: bool | None = Field(None, description="Return the unaltered HTML source as received from the server without JavaScript rendering. Only applies when browser is enabled. Disabled by default."),
    cookies: str | None = Field(None, description="HTTP cookies to include with the scraping request, formatted as semicolon-separated key-value pairs (e.g., name1=value1;name2=value2)."),
    js_snippet: str | None = Field(None, description="Base64-encoded JavaScript code to execute after the page loads in the browser. Only applies when browser is enabled. Useful for interactions, data extraction, or triggering additional content loads."),
    proxy_type: str | None = Field(None, description="Type of proxy to route the request through: datacenter (faster, shared IPs) or residential (slower, real user IPs). Defaults to datacenter."),
    proxy_country: str | None = Field(None, description="Country code for the proxy location (e.g., US, GB, DE). If not specified, a random country will be used. Only applies when using a proxy."),
    wait_for_selector: str | None = Field(None, description="CSS selector of an element to wait for before returning results. The scraper will poll until the element appears in the DOM. Only applies when browser is enabled."),
    block_resource: str | None = Field(None, description="Resource types to block from loading in the browser (e.g., image, stylesheet, script, font, xhr, fetch). Reduces bandwidth and speeds up scraping. Only applies when browser is enabled. Can be specified multiple times for multiple resource types."),
) -> dict[str, Any] | ToolResult:
    ...
    # Construct request model with validation
    try:
        _request = _models.ScrapingantGeneralRequestV2GeneralGetRequest(
            query=_models.ScrapingantGeneralRequestV2GeneralGetRequestQuery(url=url, browser=browser, return_page_source=return_page_source, cookies=cookies, js_snippet=js_snippet, proxy_type=proxy_type, proxy_country=proxy_country, wait_for_selector=wait_for_selector, block_resource=block_resource)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for scrape_webpage_modify: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("scrape_webpage_modify")
    _http_query.update(_auth.get("params", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("scrape_webpage_modify", "PATCH", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="scrape_webpage_modify",
        method="PATCH",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
@mcp.tool(
    title="Remove Webpage Scrape",
    annotations=ToolAnnotations(
        destructiveHint=True,
        openWorldHint=True
    ),
)
async def scrape_webpage_remove(
    url: str = Field(..., description="The target URL to scrape. Must be a valid, complete URL."),
    browser: bool | None = Field(None, description="Whether to use a headless browser for scraping to handle JavaScript-rendered content. Enabled by default. Required for JS execution and resource blocking features."),
    return_page_source: bool | None = Field(None, description="Return the unaltered HTML as received from the server without JavaScript rendering. Only applies when browser is enabled. Disabled by default."),
    cookies: str | None = Field(None, description="HTTP cookies to include with the scraping request, formatted as semicolon-separated key-value pairs (e.g., name1=value1;name2=value2)."),
    js_snippet: str | None = Field(None, description="Base64-encoded JavaScript code to execute after the page loads in the browser. Only applies when browser is enabled. Useful for interactions, scrolling, or data extraction."),
    proxy_type: str | None = Field(None, description="Type of proxy to route the request through: datacenter (faster, shared IPs) or residential (slower, real user IPs). Defaults to datacenter."),
    proxy_country: str | None = Field(None, description="Country code for the proxy location (e.g., US, GB, DE). If not specified, a random country will be used."),
    wait_for_selector: str | None = Field(None, description="CSS selector of an element to wait for before returning results. The service will poll until the element appears in the DOM. Only applies when browser is enabled."),
    block_resource: str | None = Field(None, description="Resource types to block from loading in the browser (e.g., image, stylesheet, script, font). Reduces bandwidth and speeds up scraping. Only applies when browser is enabled. Can be specified multiple times for multiple resource types."),
) -> dict[str, Any] | ToolResult:
    ...
    # Construct request model with validation
    try:
        _request = _models.ScrapingantGeneralRequestV2GeneralGet2Request(
            query=_models.ScrapingantGeneralRequestV2GeneralGet2RequestQuery(url=url, browser=browser, return_page_source=return_page_source, cookies=cookies, js_snippet=js_snippet, proxy_type=proxy_type, proxy_country=proxy_country, wait_for_selector=wait_for_selector, block_resource=block_resource)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for scrape_webpage_remove: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("scrape_webpage_remove")
    _http_query.update(_auth.get("params", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("scrape_webpage_remove", "DELETE", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="scrape_webpage_remove",
        method="DELETE",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python scrapingant_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Scrapingant MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting Scrapingant MCP Server")
