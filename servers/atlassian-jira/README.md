...
```bash
pip install mcparmory-atlassian-jira
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
OAUTH2_SCOPES=YOUR_OAUTH2_SCOPES \
BASIC_AUTH_USERNAME=YOUR_BASIC_AUTH_USERNAME \
...
Example (if server is at `/home/user/mcp-servers/atlassian-jira`):
```json
{
  "mcpServers": {
    "atlassian-jira": {
      "command": "python",
      "args": ["/home/user/mcp-servers/atlassian-jira/server.py"]
    }
  }
}
```
...
```bash
docker build -t atlassian-jira .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env atlassian-jira
```
