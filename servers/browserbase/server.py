...
async def update_context(id_: str = Field(..., alias="id", description="The unique identifier of the context to update.")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("update_context")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("update_context", "PUT", _http_path, _request_id)
...
async def upload_extension(file_: str = Field(..., alias="file", description="The binary file containing the extension package to be uploaded.")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("upload_extension")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("upload_extension", "POST", _http_path, _request_id)
...
async def get_project_usage(id_: str = Field(..., alias="id", description="The unique identifier of the project whose usage data you want to retrieve.")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_project_usage")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_project_usage", "GET", _http_path, _request_id)
...
async def list_sessions(status: Literal["RUNNING", "ERROR", "TIMED_OUT", "COMPLETED"] | None = Field(None, description="Filters the returned sessions by their current lifecycle status. Accepted values are RUNNING (active), ERROR (failed), TIMED_OUT (expired), or COMPLETED (finished successfully).")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_sessions")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_sessions", "GET", _http_path, _request_id)
...
async def create_session(
    ...
    viewport_parsed = parse_viewport(viewport)
        ...
        _request = _models.SessionsCreateRequest(
            body=_models.SessionsCreateRequestBody(project_id=project_id, timeout=timeout, keep_alive=keep_alive, proxies=proxies, region=region,
                browser_settings=_models.SessionsCreateRequestBodyBrowserSettings(
                    ...
                    viewport=_models.SessionsCreateRequestBodyBrowserSettingsViewport(width=viewport_parsed.get('width') if viewport_parsed else None, height=viewport_parsed.get('height') if viewport_parsed else None) if any(v is not None for v in [viewport_parsed.get('width') if viewport_parsed else None, viewport_parsed.get('height') if viewport_parsed else None]) else None
                ))
        )
    except pydantic.ValidationError as _validation_err:
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("create_session", "POST", _http_path, _request_id)
...
async def get_session(id_: str = Field(..., alias="id", description="The unique identifier of the session to retrieve.")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_session")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_session", "GET", _http_path, _request_id)
...
async def release_session(
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("release_session")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("release_session", "POST", _http_path, _request_id)
...
async def get_session_debug_urls(id_: str = Field(..., alias="id", description="The unique identifier of the session for which to retrieve debug URLs.")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_session_debug_urls")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_session_debug_urls", "GET", _http_path, _request_id)
...
async def list_session_downloads(id_: str = Field(..., alias="id", description="The unique identifier of the session whose downloads you want to retrieve.")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_session_downloads")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_session_downloads", "GET", _http_path, _request_id)
...
async def get_session_logs(id_: str = Field(..., alias="id", description="The unique identifier of the session whose logs you want to retrieve.")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_session_logs")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_session_logs", "GET", _http_path, _request_id)
...
async def get_session_recording(id_: str = Field(..., alias="id", description="The unique identifier of the session whose recording you want to retrieve.")) -> dict[str, Any] | ToolResult:
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_session_recording")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_session_recording", "GET", _http_path, _request_id)
...
async def upload_session_file(
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("upload_session_file")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("upload_session_file", "POST", _http_path, _request_id)
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python browserbase_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Browserbase MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting Browserbase MCP Server")
