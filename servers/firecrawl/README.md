...
uvx mcparmory-firecrawl
```

### With pip

```bash
pip install mcparmory-firecrawl
BEARER_TOKEN=YOUR_BEARER_TOKEN \
mcparmory-firecrawl
```
...
Example (if server is at `/home/user/mcp-servers/firecrawl`):
```json
{
  "mcpServers": {
    "firecrawl": {
      "command": "python",
      "args": ["/home/user/mcp-servers/firecrawl/server.py"]
    }
  }
}
```
