"""
Authentication module for Slack MCP server.
...
class OAuth2Auth:
    """
    OAuth 2.0 authentication for Slack Web API.
    ...
    Configuration (environment variables):
        ...
        - OAUTH2_USER_SCOPES: Comma-separated scopes for OAuth param `user_scope` (optional)

    Redirect URI:
        - Default: https://localhost:<OAUTH2_CALLBACK_PORT>/callback
        - Configured via OAUTH2_CALLBACK_PORT in .env (default: 9400)        - HTTPS localhost callbacks use OAUTH2_TLS_CERT_FILE and OAUTH2_TLS_KEY_FILE        - Must match redirect URI in your OAuth application configuration
    Token Storage:
        Location: ./tokens/slackauth_tokens.json
    ...
    def __init__(self):
        ...
        user_scope_env = os.getenv("OAUTH2_USER_SCOPES", "").strip()
        user_scope_scopes = [
            s.strip() for s in user_scope_env.split(",") if s.strip()
        ]
        if user_scope_scopes:
            self.extra_scope_params["user_scope"] = user_scope_scopes
        # Redirect URI for authorization flows
        self.callback_port = int(os.getenv("OAUTH2_CALLBACK_PORT", "9400"))
        self.tls_cert_file = os.getenv("OAUTH2_TLS_CERT_FILE", "").strip()
        self.tls_key_file = os.getenv("OAUTH2_TLS_KEY_FILE", "").strip()
        self.redirect_uri = self._build_callback_redirect_uri()

        # OAuth2 token URL (required for all flows that fetch tokens)
        self.token_url = self._resolve_url_template("https://slack.com/api/oauth.v2.user.access")
        self.auth_url = self._resolve_url_template("https://slack.com/oauth/v2/authorize")
        self.refresh_url = None

        # Token storage (secure file-based, unique per scheme)
        self.token_dir = Path(__file__).parent / "tokens"
        self.token_file = self.token_dir / "slackauth_tokens.json"
    ...
    def _build_callback_redirect_uri(self, port: int | None = None) -> str:
        ...
        default_scheme = "https" if True else "http"
    ...
    def _build_callback_ssl_context(self, redirect_uri: str) -> ssl.SSLContext | None:
        ...
        if not self.tls_cert_file:
            raise ValueError(
                "HTTPS OAuth2 redirect URI requires OAUTH2_TLS_CERT_FILE."
            )
...
OPERATION_AUTH_MAP: dict[str, list[list[str]]] = {
    "verify_authentication": [["slackAuth"]],
    "get_bot_info": [["slackAuth"]],
    "update_call": [["slackAuth"]],
    "delete_message": [["slackAuth"]],
    "delete_scheduled_message": [["slackAuth"]],
    "get_message_permalink": [["slackAuth"]],
    "send_me_message": [["slackAuth"]],
    "send_ephemeral_message": [["slackAuth"]],
    "send_message_to_channel": [["slackAuth"]],
    "schedule_message": [["slackAuth"]],
    "list_scheduled_messages": [["slackAuth"]],
    "update_message": [["slackAuth"]],
    "archive_conversation": [["slackAuth"]],
    "close_conversation": [["slackAuth"]],
    "create_conversation": [["slackAuth"]],
    "get_conversation_history": [["slackAuth"]],
    "get_conversation_info": [["slackAuth"]],
    "add_users_to_conversation": [["slackAuth"]],
    "join_conversation": [["slackAuth"]],
    "remove_user_from_conversation": [["slackAuth"]],
    "leave_conversation": [["slackAuth"]],
    "list_conversations": [["slackAuth"]],
    "mark_conversation_read": [["slackAuth"]],
    "list_conversation_members": [["slackAuth"]],
    "open_direct_message": [["slackAuth"]],
    "update_conversation": [["slackAuth"]],
    "get_conversation_thread": [["slackAuth"]],
    "update_conversation_purpose": [["slackAuth"]],
    "update_conversation_topic": [["slackAuth"]],
    "unarchive_conversation": [["slackAuth"]],
    "disable_dnd": [["slackAuth"]],
    "end_dnd_snooze": [["slackAuth"]],
    "get_dnd_status": [["slackAuth"]],
    "enable_do_not_disturb": [["slackAuth"]],
    "get_team_dnd_status": [["slackAuth"]],
    "list_emoji": [["slackAuth"]],
    "delete_file_comment": [["slackAuth"]],
    "delete_file": [["slackAuth"]],
    "get_file_info": [["slackAuth"]],
    "list_files": [["slackAuth"]],
    "revoke_file_public_url": [["slackAuth"]],
    "enable_file_public_sharing": [["slackAuth"]],
    "add_pin_to_channel": [["slackAuth"]],
    "list_channel_pins": [["slackAuth"]],
    "remove_pin_from_channel": [["slackAuth"]],
    "add_reaction_to_message": [["slackAuth"]],
    "get_reactions": [["slackAuth"]],
    "list_user_reactions": [["slackAuth"]],
    "remove_reaction": [["slackAuth"]],
    "create_reminder": [["slackAuth"]],
    "search_messages": [["slackAuth"]],
    "add_star": [["slackAuth"]],
    "list_stars": [["slackAuth"]],
    "remove_star": [["slackAuth"]],
    "get_team_info": [["slackAuth"]],
    "get_team_profile": [["slackAuth"]],
    "create_usergroup": [["slackAuth"]],
    "disable_usergroup": [["slackAuth"]],
    "enable_usergroup": [["slackAuth"]],
    "list_usergroups": [["slackAuth"]],
    "update_usergroup": [["slackAuth"]],
    "list_usergroup_members": [["slackAuth"]],
    "update_usergroup_users": [["slackAuth"]],
    "list_user_conversations": [["slackAuth"]],
    "delete_user_photo": [["slackAuth"]],
    "get_user_presence": [["slackAuth"]],
    "get_user_info": [["slackAuth"]],
    "list_users": [["slackAuth"]],
    "get_user_by_email": [["slackAuth"]],
    "get_user_profile": [["slackAuth"]],
    "users_set_photo": [["slackAuth"]],
    "update_user_presence": [["slackAuth"]]
}
