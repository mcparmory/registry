...
BASE_URL = os.getenv("BASE_URL", "https://api.semrush.com")
SERVER_NAME = "Semrush AppCenter"
...
try:
    _auth_handlers["jwtIssuerToken"] = _auth.JWTBearerAuth()
    logging.info("Authentication configured: jwtIssuerToken")
except ValueError as e:
    # Extract credential names from error message (first sentence before "Leave empty")
    error_msg = str(e).split("Leave empty")[0].strip()
    logging.warning(f"Credentials for jwtIssuerToken not configured: {error_msg}")
    _auth_handlers["jwtIssuerToken"] = None
...
mcp = FastMCP("Semrush AppCenter", middleware=[_JsonCoercionMiddleware()])

# Tags: Partner Service
@mcp.tool()
async def get_viewer_status(user_id: int | None = Field(None, description="The Semrush user ID to retrieve status for. If not provided, returns status for the authenticated user making the request.")) -> dict[str, Any] | ToolResult:
    ...
    try:
        _request = _models.PostApisV4AppCenterV2PartnerViewerStatusRequest(
            body=_models.PostApisV4AppCenterV2PartnerViewerStatusRequestBody(user_id=user_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_viewer_status: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/apis/v4/app-center/v2/partner/viewer-status"
    _http_body = _request.body.model_dump(by_alias=True, exclude_none=True) if _request.body else None

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_viewer_status")

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_viewer_status", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_viewer_status",
        method="POST",
        path=_http_path,
...
async def list_subscriptions() -> dict[str, Any] | ToolResult:
    """Retrieve all registered subscriptions available for the application. This lists the notification subscriptions that have been configured for server-to-server API events."""

    # Extract parameters for API call
    _http_path = "/apis/v4/hermes/v0/subscriptions"

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_subscriptions")

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_subscriptions", "GET", _http_path, _request_id)
...
async def get_user_subscription(
    user_id: int = Field(..., description="The unique identifier of the user whose subscription status you want to retrieve. Must be a positive integer."),
    id_: int = Field(..., alias="id", description="The unique identifier of the subscription to retrieve. Must be a positive integer corresponding to an existing subscription for the user."),
    ...
    # Construct request model with validation
    try:
        _request = _models.GetApisV4HermesV0UserSubscriptionRequest(
            path=_models.GetApisV4HermesV0UserSubscriptionRequestPath(user_id=user_id, id_=id_)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_user_subscription: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/apis/v4/hermes/v0/user/{user_id}/subscription/{id}", _request.path.model_dump(by_alias=True)) if _request.path else "/apis/v4/hermes/v0/user/{user_id}/subscription/{id}"

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_user_subscription")

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_user_subscription", "GET", _http_path, _request_id)
...
async def send_event_notification(
    type_: str | None = Field(None, alias="type", description="The category or type of event being sent (e.g., 'alert', 'update', 'reminder'). Helps classify the notification for routing and filtering."),
    id_: str | None = Field(None, alias="id", description="A unique identifier for this event using UUID v4 format. Serves as an idempotency key to prevent duplicate notifications if the request is retried."),
    user_id: int | None = Field(None, description="The Semrush user ID who should receive this notification. Must be a valid integer representing an existing user account."),
    data: str | None = Field(None, description="Custom event payload as a base64-encoded byte string. Decode to access the JSON data structure containing event-specific information."),
    attachments: list[_models.EventAttachment] | None = Field(None, description="Array of file attachments to include with the notification. Each attachment is processed in the order provided."),
) -> dict[str, Any] | ToolResult:
    ...
    # Construct request model with validation
    try:
        _request = _models.PostApisV4HermesV0EventRequest(
            body=_models.PostApisV4HermesV0EventRequestBody(type_=type_, id_=id_, user_id=user_id, data=data, attachments=attachments)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for send_event_notification: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/apis/v4/hermes/v0/event"
    _http_body = _request.body.model_dump(by_alias=True, exclude_none=True) if _request.body else None

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("send_event_notification")

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("send_event_notification", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="send_event_notification",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
# Tags: Hermes Partner API
@mcp.tool()
async def get_event_status(id_: str = Field(..., alias="id", description="The unique identifier (UUID format) of the subscription event whose status you want to retrieve.")) -> dict[str, Any] | ToolResult:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetApisV4HermesV0EventRequest(
            path=_models.GetApisV4HermesV0EventRequestPath(id_=id_)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_event_status: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/apis/v4/hermes/v0/event/{id}", _request.path.model_dump(by_alias=True)) if _request.path else "/apis/v4/hermes/v0/event/{id}"

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_event_status")

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_event_status", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_event_status",
        method="GET",
        path=_http_path,
        request_id=_request_id,
    )
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python semrush_app_center_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Semrush AppCenter MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting Semrush AppCenter MCP Server")
