"""
Apify Api MCP Server - Pydantic Models
...
__all__ = [
    ...
    "UserGetRequest",
    "UsersMeGetRequest",
    "UsersMeLimitsGetRequest",
    "UsersMeLimitsPutRequest",
...
# Operation: get_actor_build_openapi_schema_by_actor
class ActOpenapiJsonGetRequestPath(StrictModel):
    actor_id: str = Field(default=..., validation_alias="actorId", serialization_alias="actorId", description="The unique identifier of the Actor, either as a plain Actor ID or as a tilde-separated combination of the owner's username and Actor name.")
    build_id: str = Field(default=..., validation_alias="buildId", serialization_alias="buildId", description="The unique identifier of the Actor build whose OpenAPI schema you want to retrieve. Use the special value `default` to target the Actor's default build.")
class ActOpenapiJsonGetRequest(StrictModel):
    ...
    path: ActOpenapiJsonGetRequestPath
...
# Operation: get_actor_build_openapi_schema
class ActorBuildOpenapiJsonGetRequestPath(StrictModel):
    build_id: str = Field(default=..., validation_alias="buildId", serialization_alias="buildId", description="The unique identifier of the Actor build whose OpenAPI schema you want to retrieve. Use the special value `default` to fetch the schema for the Actor's default build.")
class ActorBuildOpenapiJsonGetRequest(StrictModel):
    ...
    path: ActorBuildOpenapiJsonGetRequestPath
...
class UsersMeGetRequest(StrictModel):
...
# Operation: get_account_limits
class UsersMeLimitsGetRequest(StrictModel):
...
# Operation: update_account_limits
class UsersMeLimitsPutRequestBody(StrictModel):
    max_monthly_usage_usd: float | None = Field(default=None, validation_alias="maxMonthlyUsageUsd", serialization_alias="maxMonthlyUsageUsd", description="Sets a hard cap on total monthly platform usage in USD to prevent unexpected overage charges beyond your prepaid usage allocation. When this threshold is reached, additional usage will be blocked for the billing period.", examples=[300])
    data_retention_days: int | None = Field(default=None, validation_alias="dataRetentionDays", serialization_alias="dataRetentionDays", description="Specifies how many days unnamed storages and non-recent Actor runs are retained before automatic deletion. The ten most recent Actor runs are always kept indefinitely regardless of this setting; adjust this value to extend retention or reduce storage usage.", examples=[90])
class UsersMeLimitsPutRequest(StrictModel):
    ...
    body: UsersMeLimitsPutRequestBody | None = None
...
class PutItemsRequest(PermissiveModel):
...
"""
    value: dict[str, Any] = Field(..., description="The request body containing the item(s) to add to the dataset. Can be a single\nobject or an array of objects. Each object represents one dataset item.\n")
...
class Run(PermissiveModel):
    ...
    usage_total_usd: float | None = Field(None, validation_alias="usageTotalUsd", serialization_alias="usageTotalUsd", description="Total cost in USD for this run. Represents what you actually pay. For run owners: includes platform usage (compute units) and/or event costs depending on the Actor's pricing model. For run non-owne...")
    usage_usd: RunUsageUsd | None = Field(None, validation_alias="usageUsd", serialization_alias="usageUsd", description="Platform usage costs breakdown in USD. Only present if you own the run AND are paying for platform usage (Pay-Per-Usage, Rental, or Pay-Per-Event with usage costs like standby Actors). Not availabl...")
...
class TaggedBuilds(PermissiveModel):
    """A dictionary mapping build tag names (e.g., "latest", "beta") to their build information."""
    value: dict[str, TaggedBuildInfo] = Field(..., description="A dictionary mapping build tag names (e.g., \"latest\", \"beta\") to their build information.")

class VersionSourceFiles(PermissiveModel):
    value: list[SourceCodeFile | SourceCodeFolder]
