...
"""
E2b Api MCP Server
Generated: 2026-03-30 22:30:11 UTC
...
try:
    _auth_handlers["ApiKeyAuth"] = _auth.APIKeyAuth(env_var="API_KEY_AUTH_API_KEY", location="header", param_name="X-API-Key")
...
async def check_health() -> dict[str, Any]:
    """Verifies the health and availability of the service. Use this to confirm the API is operational before making other requests."""
...
async def list_teams() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_teams", "GET", "/teams", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_teams")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_team_metrics(
    ...
    start: int | None = Field(None, description="Unix timestamp in seconds marking the start of the metrics interval. If omitted, defaults to the beginning of the current period.", ge=0, json_schema_extra={'format': 'int64'}),
    end: int | None = Field(None, description="Unix timestamp in seconds marking the end of the metrics interval. If omitted, defaults to the current time.", ge=0, json_schema_extra={'format': 'int64'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetTeamsMetricsRequest(
            ...
            query=_models.GetTeamsMetricsRequestQuery(start=start, end=end)
        )
    ...
    # Log invocation
    _log_tool_invocation("get_team_metrics", "GET", "/teams/{teamID}/metrics", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_team_metrics")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_team_metrics_maximum(
    ...
    start: int | None = Field(None, description="Unix timestamp in seconds marking the start of the interval. If omitted, metrics from the earliest available data are included.", ge=0),
    end: int | None = Field(None, description="Unix timestamp in seconds marking the end of the interval. If omitted, metrics up to the current time are included.", ge=0)
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetTeamsMetricsMaxRequest(
            ...
            query=_models.GetTeamsMetricsMaxRequestQuery(start=start, end=end, metric=metric)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("get_team_metrics_maximum", "GET", "/teams/{teamID}/metrics/max", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_team_metrics_maximum")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_sandbox(
    ...
    allow_public_traffic: bool | None = Field(True, alias="allowPublicTraffic", description="Control whether sandbox URLs are publicly accessible or require authentication to access."),
    ...
    # Log invocation
    _log_tool_invocation("create_sandbox", "POST", "/sandboxes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_sandbox")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_sandboxes(
    ...
    limit: int | None = Field(100, description="Maximum number of sandboxes to return per page. Must be between 1 and 100.", ge=1, le=100, json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetV2SandboxesRequest(
            query=_models.GetV2SandboxesRequestQuery(metadata=metadata, state=state, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("list_sandboxes", "GET", "/v2/sandboxes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_sandboxes")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_sandbox_metrics(sandbox_ids: Annotated[list[str], AfterValidator(_check_unique_items)] = Field(..., description="One or more sandbox IDs to retrieve metrics for. Provide as a comma-separated list of sandbox identifiers.", max_length=100)) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_sandbox_metrics", "GET", "/sandboxes/metrics", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_sandbox_metrics")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_sandbox_logs(
    ...
    cursor: int | None = Field(None, description="Starting timestamp in milliseconds from which logs should be returned. Use this to paginate through results or retrieve logs after a specific point in time.", ge=0),
    limit: int | None = Field(1000, description="Maximum number of log entries to return in a single response.", ge=0, le=1000, json_schema_extra={'format': 'int32'}),
    ...
    try:
        _request = _models.GetV2SandboxesLogsRequest(
            ...
            query=_models.GetV2SandboxesLogsRequestQuery(cursor=cursor, limit=limit, direction=direction)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("list_sandbox_logs", "GET", "/v2/sandboxes/{sandboxID}/logs", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_sandbox_logs")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_sandbox(sandbox_id: str = Field(..., alias="sandboxID", description="The unique identifier of the sandbox to retrieve.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_sandbox", "GET", "/sandboxes/{sandboxID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_sandbox")
    _http_headers.update(_auth.get("headers", {}))
...
async def terminate_sandbox(sandbox_id: str = Field(..., alias="sandboxID", description="The unique identifier of the sandbox to terminate.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("terminate_sandbox", "DELETE", "/sandboxes/{sandboxID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("terminate_sandbox")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_sandbox_metrics(
    ...
    start: int | None = Field(None, description="Unix timestamp in seconds marking the beginning of the metrics collection interval. If omitted, metrics are retrieved from the earliest available data.", ge=0, json_schema_extra={'format': 'int64'}),
    end: int | None = Field(None, description="Unix timestamp in seconds marking the end of the metrics collection interval. If omitted, metrics are retrieved up to the current time.", ge=0, json_schema_extra={'format': 'int64'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetSandboxesBySandboxIdMetricsRequest(
            ...
            query=_models.GetSandboxesBySandboxIdMetricsRequestQuery(start=start, end=end)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("get_sandbox_metrics", "GET", "/sandboxes/{sandboxID}/metrics", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_sandbox_metrics")
    _http_headers.update(_auth.get("headers", {}))
...
async def pause_sandbox(sandbox_id: str = Field(..., alias="sandboxID", description="The unique identifier of the sandbox to pause.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("pause_sandbox", "POST", "/sandboxes/{sandboxID}/pause", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("pause_sandbox")
    _http_headers.update(_auth.get("headers", {}))
...
async def connect_sandbox(
    ...
    timeout: int = Field(..., description="The number of seconds from the current time until the sandbox should automatically expire. Must be a non-negative value.", ge=0, json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.PostSandboxesConnectRequest(
            ...
            body=_models.PostSandboxesConnectRequestBody(timeout=timeout)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("connect_sandbox", "POST", "/sandboxes/{sandboxID}/connect", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("connect_sandbox")
    _http_headers.update(_auth.get("headers", {}))
...
async def set_sandbox_timeout(
    ...
    timeout: int = Field(..., description="The number of seconds from the current time until the sandbox should automatically expire. Must be a non-negative integer.", ge=0, json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.PostSandboxesTimeoutRequest(
            ...
            body=_models.PostSandboxesTimeoutRequestBody(timeout=timeout)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("set_sandbox_timeout", "POST", "/sandboxes/{sandboxID}/timeout", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("set_sandbox_timeout")
    _http_headers.update(_auth.get("headers", {}))
...
async def refresh_sandbox(
    ...
    # Log invocation
    _log_tool_invocation("refresh_sandbox", "POST", "/sandboxes/{sandboxID}/refreshes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("refresh_sandbox")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_sandbox_snapshot(
    ...
    # Log invocation
    _log_tool_invocation("create_sandbox_snapshot", "POST", "/sandboxes/{sandboxID}/snapshots", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_sandbox_snapshot")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_snapshots(
    ...
    limit: int | None = Field(100, description="Number of snapshots to return per page. Useful for paginating through large result sets.", ge=1, le=100, json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetSnapshotsRequest(
            query=_models.GetSnapshotsRequestQuery(sandbox_id=sandbox_id, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("list_snapshots", "GET", "/snapshots", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_snapshots")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_template(
    ...
    cpu_count: int | None = Field(None, alias="cpuCount", description="Number of CPU cores to allocate to the sandbox. Must be at least 1 core.", ge=1, json_schema_extra={'format': 'int32'}),
    memory_mb: int | None = Field(None, alias="memoryMB", description="Memory to allocate to the sandbox in mebibytes (MiB). Must be at least 128 MiB.", ge=128, json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.PostV3TemplatesRequest(
            body=_models.PostV3TemplatesRequestBody(name=name, tags=tags, cpu_count=cpu_count, memory_mb=memory_mb)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("create_template", "POST", "/v3/templates", _request_id)
...
async def get_template_files_upload_link(
    ...
    # Log invocation
    _log_tool_invocation("get_template_files_upload_link", "GET", "/templates/{templateID}/files/{hash}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_template_files_upload_link")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_templates(team_id: str | None = Field(None, alias="teamID", description="Filter templates to a specific team. If omitted, returns templates accessible to all teams or the default scope.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_templates", "GET", "/templates", _request_id)
...
async def list_template_builds(
    ...
    limit: int | None = Field(None, description="The maximum number of builds to return in a single page of results. Defaults to 100 if not specified.", ge=1, le=100, json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetTemplatesByTemplateIdRequest(
            ...
            query=_models.GetTemplatesByTemplateIdRequestQuery(limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("list_template_builds", "GET", "/templates/{templateID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_template_builds")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_template(template_id: str = Field(..., alias="templateID", description="The unique identifier of the template to delete.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_template", "DELETE", "/templates/{templateID}", _request_id)
...
async def start_template_build(
    ...
    from_image_registry: str | None = Field(None, alias="fromImageRegistry", description="Optional image registry configuration for sourcing or pushing build artifacts. Pass as a JSON-encoded string with a \"type\" discriminator field, e.g. '{\"type\": \"value\", ...}'."),
    force: bool | None = Field(False, description="Whether to bypass cache and force the entire build to execute from scratch."),
    steps: list[_models.TemplateStep] | None = Field(None, description="Ordered list of build steps to execute. Each step represents a discrete build operation performed sequentially. Default: []"),
    ...
    _from_image_registry: Any = json.loads(from_image_registry) if isinstance(from_image_registry, str) else from_image_registry
    ...
    try:
        _request = _models.PostV2TemplatesBuildsRequest(
            ...
            body=_models.PostV2TemplatesBuildsRequestBody(from_template=from_template, from_image_registry=_from_image_registry, force=force, steps=steps, start_cmd=start_cmd, ready_cmd=ready_cmd)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("start_template_build", "POST", "/v2/templates/{templateID}/builds/{buildID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("start_template_build")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_template(
    ...
    # Log invocation
    _log_tool_invocation("update_template", "PATCH", "/v2/templates/{templateID}", _request_id)
...
async def get_template_build_status(
    ...
    logs_offset: int | None = Field(0, alias="logsOffset", description="The starting index for build logs to return. Use this to paginate through large log sets.", ge=0, json_schema_extra={'format': 'int32'}),
    limit: int | None = Field(100, description="The maximum number of build logs to return in the response. Useful for controlling response size and pagination.", ge=0, le=100, json_schema_extra={'format': 'int32'}),
    ...
    try:
        _request = _models.GetTemplatesBuildsStatusRequest(
            ...
            query=_models.GetTemplatesBuildsStatusRequestQuery(logs_offset=logs_offset, limit=limit, level=level)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("get_template_build_status", "GET", "/templates/{templateID}/builds/{buildID}/status", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_template_build_status")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_template_build_logs(
    ...
    cursor: int | None = Field(None, description="Starting point for log retrieval specified as a Unix timestamp in milliseconds. Logs returned will be from this timestamp onward (or backward, depending on direction).", ge=0, json_schema_extra={'format': 'int64'}),
    limit: int | None = Field(100, description="Maximum number of log entries to return in a single response.", ge=0, le=100, json_schema_extra={'format': 'int32'}),
    ...
    try:
        _request = _models.GetTemplatesBuildsLogsRequest(
            ...
            query=_models.GetTemplatesBuildsLogsRequestQuery(cursor=cursor, limit=limit, direction=direction, level=level, source=source)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("list_template_build_logs", "GET", "/templates/{templateID}/builds/{buildID}/logs", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_template_build_logs")
    _http_headers.update(_auth.get("headers", {}))
...
async def assign_template_tags(
    ...
    # Log invocation
    _log_tool_invocation("assign_template_tags", "POST", "/templates/tags", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("assign_template_tags")
    _http_headers.update(_auth.get("headers", {}))
...
async def remove_template_tags(
    ...
    # Log invocation
    _log_tool_invocation("remove_template_tags", "DELETE", "/templates/tags", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("remove_template_tags")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_template_tags(template_id: str = Field(..., alias="templateID", description="The unique identifier of the template for which to retrieve tags.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_template_tags", "GET", "/templates/{templateID}/tags", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_template_tags")
    _http_headers.update(_auth.get("headers", {}))
...
async def check_template_alias(alias: str = Field(..., description="The unique identifier or name of the template to check for existence.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("check_template_alias", "GET", "/templates/aliases/{alias}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("check_template_alias")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_nodes() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_nodes", "GET", "/nodes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_nodes")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_node(
    ...
    cluster_id: str | None = Field(None, alias="clusterID", description="The cluster to which the node belongs. Use this to scope the node lookup to a specific cluster.", json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_node", "GET", "/nodes/{nodeID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_node")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_node_status(
    ...
    cluster_id: str | None = Field(None, alias="clusterID", description="The unique identifier of the cluster containing the node. Required to scope the node operation within the correct cluster context.", json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("update_node_status", "POST", "/nodes/{nodeID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_node_status")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: admin
@mcp.tool()
async def terminate_team_sandboxes(team_id: str = Field(..., alias="teamID", description="The unique identifier of the team whose sandboxes should be terminated.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("terminate_team_sandboxes", "POST", "/admin/teams/{teamID}/sandboxes/kill", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("terminate_team_sandboxes")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_access_token(name: str = Field(..., description="A descriptive name for the access token to help identify its purpose or associated application.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_access_token", "POST", "/access-tokens", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_access_token")
    _http_headers.update(_auth.get("headers", {}))
...
async def revoke_access_token(access_token_id: str = Field(..., alias="accessTokenID", description="The unique identifier of the access token to revoke and delete.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("revoke_access_token", "DELETE", "/access-tokens/{accessTokenID}", _request_id)
...
async def list_api_keys() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_api_keys", "GET", "/api-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_api_keys")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_api_key(name: str = Field(..., description="A descriptive name for the API key to help identify its purpose or associated application.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_api_key", "POST", "/api-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_api_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_api_key(
    ...
    # Log invocation
    _log_tool_invocation("update_api_key", "PATCH", "/api-keys/{apiKeyID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_api_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_api_key(api_key_id: str = Field(..., alias="apiKeyID", description="The unique identifier of the API key to delete.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_api_key", "DELETE", "/api-keys/{apiKeyID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_api_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_volumes() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_volumes", "GET", "/volumes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_volumes")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_volume(name: str = Field(..., description="The name identifier for the volume. Must contain only letters, numbers, hyphens, and underscores.", pattern="^[a-zA-Z0-9_-]+$")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_volume", "POST", "/volumes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_volume")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_volume(volume_id: str = Field(..., alias="volumeID", description="The unique identifier of the volume to retrieve.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_volume", "GET", "/volumes/{volumeID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_volume")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_volume(volume_id: str = Field(..., alias="volumeID", description="The unique identifier of the volume to delete.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_volume", "DELETE", "/volumes/{volumeID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_volume")
    _http_headers.update(_auth.get("headers", {}))
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python e2b_api_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="E2b Api MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting E2b Api MCP Server")
