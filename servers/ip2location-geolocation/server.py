...
Generated: 2026-05-12 11:39:41 UTC
...
SERVER_NAME = "IP2Location.io IP Geolocation"
...
mcp = FastMCP("IP2Location.io IP Geolocation", middleware=[_JsonCoercionMiddleware()])
...
async def lookup_ip_geolocation(
    ...
    _auth = await _get_auth_for_operation("lookup_ip_geolocation")
    _http_query.update(_auth.get("params", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("lookup_ip_geolocation", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="lookup_ip_geolocation",
        method="GET",
        path=_http_path,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="IP2Location.io IP Geolocation MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting IP2Location.io IP Geolocation MCP Server")
