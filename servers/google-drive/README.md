...
<!-- mcp-name: com.mcparmory/google-drive -->
...
uvx mcparmory-google-drive
```

### With pip

```bash
pip install mcparmory-google-drive
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
OAUTH2_SCOPES=YOUR_OAUTH2_SCOPES \
...
Example (if server is at `/home/user/mcp-servers/google-drive`):
```json
{
  "mcpServers": {
    "google-drive": {
      "command": "python",
      "args": ["/home/user/mcp-servers/google-drive/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens \
  ...
  ghcr.io/mcparmory/google-drive:latest
```
...
```bash
docker build -t google-drive .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env google-drive
```
