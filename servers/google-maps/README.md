...
uvx mcparmory-google-maps
```

### With pip

```bash
pip install mcparmory-google-maps
API_KEY=YOUR_API_KEY \
...
Example (if server is at `/home/user/mcp-servers/google-maps`):
```json
{
  "mcpServers": {
    "google-maps": {
      "command": "python",
      "args": ["/home/user/mcp-servers/google-maps/server.py"]
    }
  }
}
```
