...
try:
    _auth_handlers["bearer"] = _auth.BearerTokenAuth(env_var="BEARER_TOKEN", token_format="Bearer")
    logging.info("Authentication configured: bearer")
except ValueError as e:
    # Extract credential names from error message (first sentence before "Leave empty")
    error_msg = str(e).split("Leave empty")[0].strip()
    logging.warning(f"Credentials for bearer not configured: {error_msg}")
    _auth_handlers["bearer"] = None
...
async def get_credits_balance() -> dict[str, Any]:
    ...
    _http_path = "/v1/credits/balance"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_credits_balance")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_credits_balance", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_credits_balance",
        method="GET",
        path=_http_path,
...
async def get_webpage(
    ...
    include_raw_html: bool | None = Field(None, alias="includeRawHtml", description="Whether to include the raw HTML markup in the response. Disabled by default; enable if you need the unprocessed HTML source."),
    extract_images: bool | None = Field(None, alias="extractImages", description="Whether to extract and return images found on the webpage. Disabled by default; enable to retrieve image URLs and metadata."),
) -> dict[str, Any]:
    ...
    try:
        _request = _models.FetchRequest(
            body=_models.FetchRequestBody(url=url, render_js=render_js, include_raw_html=include_raw_html, extract_images=extract_images)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_webpage: {_validation_err}")
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_webpage")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_webpage", "POST", _http_path, _request_id)
...
async def search_web_content(
    ...
    output_type: Literal["searchResults", "sourcedAnswer", "structured"] = Field(..., alias="outputType", description="Specifies the response format. Use `searchResults` for raw search results, `sourcedAnswer` for an answer with source attribution, or `structured` for a custom JSON format defined by `structuredOutputSchema`."),
    structured_output_schema: str | None = Field(None, alias="structuredOutputSchema", description="A JSON schema (as a string) defining the desired response structure when using `structured` output type. The root element must be an object type."),
    include_sources: bool | None = Field(None, alias="includeSources", description="When using `structured` output type, include source citations in the response. Note that enabling this modifies the response schema structure."),
    include_images: bool | None = Field(None, alias="includeImages", description="Include images in the search results alongside text content."),
    from_date: str | None = Field(None, alias="fromDate", description="Filter results to content published on or after this date, specified in ISO 8601 format (YYYY-MM-DD). Must be after 1970-01-01 and before the `toDate` if provided."),
    to_date: str | None = Field(None, alias="toDate", description="Filter results to content published on or before this date, specified in ISO 8601 format (YYYY-MM-DD). Must be after 1970-01-01 and after the `fromDate` if provided."),
    include_domains: list[str] | None = Field(None, alias="includeDomains", description="Restrict search to specific domains only. Provide up to 100 domain names. By default, all domains are searched."),
    exclude_domains: list[str] | None = Field(None, alias="excludeDomains", description="Exclude specific domains from search results. By default, no domains are excluded."),
    include_inline_citations: bool | None = Field(None, alias="includeInlineCitations", description="When using `sourcedAnswer` output type, include inline citations within the answer text to indicate source attribution."),
    max_results: float | None = Field(None, alias="maxResults", description="The maximum number of results to return. The actual number of results returned will not exceed this value."),
) -> dict[str, Any]:
    ...
    try:
        _request = _models.SearchRequest(
            body=_models.SearchRequestBody(q=q, structured_output_schema=structured_output_schema, include_sources=include_sources, include_images=include_images, from_date=from_date, to_date=to_date, include_domains=include_domains, exclude_domains=exclude_domains, include_inline_citations=include_inline_citations, max_results=max_results, depth=depth, output_type=output_type)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_web_content: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("search_web_content")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("search_web_content", "POST", _http_path, _request_id)
...
async def list_research_tasks() -> dict[str, Any]:
    ...
    # Extract parameters for API call
    _http_path = "/v1/research"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_research_tasks")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_research_tasks", "GET", _http_path, _request_id)
...
async def create_research_task(
    ...
    structured_output_schema: str | None = Field(None, alias="structuredOutputSchema", description="A JSON schema (as a string) defining the desired response structure when using `structured` output type. The schema root must be an object type. Required only when `outputType` is set to `structured`."),
    include_sources: bool | None = Field(None, alias="includeSources", description="When using `structured` output type, set to `true` to include source citations in the response. Note that enabling this modifies the response schema structure. Defaults to `false`."),
    include_images: bool | None = Field(None, alias="includeImages", description="Set to `true` to include relevant images in the research results. Defaults to `false`."),
    from_date: str | None = Field(None, alias="fromDate", description="Filter results to only include sources published on or after this date, specified in ISO 8601 format (YYYY-MM-DD). Must be after 1970-01-01 and before `toDate` if both are provided."),
    to_date: str | None = Field(None, alias="toDate", description="Filter results to only include sources published on or before this date, specified in ISO 8601 format (YYYY-MM-DD). Must be after 1970-01-01 and after `fromDate` if both are provided."),
    include_domains: list[str] | None = Field(None, alias="includeDomains", description="Restrict search to specific domains only. Provide up to 100 domain names (e.g., 'microsoft.com', 'github.com'). By default, all domains are searched."),
    exclude_domains: list[str] | None = Field(None, alias="excludeDomains", description="Exclude specific domains from the search results. Provide domain names to filter out (e.g., 'wikipedia.com'). By default, no domains are excluded."),
    include_inline_citations: bool | None = Field(None, alias="includeInlineCitations", description="When using `sourcedAnswer` output type, set to `true` to include inline citations within the answer text. Defaults to `false`."),
    max_results: float | None = Field(None, alias="maxResults", description="The maximum number of search results to return. The actual number of results returned will not exceed this value."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.CreateResearchRequest(
            body=_models.CreateResearchRequestBody(q=q, structured_output_schema=structured_output_schema, include_sources=include_sources, include_images=include_images, from_date=from_date, to_date=to_date, include_domains=include_domains, exclude_domains=exclude_domains, include_inline_citations=include_inline_citations, max_results=max_results, output_type=output_type)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for create_research_task: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("create_research_task")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("create_research_task", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="create_research_task",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
# Tags: Research
@mcp.tool()
async def get_research(id_: str = Field(..., alias="id", description="The unique identifier of the research task to retrieve.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetResearchRequest(
            path=_models.GetResearchRequestPath(id_=id_)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_research: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/v1/research/{id}", _request.path.model_dump(by_alias=True)) if _request.path else "/v1/research/{id}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_research")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_research", "GET", _http_path, _request_id)
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python linkup_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Linkup MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting Linkup MCP Server")
