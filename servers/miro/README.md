...
Example (if server is at `/home/user/mcp-servers/miro`):
```json
{
  "mcpServers": {
    "miro": {
      "command": "python",
      "args": ["/home/user/mcp-servers/miro/server.py"]
    }
  }
}
```
...
docker build -t miro .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env miro
```
