...
<!-- mcp-name: com.mcparmory/google-analytics -->
...
uvx mcparmory-google-analytics
```

### With pip

```bash
pip install mcparmory-google-analytics
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
OAUTH2_SCOPES=YOUR_OAUTH2_SCOPES \
...
Example (if server is at `/home/user/mcp-servers/google-analytics`):
```json
{
  "mcpServers": {
    "google-analytics": {
      "command": "python",
      "args": ["/home/user/mcp-servers/google-analytics/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens \
  ...
  ghcr.io/mcparmory/google-analytics:latest
```
...
```bash
docker build -t google-analytics .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env google-analytics
```
