"""
Google Analytics Data Api MCP Server - Pydantic Models
...
class AudienceExportsListRequestQuery(StrictModel):
    page_size: int | None = Field(default=None, validation_alias="pageSize", serialization_alias="pageSize", description="Maximum number of audience exports to return per page. The service may return fewer than specified. Higher values are coerced to the maximum allowed.", ge=1, le=1000)
...
class CohortsRange(PermissiveModel):
    ...
    end_offset: int | None = Field(None, validation_alias="endOffset", serialization_alias="endOffset", description="Required. `endOffset` specifies the end date of the extended reporting date range for a cohort report. `endOffset` can be any positive integer but is commonly set to 5 to 10 so that reports contain...", json_schema_extra={'format': 'int32'})
    ...
    start_offset: int | None = Field(None, validation_alias="startOffset", serialization_alias="startOffset", description="`startOffset` specifies the start date of the extended reporting date range for a cohort report. `startOffset` is commonly set to 0 so that reports contain data from the acquisition of the cohort f...", json_schema_extra={'format': 'int32'})

class ConcatenateExpression(PermissiveModel):
    """Used to combine dimension values to a single dimension."""
    delimiter: str | None = Field(None, description="The delimiter placed between dimension names. Delimiters are often single characters such as \"|\" or \",\" but can be longer strings. If a dimension value contains the delimiter, both will be pres...")
...
class DateRange(PermissiveModel):
    ...
    end_date: str | None = Field(None, validation_alias="endDate", serialization_alias="endDate", description="The inclusive end date for the query in the format `YYYY-MM-DD`. Cannot be before `start_date`. The format `NdaysAgo`, `yesterday`, or `today` is also accepted, and in that case, the date is inferr...")
    name: str | None = Field(None, description="Assigns a name to this date range. The dimension `dateRange` is valued to this name in a report response. If set, cannot begin with `date_range_` or `RESERVED_`. If not set, date ranges are named b...")
    start_date: str | None = Field(None, validation_alias="startDate", serialization_alias="startDate", description="The inclusive start date for the query in the format `YYYY-MM-DD`. Cannot be after `end_date`. The format `NdaysAgo`, `yesterday`, or `today` is also accepted, and in that case, the date is inferre...")
...
class Cohort(PermissiveModel):
    ...
    date_range: DateRange | None = Field(None, validation_alias="dateRange", serialization_alias="dateRange", description="The cohort selects users whose first touch date is between start date and end date defined in the `dateRange`. This `dateRange` does not specify the full date range of event data that is present in...")
    dimension: str | None = Field(None, description="Dimension used by the cohort. Required and only supports `firstSessionDate`.")
    name: str | None = Field(None, description="Assigns a name to this cohort. The dimension `cohort` is valued to this name in a report response. If set, cannot begin with `cohort_` or `RESERVED_`. If not set, cohorts are named by their zero ba...")
...
class CohortSpec(PermissiveModel):
    ...
    cohorts: list[Cohort] | None = Field(None, description="Defines the selection criteria to group users into cohorts. Most cohort reports define only a single cohort. If multiple cohorts are specified, each cohort can be recognized in the report by their ...")
...
class Dimension(PermissiveModel):
    ...
    name: str | None = Field(None, description="The name of the dimension. See the [API Dimensions](https://developers.google.com/analytics/devguides/reporting/data/v1/api-schema#dimensions) for the list of dimension names supported by core repo...")
...
class Metric(PermissiveModel):
    ...
    invisible: bool | None = Field(None, description="Indicates if a metric is invisible in the report response. If a metric is invisible, the metric will not produce a column in the response, but can be used in `metricFilter`, `orderBys`, or a metric...")
    name: str | None = Field(None, description="The name of the metric. See the [API Metrics](https://developers.google.com/analytics/devguides/reporting/data/v1/api-schema#metrics) for the list of metric names supported by core reporting method...")
...
class MinuteRange(PermissiveModel):
    ...
    end_minutes_ago: int | None = Field(None, validation_alias="endMinutesAgo", serialization_alias="endMinutesAgo", description="The inclusive end minute for the query as a number of minutes before now. Cannot be before `startMinutesAgo`. For example, `\"endMinutesAgo\": 15` specifies the report should include event data fro...", json_schema_extra={'format': 'int32'})
    name: str | None = Field(None, description="Assigns a name to this minute range. The dimension `dateRange` is valued to this name in a report response. If set, cannot begin with `date_range_` or `RESERVED_`. If not set, minute ranges are nam...")
    start_minutes_ago: int | None = Field(None, validation_alias="startMinutesAgo", serialization_alias="startMinutesAgo", description="The inclusive start minute for the query as a number of minutes before now. For example, `\"startMinutesAgo\": 29` specifies the report should include event data from 29 minutes ago and after. Cann...", json_schema_extra={'format': 'int32'})
...
class PivotOrderBy(PermissiveModel):
    ...
    pivot_selections: list[PivotSelection] | None = Field(None, validation_alias="pivotSelections", serialization_alias="pivotSelections", description="Used to select a dimension name and value pivot. If multiple pivot selections are given, the sort occurs on rows where all pivot selection dimension name and value pairs match the row's dimension n...")
...
class Pivot(PermissiveModel):
    ...
    field_names: list[str] | None = Field(None, validation_alias="fieldNames", serialization_alias="fieldNames", description="Dimension names for visible columns in the report response. Including \"dateRange\" produces a date range column; for each row in the response, dimension values in the date range column will indica...")
    limit: str | None = Field(None, description="The number of unique combinations of dimension values to return in this pivot. The `limit` parameter is required. A `limit` of 10,000 is common for single pivot requests. The product of the `limit`...", json_schema_extra={'format': 'int64'})
    ...
    order_bys: list[OrderBy] | None = Field(None, validation_alias="orderBys", serialization_alias="orderBys", description="Specifies how dimensions are ordered in the pivot. In the first Pivot, the OrderBys determine Row and PivotDimensionHeader ordering; in subsequent Pivots, the OrderBys determine only PivotDimension...")
...
class PropertyQuota(PermissiveModel):
    ...
    tokens_per_hour: QuotaStatus | None = Field(None, validation_alias="tokensPerHour", serialization_alias="tokensPerHour", description="Standard Analytics Properties can use up to 40,000 tokens per hour; Analytics 360 Properties can use 400,000 tokens per hour. An API request consumes a single number of tokens, and that number is d...")
    tokens_per_project_per_hour: QuotaStatus | None = Field(None, validation_alias="tokensPerProjectPerHour", serialization_alias="tokensPerProjectPerHour", description="Analytics Properties can use up to 35% of their tokens per project per hour. This amounts to standard Analytics Properties can use up to 14,000 tokens per project per hour, and Analytics 360 Proper...")
...
class FilterModel(PermissiveModel):
    ...
    field_name: str | None = Field(None, validation_alias="fieldName", serialization_alias="fieldName", description="The dimension name or metric name. In most methods, dimensions & metrics can be used for the first time in this field. However in a RunPivotReportRequest, this field must be additionally specified ...")
