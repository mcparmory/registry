...
Generated: 2026-04-06 16:18:50 UTC
...
mcp = FastMCP("BuiltWith API", middleware=[_JsonCoercionMiddleware()])
...
async def detect_technologies(
    ...
    trust: Literal["yes"] | None = Field(None, alias="TRUST", description="Enrich results with additional trust and security data from the Trust API (consumes extra API credits)."),
    nometa: Literal["yes"] | None = Field(None, alias="NOMETA", description="Exclude metadata such as business addresses and contact names to reduce response size and improve performance."),
    nopii: Literal["yes"] | None = Field(None, alias="NOPII", description="Remove personally identifiable information including people names and email addresses (EU and California PII automatically redacted regardless)."),
    ...
    try:
        _request = _models.GetV22ApiJsonRequest(
            query=_models.GetV22ApiJsonRequestQuery(lookup=lookup, liveonly=liveonly, trust=trust, nometa=nometa, nopii=nopii, fdrange=fdrange, ldrange=ldrange, noattr=noattr)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for detect_technologies: {_validation_err}")
    ...
    _log_tool_invocation("detect_technologies", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="detect_technologies",
        method="GET",
        path=_http_path,
...
async def get_domain_technologies(
    ...
    trust: Literal["yes"] | None = Field(None, alias="TRUST", description="When enabled, includes additional Trust API data alongside technology information for enhanced security and reputation insights."),
    nometa: Literal["yes"] | None = Field(None, alias="NOMETA", description="When enabled, excludes metadata from the response to reduce payload size and improve response performance."),
    nopii: Literal["yes"] | None = Field(None, alias="NOPII", description="When enabled, removes personally identifiable information such as people names and email addresses from the results."),
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22ApiXmlRequest(
            query=_models.GetV22ApiXmlRequestQuery(lookup=lookup, liveonly=liveonly, trust=trust, nometa=nometa, nopii=nopii, fdrange=fdrange, ldrange=ldrange, noattr=noattr)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_domain_technologies: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_domain_technologies", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_domain_technologies",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_domain_technologies_csv(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22ApiCsvRequest(
            query=_models.GetV22ApiCsvRequestQuery(lookup=lookup, liveonly=liveonly, nometa=nometa, nopii=nopii, noattr=noattr)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_domain_technologies_csv: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_domain_technologies_csv", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_domain_technologies_csv",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def bulk_lookup_domains(
    ...
    # Construct request model with validation
    try:
        _request = _models.PostV22DomainBulkRequest(
            body=_models.PostV22DomainBulkRequestBody(lookups=lookups,
                options=_models.PostV22DomainBulkRequestBodyOptions(no_meta=no_meta, no_pii=no_pii, live_only=live_only) if any(v is not None for v in [no_meta, no_pii, live_only]) else None)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for bulk_lookup_domains: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("bulk_lookup_domains", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="bulk_lookup_domains",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_bulk_domain_job_status(job_id: str = Field(..., description="The unique identifier of the bulk job, provided as a UUID.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22DomainBulkJobIdRequest(
            path=_models.GetV22DomainBulkJobIdRequestPath(job_id=job_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_bulk_domain_job_status: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_bulk_domain_job_status", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_bulk_domain_job_status",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def retrieve_bulk_domain_job_result(job_id: str = Field(..., description="The unique identifier of the bulk job, formatted as a UUID.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22DomainBulkJobIdResultRequest(
            path=_models.GetV22DomainBulkJobIdResultRequestPath(job_id=job_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for retrieve_bulk_domain_job_result: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("retrieve_bulk_domain_job_result", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="retrieve_bulk_domain_job_result",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_domain_technology_summary(lookup: str = Field(..., alias="LOOKUP", description="The domain to analyze. Provide only the root domain (e.g., hotelscombined.com); subdomains will automatically resolve to their root domain.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Free1ApiJsonRequest(
            query=_models.GetV22Free1ApiJsonRequestQuery(lookup=lookup)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_domain_technology_summary: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_domain_technology_summary", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_domain_technology_summary",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_domain_technology_summary_xml(lookup: str = Field(..., alias="LOOKUP", description="The domain to analyze. Only root domain results are returned; subdomains are automatically resolved to their root domain.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Free1ApiXmlRequest(
            query=_models.GetV22Free1ApiXmlRequestQuery(lookup=lookup)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_domain_technology_summary_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_domain_technology_summary_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_domain_technology_summary_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_website_relationships(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Rv4ApiJsonRequest(
            query=_models.GetV22Rv4ApiJsonRequestQuery(lookup=lookup, ip=ip)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_website_relationships: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_website_relationships", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_website_relationships",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_website_relationships_xml(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Rv4ApiXmlRequest(
            query=_models.GetV22Rv4ApiXmlRequestQuery(lookup=lookup, ip=ip)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_website_relationships_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_website_relationships_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_website_relationships_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_website_relationships_csv(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Rv4ApiCsvRequest(
            query=_models.GetV22Rv4ApiCsvRequestQuery(lookup=lookup, ip=ip)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_website_relationships_csv: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_website_relationships_csv", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_website_relationships_csv",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_website_relationships_tsv(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Rv4ApiTsvRequest(
            query=_models.GetV22Rv4ApiTsvRequestQuery(lookup=lookup, ip=ip)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_website_relationships_tsv: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_website_relationships_tsv", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_website_relationships_tsv",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_websites_by_technology(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Lists12ApiJsonRequest(
            query=_models.GetV22Lists12ApiJsonRequestQuery(tech=tech, meta=meta, country=country)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_websites_by_technology: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_websites_by_technology", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_websites_by_technology",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_websites_by_technology_xml(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Lists12ApiXmlRequest(
            query=_models.GetV22Lists12ApiXmlRequestQuery(tech=tech, meta=meta, country=country)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_websites_by_technology_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_websites_by_technology_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_websites_by_technology_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def lookup_company_domains(company: str = Field(..., alias="COMPANY", description="One or more company names to look up, provided as a single name or as a comma-separated list of names (URL encoded).")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Ctu3ApiJsonRequest(
            query=_models.GetV22Ctu3ApiJsonRequestQuery(company=company)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for lookup_company_domains: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("lookup_company_domains", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="lookup_company_domains",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def resolve_company_domains(company: str = Field(..., alias="COMPANY", description="A single company name or comma-separated list of company names to look up. Each company name should be URL encoded if containing special characters.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Ctu3ApiXmlRequest(
            query=_models.GetV22Ctu3ApiXmlRequestQuery(company=company)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for resolve_company_domains: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("resolve_company_domains", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="resolve_company_domains",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_domains_by_attribute(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Tag1ApiJsonRequest(
            query=_models.GetV22Tag1ApiJsonRequestQuery(lookup=lookup, types=types)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_domains_by_attribute: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_domains_by_attribute", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_domains_by_attribute",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_domains_by_attribute_xml(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Tag1ApiXmlRequest(
            query=_models.GetV22Tag1ApiXmlRequestQuery(lookup=lookup, types=types)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_domains_by_attribute_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_domains_by_attribute_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_domains_by_attribute_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_technology_recommendations(lookup: str = Field(..., alias="LOOKUP", description="One or more root domains to analyze, provided as a single domain or comma-separated list of up to 16 domains. Only alphanumeric characters, dots, hyphens, and commas are allowed.", pattern="^[a-zA-Z0-9.,-]+$")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Rec1ApiJsonRequest(
            query=_models.GetV22Rec1ApiJsonRequestQuery(lookup=lookup)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_technology_recommendations: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_technology_recommendations", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_technology_recommendations",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_technology_recommendations_xml(lookup: str = Field(..., alias="LOOKUP", description="One or more root domains to analyze, provided as a single domain or comma-separated list of up to 16 domains. Only alphanumeric characters, dots, hyphens, and commas are allowed.", pattern="^[a-zA-Z0-9.,-]+$")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Rec1ApiXmlRequest(
            query=_models.GetV22Rec1ApiXmlRequestQuery(lookup=lookup)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_technology_recommendations_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_technology_recommendations_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_technology_recommendations_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_domain_redirects(lookup: str = Field(..., alias="LOOKUP", description="The root domain to look up (e.g., hotelscombined.com). Only root domains are supported; internal pages and subdomains are not valid.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Redirect1ApiJsonRequest(
            query=_models.GetV22Redirect1ApiJsonRequestQuery(lookup=lookup)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_domain_redirects: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_domain_redirects", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_domain_redirects",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_domain_redirects_xml(lookup: str = Field(..., alias="LOOKUP", description="The root domain to look up redirects for. Only root domains are supported (e.g., builtwith.com); internal pages and subdomains are not accepted.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Redirect1ApiXmlRequest(
            query=_models.GetV22Redirect1ApiXmlRequestQuery(lookup=lookup)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_domain_redirects_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_domain_redirects_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_domain_redirects_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_domain_keywords(lookup: str = Field(..., alias="LOOKUP", description="One or more root domains to analyze (e.g., cnn.com). Use comma-separated values to look up multiple domains at once, up to 16 domains maximum. Only root domains are supported; subdomains and internal page URLs will not work.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Kw2ApiJsonRequest(
            query=_models.GetV22Kw2ApiJsonRequestQuery(lookup=lookup)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_domain_keywords: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_domain_keywords", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_domain_keywords",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def extract_domain_keywords(lookup: str = Field(..., alias="LOOKUP", description="One or more root domains to analyze for keywords. Provide a single domain (e.g., hotelscombined.com) or multiple domains in comma-separated format for batch lookup of up to 16 domains. Subdomains and internal page URLs are not supported.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Kw2ApiXmlRequest(
            query=_models.GetV22Kw2ApiXmlRequestQuery(lookup=lookup)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for extract_domain_keywords: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("extract_domain_keywords", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="extract_domain_keywords",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def search_websites_by_keyword(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Kws1ApiJsonRequest(
            query=_models.GetV22Kws1ApiJsonRequestQuery(keyword=keyword, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_websites_by_keyword: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("search_websites_by_keyword", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="search_websites_by_keyword",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def search_websites_by_keyword_csv(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Kws1ApiCsvRequest(
            query=_models.GetV22Kws1ApiCsvRequestQuery(keyword=keyword, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_websites_by_keyword_csv: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("search_websites_by_keyword_csv", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="search_websites_by_keyword_csv",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def search_technologies(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22VectorV1ApiJsonRequest(
            query=_models.GetV22VectorV1ApiJsonRequestQuery(query=query, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_technologies: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("search_technologies", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="search_technologies",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def search_technologies_xml(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22VectorV1ApiXmlRequest(
            query=_models.GetV22VectorV1ApiXmlRequestQuery(query=query, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_technologies_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("search_technologies_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="search_technologies_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def search_technologies_csv(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22VectorV1ApiCsvRequest(
            query=_models.GetV22VectorV1ApiCsvRequestQuery(query=query, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_technologies_csv: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("search_technologies_csv", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="search_technologies_csv",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_technology_trends(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22TrendsV6ApiJsonRequest(
            query=_models.GetV22TrendsV6ApiJsonRequestQuery(tech=tech, date=date)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_technology_trends: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_technology_trends", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_technology_trends",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_technology_trends_xml(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22TrendsV6ApiXmlRequest(
            query=_models.GetV22TrendsV6ApiXmlRequestQuery(tech=tech, date=date)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_technology_trends_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_technology_trends_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_technology_trends_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def search_product_listings(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Productv1ApiJsonRequest(
            query=_models.GetV22Productv1ApiJsonRequestQuery(query=query, limit=limit, page=page)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_product_listings: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("search_product_listings", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="search_product_listings",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def assess_domain_trust(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Trustv1ApiJsonRequest(
            query=_models.GetV22Trustv1ApiJsonRequestQuery(lookup=lookup, live=live, words=words)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for assess_domain_trust: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("assess_domain_trust", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="assess_domain_trust",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def assess_domain_trust_xml(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetV22Trustv1ApiXmlRequest(
            query=_models.GetV22Trustv1ApiXmlRequestQuery(lookup=lookup, live=live, words=words)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for assess_domain_trust_xml: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    _request_id = str(uuid.uuid4())
    _log_tool_invocation("assess_domain_trust_xml", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="assess_domain_trust_xml",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BuiltWith API MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting BuiltWith API MCP Server")
