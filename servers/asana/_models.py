...
__all__ = [
    ...
    "GetProjectPortfolioSettingsForPortfolioRequest",
    ...
    "CreateMembershipRequest",
    "CreateWebhookBodyDataFiltersItem",
    ...
    "TaskRequest",
    "TeamRequest",
    "UpdateWebhookBodyDataFiltersItem",
    "UserUpdateRequest",
]
...
# Operation: execute_batch_requests
class CreateBatchRequestBodyData(StrictModel):
    actions: list[BatchRequestAction] | None = Field(default=None, validation_alias="actions", serialization_alias="actions", description="An array of individual API action objects to execute in parallel, where each item specifies the HTTP method, relative URL, and any request body or query parameters for that sub-request.")
class CreateBatchRequestBody(StrictModel):
    """The requests to batch together via the Batch API."""
    data: CreateBatchRequestBodyData | None = None
class CreateBatchRequest(StrictModel):
    ...
    body: CreateBatchRequestBody | None = None
...
# Operation: list_portfolio_project_settings
class GetProjectPortfolioSettingsForPortfolioRequestPath(StrictModel):
    portfolio_gid: str = Field(default=..., description="The globally unique identifier of the portfolio whose project portfolio settings you want to retrieve.")
class GetProjectPortfolioSettingsForPortfolioRequestQuery(StrictModel):
    limit: int | None = Field(default=None, description="The number of project portfolio settings to return per page. Must be between 1 and 100.", ge=1, le=100)
class GetProjectPortfolioSettingsForPortfolioRequest(StrictModel):
    ...
    path: GetProjectPortfolioSettingsForPortfolioRequestPath
    query: GetProjectPortfolioSettingsForPortfolioRequestQuery | None = None
...
class GetRolesRequestQuery(StrictModel):
    workspace: str | None = Field(default=None, description="The workspace or organization ID to filter roles by, limiting results to roles defined within that specific workspace.")
    archived: bool | None = Field(default=None, description="When set to true, returns only archived roles; when set to false, returns only active roles. Omit to return all roles regardless of archived status.")
class GetRolesRequest(StrictModel):
    ...
    query: GetRolesRequestQuery | None = None

# Operation: create_role
class CreateRoleRequestBody(StrictModel):
    """The role to create."""
    data: RbacRoleRequest | None = Field(default=None, description="The role definition object containing the name, permissions, and any other attributes for the new RBAC role to be created.")
class CreateRoleRequest(StrictModel):
    ...
    body: CreateRoleRequestBody | None = None

# Operation: get_role
class GetRoleRequestPath(StrictModel):
    role_gid: str = Field(default=..., description="The globally unique identifier of the role to retrieve.")
...
class UpdateRoleRequestPath(StrictModel):
    role_gid: str = Field(default=..., description="The globally unique identifier of the role to update.")
class UpdateRoleRequestBody(StrictModel):
    """The updated fields for the role."""
    data: RbacRoleUpdateRequest | None = Field(default=None, description="The role fields to update, such as name or permissions associated with the role.")
...
class DeleteRoleRequestPath(StrictModel):
    role_gid: str = Field(default=..., description="The globally unique identifier of the role to delete.")
...
# Operation: trigger_rule
class TriggerRuleRequestPath(StrictModel):
    rule_trigger_gid: str = Field(default=..., description="The unique identifier of the incoming web request trigger to fire, found in the rule's trigger configuration within Asana.")
class TriggerRuleRequestBodyData(StrictModel):
    resource: str = Field(default=..., validation_alias="resource", serialization_alias="resource", description="The unique identifier of the task to run the rule against; the task must belong to the project in which the rule was created.")
    action_data: dict[str, Any] = Field(default=..., validation_alias="action_data", serialization_alias="action_data", description="A key-value map of dynamic data passed to the rule's action at runtime, allowing external context (such as ticket names or IDs from third-party systems) to be used within the rule's action logic.")
class TriggerRuleRequestBody(StrictModel):
    """A dictionary of variables accessible from within the rule."""
    data: TriggerRuleRequestBodyData
class TriggerRuleRequest(StrictModel):
    ...
    path: TriggerRuleRequestPath
    body: TriggerRuleRequestBody
...
class GetWebhooksRequestQuery(StrictModel):
    workspace: str = Field(default=..., description="The unique identifier of the workspace to query for registered webhooks.")
    resource: str | None = Field(default=None, description="When provided, filters the returned webhooks to only those associated with the specified resource identifier.")
...
class CreateWebhookRequestBodyData(StrictModel):
    resource: str = Field(default=..., validation_alias="resource", serialization_alias="resource", description="The GID of the Asana resource to monitor for events (e.g., a task, project, or workspace). Note that higher-level resources such as workspaces require filters to be specified alongside the subscription.")
    target: str = Field(default=..., validation_alias="target", serialization_alias="target", description="The fully-qualified HTTPS URL that will receive webhook event POST requests. Query parameters may be included to encode application-specific state that will be preserved across all deliveries.", json_schema_extra={'format': 'uri'})
    filters: list[CreateWebhookBodyDataFiltersItem] | None = Field(default=None, validation_alias="filters", serialization_alias="filters", description="An optional array of WebhookFilter objects defining an allowlist of conditions; an event is delivered only if it matches at least one filter. If omitted, all events for the resource are delivered. Order of filters is not significant.")
class CreateWebhookRequestBody(StrictModel):
    """The webhook workspace and target."""
    data: CreateWebhookRequestBodyData
...
class UpdateWebhookRequestPath(StrictModel):
    webhook_gid: str = Field(default=..., description="The globally unique identifier of the webhook to update.")
class UpdateWebhookRequestBodyData(StrictModel):
    filters: list[UpdateWebhookBodyDataFiltersItem] | None = Field(default=None, validation_alias="filters", serialization_alias="filters", description="An array of WebhookFilter objects defining which events should trigger delivery; the entire existing filters array is replaced by this new array. A webhook event is delivered if it matches any filter in the array.")
class UpdateWebhookRequestBody(StrictModel):
    """The updated filters for the webhook."""
    data: UpdateWebhookRequestBodyData | None = None
...
class BatchRequestActionOptions(PermissiveModel):
    ...
    limit: int | None = Field(None, description="Pagination limit for the request.")
    offset: int | None = Field(None, description="Pagination offset for the request.")
    fields: list[str] | None = Field(None, description="The fields to retrieve in the request.")

class BatchRequestAction(PermissiveModel):
    """An action object for use in a batch request."""
    relative_path: str = Field(..., description="The path of the desired endpoint relative to the API’s base URL. Query parameters are not accepted here; put them in `data` instead.")
    method: Literal["get", "post", "put", "delete", "patch", "head"] = Field(..., description="The HTTP method you wish to emulate for the action.")
    data: dict[str, Any] | None = Field(None, description="For `GET` requests, this should be a map of query parameters you would have normally passed in the URL. Options and pagination are not accepted here; put them in `options` instead. For `POST`, `PAT...")
    options: BatchRequestActionOptions | None = Field(None, description="Pagination (`limit` and `offset`) and output options (`fields` or `expand`) for the action. “Pretty” JSON output is not an available option on individual actions; if you want pretty output, specify...")
...
class BudgetEstimateRequest(PermissiveModel):
    ...
    source: Literal["none", "tasks", "capacity_plans"] | None = Field(None, description="The data source for the estimate. `tasks`: use task-level estimated time attributed to the parent. `capacity_plans`: use capacity plan estimates attributed to the parent. `none`: disables the estim...")
    enabled: bool | None = Field(None, description="Controls whether the estimate is displayed in the budget. This flag primarily affects UI presentation and the response payload. When `false` (and `source` is `none`), the estimate is hidden and the...")
...
class BudgetTotalRequest(PermissiveModel):
    ...
    value: float | None = Field(None, description="The user-set value for the total budget. When `budget_type` is `time`, represents minutes. When `budget_type` is `cost`, represents the monetary amount in the domain's currency. This value is store...")
...
class CreateWebhookBodyDataFiltersItem(PermissiveModel):
    resource_type: str | None = Field(None, description="The type of the resource which created the event when modified; for example, to filter to changes on regular tasks this field should be set to `task`.")
    resource_subtype: str | None = Field(None, description="The resource subtype of the resource that the filter applies to. This should be set to the same value as is returned on the `resource_subtype` field on the resources themselves.")
    action: str | None = Field(None, description="The type of change on the **resource** to pass through the filter. For more information refer to `Event.action` in the [event](/reference/events) schema. This can be one of `changed`, `added`, `rem...")
    fields: list[str] | None = Field(None, description="*Conditional.* A whitelist of fields for events which will pass the filter when the resource is changed. These can be any combination of the fields on the resources themselves. This field is only v...")
...
Users in Asana can [lock custom fields](https://asana.com/guide/help/premium/custom-fields#gl-lock-fields), which will make them read-only when accessed by other users. Attempting to edit a locked custom field will return HTTP error code `403 Forbidden`."""
    ...
    enum_options: list[EnumOption] | None = Field(None, description="*Conditional*. Only relevant for custom fields of type `enum` or `multi_enum`. This array specifies the possible values which an `enum` custom field can adopt. To modify the enum options, refer to ...")
    enabled: bool | None = Field(None, description="*Conditional*. This field applies only to [custom field values](/docs/custom-fields-guide#/accessing-custom-field-values-on-tasks-or-projects) and is not available for [custom field definitions](/d...")
    ...
    date_value: CustomFieldCompactDateValue | None = Field(None, description="*Conditional*. Only relevant for custom fields of type `date`. This object reflects the chosen date (and optionally, time) value of a `date` custom field. If no date is selected, the value of `date...")
    ...
    display_value: str | None = Field(None, description="A string representation for the value of the custom field. Integrations that don't require the underlying type should use this field to read values. Using this field will future-proof an app agains...")
...
class CustomFieldBase(PermissiveModel):
    ...
    input_restrictions: list[str] | None = Field(None, description="*Conditional*. Only relevant for custom fields of type `reference`. This array of strings reflects the allowed types of objects that can be written to a `reference` custom field value.")
    is_formula_field: bool | None = Field(None, description="*Conditional*. This flag describes whether a custom field is a formula custom field.")
    date_value: CustomFieldBaseDateValue | None = Field(None, description="*Conditional*. Only relevant for custom fields of type `date`. This object reflects the chosen date (and optionally, time) value of a `date` custom field. If no date is selected, the value of `date...")
    enum_value: EnumOption | dict[str, Any] | None = None
    multi_enum_values: list[EnumOption] | None = Field(None, description="*Conditional*. Only relevant for custom fields of type `multi_enum`. This object is the chosen values of a `multi_enum` custom field.")
    number_value: float | None = Field(None, description="*Conditional*. This number is the value of a `number` custom field.")
    text_value: str | None = Field(None, description="*Conditional*. This string is the value of a `text` custom field.")
    ...
    precision: int | None = Field(None, description="Only relevant for custom fields of type `Number`. This field dictates the number of places after the decimal to round to, i.e. 0 is integer values, 1 rounds to the nearest tenth, and so on. Must be...")
...
class CustomFieldRequest(PermissiveModel):
    ...
    owned_by_app: bool | None = Field(None, description="*Allow-listed*. Instructs the API that this Custom Field is app-owned. This parameter is allow-listed to specific apps at this point in time. For apps that are not allow-listed, providing this para...")
    people_value: list[str] | None = Field(None, description="*Conditional*. Only relevant for custom fields of type `people`. This array of user GIDs reflects the users to be written to a `people` custom field. Note that *write* operations will replace exist...")
    reference_value: list[str] | None = Field(None, description="*Conditional*. Only relevant for custom fields of type `reference`. This array of GIDs reflects the objects to be written to a `reference` custom field. Note that *write* operations will replace ex...")
...
class GoalBase(PermissiveModel):
    ...
    is_workspace_level: bool | None = Field(None, description="*Conditional*. This property is only present when the `workspace` provided is an organization. Whether the goal belongs to the `workspace` (and is listed as part of the workspace’s goals) or not. I...")
...
class GoalUpdateRequest(PermissiveModel):
    status: str | None = Field(None, description="The current status of this goal. When the goal is open, its status can be `green`, `yellow`, and `red` to reflect \"On Track\", \"At Risk\", and \"Off Track\", respectively. When the goal is closed...")
    custom_fields: dict[str, str] | None = Field(None, description="An object where each key is the GID of a custom field and its corresponding value is either an enum GID, string, number, or object (depending on the custom field type). See the [custom fields guide...")

class MembershipRequest(PermissiveModel):
    access_level: str | None = Field(None, description="Sets the access level for the member. Goals can have access levels `viewer`, `commenter`, `editor` or `admin`. Projects can have access levels `admin`, `editor` or `commenter`. Portfolios can have ...")
...
class PortfolioBase(PermissiveModel):
    ...
    start_on: str | None = Field(None, description="The day on which work for this portfolio begins, or null if the portfolio has no start date. This takes a date with `YYYY-MM-DD` format. *Note: `due_on` must be present in the request when setting ...", json_schema_extra={'format': 'date'})
...
class RbacRoleBasePermissions(PermissiveModel):
    """The permissions of the role."""
    create_app_authorization: bool | None = Field(None, description="Controls whether users with this role can create app authorizations")
    create_pat_authorization: bool | None = Field(None, description="Controls whether users with this role can create PAT authorizations")
    share_goal_with_domain: bool | None = Field(None, description="Controls whether users with this role can create company-wide goals")
    share_portfolios_with_org: bool | None = Field(None, description="Controls whether users with this role can create portfolios that are shared with the organization and share existing portfolios to the organization")
    assign_roles: bool | None = Field(None, description="Controls whether users with this role can assign guest, member, and admin roles to other users")
    manage_roles: bool | None = Field(None, description="Controls whether users with this role can create, edit, and delete roles")
    export_project_data: bool | None = Field(None, description="Controls whether users with this role can export project data")
    task_deletion_policy: Literal["all", "owned", "none"] | None = Field(None, description="Determines what tasks users with this role are allowed to delete")
    download_mobile_attachments: bool | None = Field(None, description="Controls whether users with this role can download attachments from mobile app")
    import_data: bool | None = Field(None, description="Controls whether users with this role can import data into the domain")
    create_global_custom_fields: bool | None = Field(None, description="Controls whether users with this role can create global custom fields")
    upload_attachments: bool | None = Field(None, description="Controls whether users with this role can upload attachments")
    create_team: bool | None = Field(None, description="Controls whether users with this role can create teams")
    allowed_guest_invites: Literal["all", "none"] | None = Field(None, description="Controls what type of email users with this role are allowed to invite")
    create_and_edit_ai_automations: bool | None = Field(None, description="Controls whether users with this role can create and edit AI automations")
    proactive_ai: bool | None = Field(None, description="Controls whether users with this role can use proactive AI features")
    standard_ai: bool | None = Field(None, description="Controls whether users with this role can use standard AI features")

class RbacRoleBaseV1Permissions(PermissiveModel):
    """The permissions of the role."""
    create_app_authorization: bool | None = Field(None, description="Controls whether users with this role can create app authorizations")
    create_pat_authorization: bool | None = Field(None, description="Controls whether users with this role can create PAT authorizations")
    share_goal_with_domain: bool | None = Field(None, description="Controls whether users with this role can create company-wide goals")
...
class RbacRoleCompact(PermissiveModel):
    ...
    is_standard_role: bool | None = Field(None, description="Whether the role is a standard role or a custom role. Defaults to `false`.")

class RbacRoleBase(PermissiveModel):
    gid: str | None = Field(None, description="Globally unique identifier of the resource, as a string.")
    resource_type: str | None = Field(None, description="The base type of this resource.")
    name: str | None = Field(None, description="The name of the role.")
    description: str | None = Field(None, description="The description of the role.")
    is_standard_role: bool | None = Field(None, description="Whether the role is a standard role or a custom role. Defaults to `false`.")
    creation_time: str | None = Field(None, description="The time at which this role was created.", json_schema_extra={'format': 'date-time'})
    modified_at: str | None = Field(None, description="The time at which this role was last modified.", json_schema_extra={'format': 'date-time'})
    base_role_type: Literal["guest", "member", "admin", "super_admin"] | None = Field(None, description="The base role type of the role.")
    permissions: RbacRoleBasePermissions | None = Field(None, description="The permissions of the role.")

class RbacRoleRequest(PermissiveModel):
    workspace: str = Field(..., description="*Create-only*. The GID of the workspace this role is associated with. Once created, roles cannot be moved to a different workspace. This attribute can only be specified at creation time.")

class RbacRoleUpdateRequest(PermissiveModel):
    pass
...
class ResourceExportRequestParameter(PermissiveModel):
    ...
    fields: list[str] | None = Field(None, description="An array of fields to include for the resource type. If not provided, all non-optional fields for the resource type will be included. This conforms to the fields optional parameter available for al...")
...
class UpdateWebhookBodyDataFiltersItem(PermissiveModel):
    resource_type: str | None = Field(None, description="The type of the resource which created the event when modified; for example, to filter to changes on regular tasks this field should be set to `task`.")
    resource_subtype: str | None = Field(None, description="The resource subtype of the resource that the filter applies to. This should be set to the same value as is returned on the `resource_subtype` field on the resources themselves.")
    action: str | None = Field(None, description="The type of change on the **resource** to pass through the filter. For more information refer to `Event.action` in the [event](/reference/events) schema. This can be one of `changed`, `added`, `rem...")
    fields: list[str] | None = Field(None, description="*Conditional.* A whitelist of fields for events which will pass the filter when the resource is changed. These can be any combination of the fields on the resources themselves. This field is only v...")
...
class CustomFieldResponse(PermissiveModel):
    ...
    default_access_level: Literal["admin", "editor", "user"] | None = Field(None, description="The default access level when inviting new members to the custom field. This isn't applied when the `privacy_setting` is `private`, or the user is a guest. For local fields in a project or portfoli...")
...
class ProjectStatusResponse(PermissiveModel):
    ...
    modified_at: str | None = Field(None, description="The time at which this project status was last modified.\n*Note: This does not currently reflect any changes in associations such as comments that may have been added or removed from the project st...", json_schema_extra={'format': 'date-time'})
...
class ProjectBase(PermissiveModel):
    ...
    public: bool | None = Field(None, description="*Deprecated:* new integrations use `privacy_setting` instead.")
    privacy_setting: Literal["public_to_workspace", "private_to_team", "private"] | None = Field(None, description="The privacy setting of the project. *Note: Administrators in your organization may restrict the values of `privacy_setting`.* The value `private_to_team` is deprecated. Use `POST /memberships` to s...")
    start_on: str | None = Field(None, description="The day on which work for this project begins, or null if the project has no start date. This takes a date with `YYYY-MM-DD` format. *Note: `due_on` or `due_at` must be present in the request when ...", json_schema_extra={'format': 'date'})
...
class TaskBase(PermissiveModel):
    ...
    approval_status: Literal["pending", "approved", "rejected", "changes_requested"] | None = Field(None, description="*Conditional* Reflects the approval status of this task. This field is kept in sync with `completed`, meaning `pending` translates to false while `approved`, `rejected`, and `changes_requested` tra...")
    assignee_status: Literal["today", "upcoming", "later", "new", "inbox"] | None = Field(None, description="*Deprecated* Scheduling status of this task for the user it is assigned to. This field can only be set if the assignee is non-null. Setting this field to \"inbox\" or \"upcoming\" inserts it at the...")
    ...
    is_rendered_as_separator: bool | None = Field(None, description="[Opt In](/docs/inputoutput-options). In some contexts tasks can be rendered as a visual separator; for instance, subtasks can appear similar to [sections](/reference/sections) without being true `s...")
    ...
    start_at: str | None = Field(None, description="Date and time on which work begins for the task, or null if the task has no start time. This takes an ISO 8601 date string in UTC and should not be used together with `start_on`.\n*Note: `due_at` m...", json_schema_extra={'format': 'date-time'})
    start_on: str | None = Field(None, description="The day on which work begins for the task , or null if the task has no start date. This takes a date with `YYYY-MM-DD` format and should not be used together with `start_at`.\n*Note: `due_on` or `d...", json_schema_extra={'format': 'date'})
    actual_time_minutes: float | None = Field(None, description="<p><strong style={{ color: \"#4573D2\" }}>Full object requires scope: </strong><code>time_tracking_entries:read</code></p>\n\nThis value represents the sum of all the Time Tracking entries in the A...")

class TaskRequest(PermissiveModel):
    assignee: str | None = Field(None, description="Gid of a user.")
    assignee_section: str | None = Field(None, description="The *assignee section* is a subdivision of a project that groups tasks together in the assignee's \"My tasks\" list. It can either be a header above a list of tasks in a list view or a column in a ...")
    custom_fields: dict[str, str] | None = Field(None, description="An object where each key is the GID of a custom field and its corresponding value is either an enum GID, string, number, object, or array (depending on the custom field type). See the [custom field...")
    followers: list[str] | None = Field(None, description="*Create-Only* An array of strings identifying users. These can either be the string \"me\", an email, or the gid of a user. In order to change followers on an existing task use `addFollowers` and `...")
...
class WebhookFilter(PermissiveModel):
...
BatchRequestAction.model_rebuild()
BatchRequestActionOptions.model_rebuild()
...
CreateWebhookBodyDataFiltersItem.model_rebuild()
...
RbacRoleBase.model_rebuild()
RbacRoleBasePermissions.model_rebuild()
RbacRoleBaseV1Permissions.model_rebuild()
RbacRoleCompact.model_rebuild()
RbacRoleRequest.model_rebuild()
RbacRoleUpdateRequest.model_rebuild()
...
UpdateWebhookBodyDataFiltersItem.model_rebuild()
UserCompact.model_rebuild()
UserUpdateRequest.model_rebuild()
WebhookFilter.model_rebuild()
