...
class OAuth2Auth:
    """
    OAuth 2.0 authentication for Notion API.
    ...
    Token Storage:
        Location: ./tokens/bearerauth_tokens.json
    ...
    def __init__(self):
        ...
        self.token_url = self._resolve_url_template("https://api.notion.com/v1/oauth/token")
        self.auth_url = self._resolve_url_template("https://api.notion.com/v1/oauth/authorize")
        self.refresh_url = None

        # Token storage (secure file-based, unique per scheme)
        self.token_dir = Path(__file__).parent / "tokens"
        self.token_file = self.token_dir / "bearerauth_tokens.json"
