...
"""
AgentQL MCP Server
Generated: 2026-05-11 22:50:10 UTC
...
mcp = FastMCP("AgentQL", middleware=[_JsonCoercionMiddleware()])

# Tags: AgentQL REST API
@mcp.tool(
    title="Query Webpage Data",
    annotations=ToolAnnotations(
        openWorldHint=True
    ),
)
async def query_webpage_data(
    content_type: str = Field(..., alias="content-type", description="MIME type of the request body, required to correctly parse the payload."),
    query: str | None = Field(None, description="An AgentQL (AQL) query string specifying the exact data fields to extract from the page. If omitted, a query will be auto-generated from the prompt."),
    ...
    url: str | None = Field(None, description="The fully qualified URL of the webpage to load and query. Either url or html must be provided as the data source."),
    ...
    mode: Literal["fast", "standard"] | None = Field(None, description="Controls the response generation strategy: 'fast' prioritizes speed, 'standard' prioritizes accuracy and completeness."),
    wait_for: int | None = Field(None, description="Number of seconds to wait for dynamic page content to load before capturing the snapshot. Maximum allowed wait time is 10 seconds."),
    is_scroll_to_bottom_enabled: bool | None = Field(None, description="When enabled, the browser scrolls to the bottom of the page before capturing the snapshot, useful for triggering lazy-loaded content."),
    is_screenshot_enabled: bool | None = Field(None, description="When enabled, a screenshot of the page is captured during the query session, which may be useful for debugging or visual verification."),
    browser_profile: Literal["light", "stealth", "tf-browser"] | None = Field(None, description="Determines the browser profile used for the session: 'light' uses a fast headless browser, 'stealth' applies anti-detection techniques for bot-protected pages."),
    ...
    try:
        _request = _models.QueryDataServiceV1QueryDataPostRequest(
            ...
            body=_models.QueryDataServiceV1QueryDataPostRequestBody(query=query, prompt=prompt, url=url, html=html,
                params=_models.QueryDataServiceV1QueryDataPostRequestBodyParams(mode=mode, wait_for=wait_for, is_scroll_to_bottom_enabled=is_scroll_to_bottom_enabled, is_screenshot_enabled=is_screenshot_enabled, browser_profile=browser_profile, proxy=proxy) if any(v is not None for v in [mode, wait_for, is_scroll_to_bottom_enabled, is_screenshot_enabled, browser_profile, proxy]) else None)
        )
    ...
    _auth = await _get_auth_for_operation("query_webpage_data")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("query_webpage_data", "POST", _http_path, _request_id)
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python agent_ql_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting AgentQL MCP Server")
