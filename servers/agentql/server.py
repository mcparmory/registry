...
Generated: 2026-03-25 13:40:59 UTC
...
async def query_webpage_data(
    ...
    mode: Literal["fast", "standard"] | None = Field('fast', description="Controls the response generation strategy: 'fast' prioritizes speed, 'standard' prioritizes accuracy and completeness."),
    ...
    is_scroll_to_bottom_enabled: bool | None = Field(False, description="When enabled, the browser scrolls to the bottom of the page before capturing the snapshot, useful for triggering lazy-loaded content."),
    is_screenshot_enabled: bool | None = Field(False, description="When enabled, a screenshot of the page is captured during the query session, which may be useful for debugging or visual verification."),
    browser_profile: Literal["light", "stealth", "tf-browser"] | None = Field('light', description="Determines the browser profile used for the session: 'light' uses a fast headless browser, 'stealth' applies anti-detection techniques for bot-protected pages."),
...
async def create_browser_session(
    browser_ua_preset: Literal["windows", "macos", "linux"] | None = Field('linux', description="The operating system user agent preset the browser will identify as, affecting how websites perceive the client environment."),
    browser_profile: Literal["light", "stealth", "tf-browser"] | None = Field('light', description="The browser profile determining capability and detection resistance: 'light' prioritizes speed with minimal overhead, 'stealth' enables full anti-detection features, and 'tf-browser' uses a custom TF Browser configuration."),
    shutdown_mode: Literal["on_disconnect", "on_inactivity_timeout"] | None = Field('on_disconnect', description="Controls session teardown behavior on disconnect: 'on_disconnect' immediately stops the session when all connections close, while 'on_inactivity_timeout' keeps the session alive to allow reconnection until the inactivity timeout elapses."),
    inactivity_timeout_seconds: int | None = Field(300, description="How long the session remains alive without active connections before being shut down, applicable when shutdown_mode is 'on_inactivity_timeout'. Accepts values between 5 seconds and 86400 seconds (24 hours).", ge=5.0, le=86400.0),
    ...
    # Log invocation
    _log_tool_invocation("create_browser_session", "POST", "/v1/tetra/sessions", _request_id)
...
async def list_session_usage(
    ...
    start_after: str | None = Field(None, description="Return only sessions that started after this timestamp, specified in ISO 8601 date-time format.", json_schema_extra={'format': 'date-time'}),
    end_before: str | None = Field(None, description="Return only sessions that ended before this timestamp, specified in ISO 8601 date-time format.", json_schema_extra={'format': 'date-time'}),
    ...
    limit: int | None = Field(100, description="Maximum number of session records to return per page; must be between 1 and 1000.", ge=1, le=1000),
    page: int | None = Field(1, description="Page number to retrieve for paginated results; must be 1 or greater.", ge=1)
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_session_usage", "GET", "/v1/tetra/usage", _request_id)
