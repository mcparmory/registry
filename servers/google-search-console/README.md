...
<!-- mcp-name: com.mcparmory/google-search-console -->
...
uvx mcparmory-google-search-console
```

### With pip

```bash
pip install mcparmory-google-search-console
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
OAUTH2_SCOPES=YOUR_OAUTH2_SCOPES \
...
Example (if server is at `/home/user/mcp-servers/google-search-console`):
```json
{
  "mcpServers": {
    "google-search-console": {
      "command": "python",
      "args": ["/home/user/mcp-servers/google-search-console/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens \
  ...
  ghcr.io/mcparmory/google-search-console:latest
```
...
```bash
docker build -t google-search-console .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env google-search-console
```
