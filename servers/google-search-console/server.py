...
async def query_search_analytics(
    ...
    data_state: Literal["DATA_STATE_UNSPECIFIED", "FINAL", "ALL", "HOURLY_ALL"] | None = Field(None, alias="dataState", description="Which data state to retrieve: FINAL (complete data only), ALL (complete and partial data), or HOURLY_ALL (includes hourly data). Defaults to FINAL if not specified."),
    dimension_filter_groups: list[_models.ApiDimensionFilterGroup] | None = Field(None, alias="dimensionFilterGroups", description="Zero or more filter groups to narrow results by dimension values (e.g., query contains 'buy', country equals 'USA'). Filters are applied without requiring those dimensions to be in the grouping."),
    dimensions: list[Literal["DATE", "QUERY", "PAGE", "COUNTRY", "DEVICE", "SEARCH_APPEARANCE", "HOUR"]] | None = Field(None, description="Zero or more dimensions to group results by (e.g., query, page, country, device). Results are grouped in the order specified. At least one dimension is typically needed for meaningful results."),
    end_date: str | None = Field(None, alias="endDate", description="End date for the query range in YYYY-MM-DD format (PST/UTC-8). Must be greater than or equal to the start date and is included in the range."),
    row_limit: str | None = Field(None, alias="rowLimit", description="Maximum number of rows to return in the response. Must be between 1 and 25,000 inclusive. Defaults to 1,000 if not specified."),
    ...
    _start_row = _parse_int(start_row)

    # Construct request model with validation
    try:
        _request = _models.WebmastersSearchanalyticsQueryRequest(
            path=_models.WebmastersSearchanalyticsQueryRequestPath(site_url=site_url),
            body=_models.WebmastersSearchanalyticsQueryRequestBody(aggregation_type=aggregation_type, data_state=data_state, dimension_filter_groups=dimension_filter_groups, dimensions=dimensions, end_date=end_date, row_limit=_row_limit, search_type=search_type, start_date=start_date, start_row=_start_row)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for query_search_analytics: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/webmasters/v3/sites/{siteUrl}/searchAnalytics/query", _request.path.model_dump(by_alias=True)) if _request.path else "/webmasters/v3/sites/{siteUrl}/searchAnalytics/query"
    _http_body = _request.body.model_dump(by_alias=True, exclude_none=True) if _request.body else None
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("query_search_analytics")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("query_search_analytics", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="query_search_analytics",
        method="POST",
        path=_http_path,
...
async def get_sitemap(
    ...
    # Construct request model with validation
    try:
        _request = _models.WebmastersSitemapsGetRequest(
            path=_models.WebmastersSitemapsGetRequestPath(site_url=site_url, feedpath=feedpath)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_sitemap: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/webmasters/v3/sites/{siteUrl}/sitemaps/{feedpath}", _request.path.model_dump(by_alias=True)) if _request.path else "/webmasters/v3/sites/{siteUrl}/sitemaps/{feedpath}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_sitemap")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_sitemap", "GET", _http_path, _request_id)
...
async def submit_sitemap(
    ...
    # Construct request model with validation
    try:
        _request = _models.WebmastersSitemapsSubmitRequest(
            path=_models.WebmastersSitemapsSubmitRequestPath(site_url=site_url, feedpath=feedpath)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for submit_sitemap: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("submit_sitemap")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("submit_sitemap", "PUT", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="submit_sitemap",
        method="PUT",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def delete_sitemap(
    ...
    # Construct request model with validation
    try:
        _request = _models.WebmastersSitemapsDeleteRequest(
            path=_models.WebmastersSitemapsDeleteRequestPath(site_url=site_url, feedpath=feedpath)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for delete_sitemap: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("delete_sitemap")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("delete_sitemap", "DELETE", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="delete_sitemap",
        method="DELETE",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def list_sitemaps(
    ...
    # Construct request model with validation
    try:
        _request = _models.WebmastersSitemapsListRequest(
            path=_models.WebmastersSitemapsListRequestPath(site_url=site_url),
            query=_models.WebmastersSitemapsListRequestQuery(sitemap_index=sitemap_index)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_sitemaps: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/webmasters/v3/sites/{siteUrl}/sitemaps", _request.path.model_dump(by_alias=True)) if _request.path else "/webmasters/v3/sites/{siteUrl}/sitemaps"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_sitemaps")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_sitemaps", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_sitemaps",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_site(site_url: str = Field(..., alias="siteUrl", description="The site URL or domain property identifier as it appears in Search Console. Use the full URL format (e.g., http://www.example.com/) for URL-prefix properties or the domain format (e.g., sc-domain:example.com) for domain properties.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.WebmastersSitesGetRequest(
            path=_models.WebmastersSitesGetRequestPath(site_url=site_url)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_site: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/webmasters/v3/sites/{siteUrl}", _request.path.model_dump(by_alias=True)) if _request.path else "/webmasters/v3/sites/{siteUrl}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_site")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_site", "GET", _http_path, _request_id)
...
async def add_site(site_url: str = Field(..., alias="siteUrl", description="The complete URL of the website to add to Search Console (e.g., domain, subdomain, or URL prefix).")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.WebmastersSitesAddRequest(
            path=_models.WebmastersSitesAddRequestPath(site_url=site_url)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for add_site: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("add_site")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("add_site", "PUT", _http_path, _request_id)
...
async def delete_site(site_url: str = Field(..., alias="siteUrl", description="The site URL or domain property identifier as it appears in Search Console. Use the full URL format (e.g., http://www.example.com/) for URL-prefix properties or the domain format (e.g., sc-domain:example.com) for domain properties.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.WebmastersSitesDeleteRequest(
            path=_models.WebmastersSitesDeleteRequestPath(site_url=site_url)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for delete_site: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("delete_site")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("delete_site", "DELETE", _http_path, _request_id)
...
async def list_sites() -> dict[str, Any]:
    ...
    # Extract parameters for API call
    _http_path = "/webmasters/v3/sites"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_sites")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_sites", "GET", _http_path, _request_id)
...
async def inspect_url_index_status(
    ...
    # Construct request model with validation
    try:
        _request = _models.SearchconsoleUrlInspectionIndexInspectRequest(
            body=_models.SearchconsoleUrlInspectionIndexInspectRequestBody(inspection_url=inspection_url, language_code=language_code, site_url=site_url)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for inspect_url_index_status: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/v1/urlInspection/index:inspect"
    _http_body = _request.body.model_dump(by_alias=True, exclude_none=True) if _request.body else None
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("inspect_url_index_status")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("inspect_url_index_status", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="inspect_url_index_status",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def test_mobile_friendly(
    ...
    # Construct request model with validation
    try:
        _request = _models.SearchconsoleUrlTestingToolsMobileFriendlyTestRunRequest(
            body=_models.SearchconsoleUrlTestingToolsMobileFriendlyTestRunRequestBody(request_screenshot=request_screenshot, url=url)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for test_mobile_friendly: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/v1/urlTestingTools/mobileFriendlyTest:run"
    _http_body = _request.body.model_dump(by_alias=True, exclude_none=True) if _request.body else None
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("test_mobile_friendly")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("test_mobile_friendly", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="test_mobile_friendly",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python google_search_console_api_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
