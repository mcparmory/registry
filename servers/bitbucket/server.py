...
async def update_addon() -> dict[str, Any]:
    """Updates an installed Bitbucket Connect app for the authenticated installation, identified via JWT. Supports refreshing the existing descriptor, providing a new descriptor inline, or changing the descriptor URL to a new marketplace-registered location."""

    # Generate request ID
    _request_id = str(uuid.uuid4())

    # Log invocation
    _log_tool_invocation("update_addon", "PUT", "/addon", _request_id)
    ...
    _auth = _get_auth_for_operation("update_addon")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="update_addon",
        method="PUT",
        path=_http_path,
...
# Tags: Addon
@mcp.tool()
async def delete_addon() -> dict[str, Any]:
    """Permanently removes the installed Bitbucket Connect app associated with the authenticated JWT token. This endpoint is exclusively for Connect apps using JWT authentication and is not intended for apps managed through the Bitbucket Marketplace UI."""

    # Generate request ID
    _request_id = str(uuid.uuid4())

    # Log invocation
    _log_tool_invocation("delete_addon", "DELETE", "/addon", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_addon")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="delete_addon",
        method="DELETE",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def list_webhook_event_types() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_webhook_event_types", "GET", "/hook_events", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_webhook_event_types")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_webhook_event_types_by_subject(subject_type: Literal["repository", "workspace"] = Field(..., description="The entity type for which to retrieve subscribable webhook events. Note: team and user subject types are deprecated; use workspace instead.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_webhook_event_types_by_subject", "GET", "/hook_events/{subject_type}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_webhook_event_types_by_subject")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspace_repositories(
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_repositories", "GET", "/repositories/{workspace}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_repositories")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository(
    ...
    # Log invocation
    _log_tool_invocation("get_repository", "GET", "/repositories/{workspace}/{repo_slug}", _request_id)
...
async def create_repository(
    ...
    # Log invocation
    _log_tool_invocation("create_repository", "POST", "/repositories/{workspace}/{repo_slug}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_repository")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_repository(
    ...
    # Log invocation
    _log_tool_invocation("update_repository", "PUT", "/repositories/{workspace}/{repo_slug}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_repository")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_repository(
    ...
    # Log invocation
    _log_tool_invocation("delete_repository", "DELETE", "/repositories/{workspace}/{repo_slug}", _request_id)
...
async def list_branch_restrictions(
    ...
    # Log invocation
    _log_tool_invocation("list_branch_restrictions", "GET", "/repositories/{workspace}/{repo_slug}/branch-restrictions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_branch_restrictions")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_branch_restriction(
    ...
    # Log invocation
    _log_tool_invocation("create_branch_restriction", "POST", "/repositories/{workspace}/{repo_slug}/branch-restrictions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_branch_restriction")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_branch_restriction(
    ...
    # Log invocation
    _log_tool_invocation("get_branch_restriction", "GET", "/repositories/{workspace}/{repo_slug}/branch-restrictions/{id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_branch_restriction")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_branch_restriction(
    ...
    # Log invocation
    _log_tool_invocation("update_branch_restriction", "PUT", "/repositories/{workspace}/{repo_slug}/branch-restrictions/{id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_branch_restriction")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_branch_restriction(
    ...
    # Log invocation
    _log_tool_invocation("delete_branch_restriction", "DELETE", "/repositories/{workspace}/{repo_slug}/branch-restrictions/{id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_branch_restriction")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_branching_model(
    ...
    # Log invocation
    _log_tool_invocation("get_branching_model", "GET", "/repositories/{workspace}/{repo_slug}/branching-model", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_branching_model")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_branching_model_settings(
    ...
    # Log invocation
    _log_tool_invocation("get_branching_model_settings", "GET", "/repositories/{workspace}/{repo_slug}/branching-model/settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_branching_model_settings")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_branching_model_settings(
    ...
    # Log invocation
    _log_tool_invocation("update_branching_model_settings", "PUT", "/repositories/{workspace}/{repo_slug}/branching-model/settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_branching_model_settings")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_commit(
    ...
    # Log invocation
    _log_tool_invocation("get_commit", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}", _request_id)
...
async def approve_commit(
    ...
    # Log invocation
    _log_tool_invocation("approve_commit", "POST", "/repositories/{workspace}/{repo_slug}/commit/{commit}/approve", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("approve_commit")
    _http_headers.update(_auth.get("headers", {}))
...
async def unapprove_commit(
    ...
    # Log invocation
    _log_tool_invocation("unapprove_commit", "DELETE", "/repositories/{workspace}/{repo_slug}/commit/{commit}/approve", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("unapprove_commit")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commit_comments(
    ...
    # Log invocation
    _log_tool_invocation("list_commit_comments", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/comments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commit_comments")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_commit_comment(
    ...
    # Log invocation
    _log_tool_invocation("create_commit_comment", "POST", "/repositories/{workspace}/{repo_slug}/commit/{commit}/comments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_commit_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_commit_comment(
    comment_id: int = Field(..., description="The unique numeric identifier of the commit comment to retrieve.", json_schema_extra={'format': 'int64'}),
    ...
    try:
        _request = _models.GetRepositoriesByWorkspaceByRepoSlugCommitByCommitCommentsByCommentIdRequest(
            path=_models.GetRepositoriesByWorkspaceByRepoSlugCommitByCommitCommentsByCommentIdRequestPath(comment_id=comment_id, commit=commit, repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("get_commit_comment", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_commit_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_commit_comment(
    comment_id: int = Field(..., description="The unique numeric identifier of the comment to update.", json_schema_extra={'format': 'int64'}),
    ...
    try:
        _request = _models.PutRepositoriesCommitCommentsRequest(
            path=_models.PutRepositoriesCommitCommentsRequestPath(comment_id=comment_id, commit=commit, repo_slug=repo_slug, workspace=workspace),
    ...
    # Log invocation
    _log_tool_invocation("update_commit_comment", "PUT", "/repositories/{workspace}/{repo_slug}/commit/{commit}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_commit_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_commit_comment(
    comment_id: int = Field(..., description="The unique numeric identifier of the commit comment to delete.", json_schema_extra={'format': 'int64'}),
    ...
    try:
        _request = _models.DeleteRepositoriesCommitCommentsRequest(
            path=_models.DeleteRepositoriesCommitCommentsRequestPath(comment_id=comment_id, commit=commit, repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("delete_commit_comment", "DELETE", "/repositories/{workspace}/{repo_slug}/commit/{commit}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_commit_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_commit_property(
    ...
    # Log invocation
    _log_tool_invocation("get_commit_property", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_commit_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_commit_property(
    ...
    # Log invocation
    _log_tool_invocation("update_commit_property", "PUT", "/repositories/{workspace}/{repo_slug}/commit/{commit}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_commit_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_commit_property(
    ...
    # Log invocation
    _log_tool_invocation("delete_commit_property", "DELETE", "/repositories/{workspace}/{repo_slug}/commit/{commit}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_commit_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commit_pull_requests(
    ...
    pagelen: int | None = Field(30, description="The number of pull requests to return per page; controls pagination page size.", json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetRepositoriesCommitPullrequestsRequest(
            ...
            query=_models.GetRepositoriesCommitPullrequestsRequestQuery(pagelen=pagelen)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("list_commit_pull_requests", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/pullrequests", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commit_pull_requests")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commit_reports(
    ...
    # Log invocation
    _log_tool_invocation("list_commit_reports", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commit_reports")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_commit_report(
    ...
    # Log invocation
    _log_tool_invocation("get_commit_report", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports/{reportId}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_commit_report")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_or_update_commit_report(
    ...
    # Log invocation
    _log_tool_invocation("create_or_update_commit_report", "PUT", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports/{reportId}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_or_update_commit_report")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_commit_report(
    ...
    # Log invocation
    _log_tool_invocation("delete_commit_report", "DELETE", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports/{reportId}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_commit_report")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commit_report_annotations(
    ...
    # Log invocation
    _log_tool_invocation("list_commit_report_annotations", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports/{reportId}/annotations", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commit_report_annotations")
    _http_headers.update(_auth.get("headers", {}))
...
async def bulk_create_annotations(
    ...
    # Log invocation
    _log_tool_invocation("bulk_create_annotations", "POST", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports/{reportId}/annotations", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("bulk_create_annotations")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_commit_report_annotation(
    ...
    # Log invocation
    _log_tool_invocation("get_commit_report_annotation", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports/{reportId}/annotations/{annotationId}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_commit_report_annotation")
    _http_headers.update(_auth.get("headers", {}))
...
async def upsert_commit_report_annotation(
    ...
    # Log invocation
    _log_tool_invocation("upsert_commit_report_annotation", "PUT", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports/{reportId}/annotations/{annotationId}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("upsert_commit_report_annotation")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_commit_annotation(
    ...
    # Log invocation
    _log_tool_invocation("delete_commit_annotation", "DELETE", "/repositories/{workspace}/{repo_slug}/commit/{commit}/reports/{reportId}/annotations/{annotationId}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_commit_annotation")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commit_statuses(
    ...
    # Log invocation
    _log_tool_invocation("list_commit_statuses", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/statuses", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commit_statuses")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_commit_build_status(
    ...
    # Log invocation
    _log_tool_invocation("create_commit_build_status", "POST", "/repositories/{workspace}/{repo_slug}/commit/{commit}/statuses/build", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_commit_build_status")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_commit_build_status(
    ...
    # Log invocation
    _log_tool_invocation("get_commit_build_status", "GET", "/repositories/{workspace}/{repo_slug}/commit/{commit}/statuses/build/{key}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_commit_build_status")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_commit_build_status(
    ...
    # Log invocation
    _log_tool_invocation("update_commit_build_status", "PUT", "/repositories/{workspace}/{repo_slug}/commit/{commit}/statuses/build/{key}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_commit_build_status")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commits(
    ...
    # Log invocation
    _log_tool_invocation("list_commits", "GET", "/repositories/{workspace}/{repo_slug}/commits", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commits")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commits_with_filters(
    ...
    # Log invocation
    _log_tool_invocation("list_commits_with_filters", "POST", "/repositories/{workspace}/{repo_slug}/commits", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commits_with_filters")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commits_by_revision(
    ...
    # Log invocation
    _log_tool_invocation("list_commits_by_revision", "GET", "/repositories/{workspace}/{repo_slug}/commits/{revision}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commits_by_revision")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_commits_by_revision_post(
    ...
    # Log invocation
    _log_tool_invocation("list_commits_by_revision_post", "POST", "/repositories/{workspace}/{repo_slug}/commits/{revision}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_commits_by_revision_post")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_default_reviewers(
    ...
    # Log invocation
    _log_tool_invocation("list_default_reviewers", "GET", "/repositories/{workspace}/{repo_slug}/default-reviewers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_default_reviewers")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_default_reviewer(
    ...
    # Log invocation
    _log_tool_invocation("get_default_reviewer", "GET", "/repositories/{workspace}/{repo_slug}/default-reviewers/{target_username}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_default_reviewer")
    _http_headers.update(_auth.get("headers", {}))
...
async def add_default_reviewer(
    ...
    # Log invocation
    _log_tool_invocation("add_default_reviewer", "PUT", "/repositories/{workspace}/{repo_slug}/default-reviewers/{target_username}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("add_default_reviewer")
    _http_headers.update(_auth.get("headers", {}))
...
async def remove_default_reviewer(
    ...
    # Log invocation
    _log_tool_invocation("remove_default_reviewer", "DELETE", "/repositories/{workspace}/{repo_slug}/default-reviewers/{target_username}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("remove_default_reviewer")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_deploy_keys(
    ...
    # Log invocation
    _log_tool_invocation("list_deploy_keys", "GET", "/repositories/{workspace}/{repo_slug}/deploy-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_deploy_keys")
    _http_headers.update(_auth.get("headers", {}))
...
async def add_deploy_key(
    ...
    # Log invocation
    _log_tool_invocation("add_deploy_key", "POST", "/repositories/{workspace}/{repo_slug}/deploy-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("add_deploy_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_deploy_key(
    ...
    # Log invocation
    _log_tool_invocation("get_deploy_key", "GET", "/repositories/{workspace}/{repo_slug}/deploy-keys/{key_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_deploy_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_deploy_key(
    ...
    # Log invocation
    _log_tool_invocation("update_deploy_key", "PUT", "/repositories/{workspace}/{repo_slug}/deploy-keys/{key_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_deploy_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_deploy_key(
    ...
    # Log invocation
    _log_tool_invocation("delete_deploy_key", "DELETE", "/repositories/{workspace}/{repo_slug}/deploy-keys/{key_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_deploy_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_deployments(
    ...
    # Log invocation
    _log_tool_invocation("list_deployments", "GET", "/repositories/{workspace}/{repo_slug}/deployments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_deployments")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_deployment(
    ...
    # Log invocation
    _log_tool_invocation("get_deployment", "GET", "/repositories/{workspace}/{repo_slug}/deployments/{deployment_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_deployment")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_environment_variables(
    ...
    # Log invocation
    _log_tool_invocation("list_environment_variables", "GET", "/repositories/{workspace}/{repo_slug}/deployments_config/environments/{environment_uuid}/variables", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_environment_variables")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_environment_variable(
    ...
    # Log invocation
    _log_tool_invocation("create_environment_variable", "POST", "/repositories/{workspace}/{repo_slug}/deployments_config/environments/{environment_uuid}/variables", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_environment_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_environment_variable(
    ...
    # Log invocation
    _log_tool_invocation("update_environment_variable", "PUT", "/repositories/{workspace}/{repo_slug}/deployments_config/environments/{environment_uuid}/variables/{variable_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_environment_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_environment_variable(
    ...
    # Log invocation
    _log_tool_invocation("delete_environment_variable", "DELETE", "/repositories/{workspace}/{repo_slug}/deployments_config/environments/{environment_uuid}/variables/{variable_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_environment_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository_diff(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_diff", "GET", "/repositories/{workspace}/{repo_slug}/diff/{spec}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_diff")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_diffstat(
    ...
    # Log invocation
    _log_tool_invocation("get_diffstat", "GET", "/repositories/{workspace}/{repo_slug}/diffstat/{spec}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_diffstat")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_download_artifacts(
    ...
    # Log invocation
    _log_tool_invocation("list_download_artifacts", "GET", "/repositories/{workspace}/{repo_slug}/downloads", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_download_artifacts")
    _http_headers.update(_auth.get("headers", {}))
...
async def upload_download_artifact(
    ...
    # Log invocation
    _log_tool_invocation("upload_download_artifact", "POST", "/repositories/{workspace}/{repo_slug}/downloads", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("upload_download_artifact")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_download_artifact(
    ...
    # Log invocation
    _log_tool_invocation("get_download_artifact", "GET", "/repositories/{workspace}/{repo_slug}/downloads/{filename}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_download_artifact")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_download_artifact(
    ...
    # Log invocation
    _log_tool_invocation("delete_download_artifact", "DELETE", "/repositories/{workspace}/{repo_slug}/downloads/{filename}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_download_artifact")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_effective_branching_model(
    ...
    # Log invocation
    _log_tool_invocation("get_effective_branching_model", "GET", "/repositories/{workspace}/{repo_slug}/effective-branching-model", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_effective_branching_model")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_effective_default_reviewers(
    ...
    # Log invocation
    _log_tool_invocation("list_effective_default_reviewers", "GET", "/repositories/{workspace}/{repo_slug}/effective-default-reviewers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_effective_default_reviewers")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_environments(
    ...
    # Log invocation
    _log_tool_invocation("list_environments", "GET", "/repositories/{workspace}/{repo_slug}/environments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_environments")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_environment(
    ...
    # Log invocation
    _log_tool_invocation("create_environment", "POST", "/repositories/{workspace}/{repo_slug}/environments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_environment")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_environment(
    ...
    # Log invocation
    _log_tool_invocation("get_environment", "GET", "/repositories/{workspace}/{repo_slug}/environments/{environment_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_environment")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_environment(
    ...
    # Log invocation
    _log_tool_invocation("delete_environment", "DELETE", "/repositories/{workspace}/{repo_slug}/environments/{environment_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_environment")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_environment(
    ...
    # Log invocation
    _log_tool_invocation("update_environment", "POST", "/repositories/{workspace}/{repo_slug}/environments/{environment_uuid}/changes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_environment")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_file_history(
    ...
    # Log invocation
    _log_tool_invocation("list_file_history", "GET", "/repositories/{workspace}/{repo_slug}/filehistory/{commit}/{path}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_file_history")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_forks(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_forks", "GET", "/repositories/{workspace}/{repo_slug}/forks", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_forks")
    _http_headers.update(_auth.get("headers", {}))
...
async def fork_repository(
    ...
    # Log invocation
    _log_tool_invocation("fork_repository", "POST", "/repositories/{workspace}/{repo_slug}/forks", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("fork_repository")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_webhooks(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_webhooks", "GET", "/repositories/{workspace}/{repo_slug}/hooks", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_webhooks")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_repository_webhook(
    ...
    """Creates a new webhook on the specified repository, allowing event-driven notifications to be sent to an external URL. Supports HMAC signature verification via a shared secret and subscribes to one or more repository events."""

    # Construct request model with validation
    try:
        _request = _models.PostRepositoriesHooksRequest(
            path=_models.PostRepositoriesHooksRequestPath(repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for create_repository_webhook: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("create_repository_webhook", "POST", "/repositories/{workspace}/{repo_slug}/hooks", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_repository_webhook")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="create_repository_webhook",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def get_repository_webhook(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_webhook", "GET", "/repositories/{workspace}/{repo_slug}/hooks/{uid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_webhook")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_repository_webhook(
    ...
    # Log invocation
    _log_tool_invocation("update_repository_webhook", "PUT", "/repositories/{workspace}/{repo_slug}/hooks/{uid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_repository_webhook")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_repository_webhook(
    ...
    # Log invocation
    _log_tool_invocation("delete_repository_webhook", "DELETE", "/repositories/{workspace}/{repo_slug}/hooks/{uid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_repository_webhook")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_merge_base(
    ...
    # Log invocation
    _log_tool_invocation("get_merge_base", "GET", "/repositories/{workspace}/{repo_slug}/merge-base/{revspec}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_merge_base")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository_override_settings(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_override_settings", "GET", "/repositories/{workspace}/{repo_slug}/override-settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_override_settings")
    _http_headers.update(_auth.get("headers", {}))
...
async def set_repository_override_settings(
    ...
    # Log invocation
    _log_tool_invocation("set_repository_override_settings", "PUT", "/repositories/{workspace}/{repo_slug}/override-settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("set_repository_override_settings")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository_patch(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_patch", "GET", "/repositories/{workspace}/{repo_slug}/patch/{spec}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_patch")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_group_permissions(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_group_permissions", "GET", "/repositories/{workspace}/{repo_slug}/permissions-config/groups", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_group_permissions")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository_group_permission(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_group_permission", "GET", "/repositories/{workspace}/{repo_slug}/permissions-config/groups/{group_slug}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_group_permission")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Repositories
@mcp.tool()
async def set_group_repository_permission(
    group_slug: str = Field(..., description="The slug identifier of the group whose repository permission is being set or updated."),
    repo_slug: str = Field(..., description="The repository slug or UUID identifying the target repository; UUIDs must be surrounded by curly-braces."),
    workspace: str = Field(..., description="The workspace slug or UUID identifying the workspace that owns the repository; UUIDs must be surrounded by curly-braces.")
) -> dict[str, Any]:
    """Grants or updates an explicit permission level for a group on a specific repository. Requires admin permission on the repository and authentication via app passwords; valid permission levels are read, write, and admin."""

    # Construct request model with validation
    try:
        _request = _models.PutRepositoriesPermissionsConfigGroupsRequest(
            path=_models.PutRepositoriesPermissionsConfigGroupsRequestPath(group_slug=group_slug, repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for set_group_repository_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("set_group_repository_permission", "PUT", "/repositories/{workspace}/{repo_slug}/permissions-config/groups/{group_slug}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("set_group_repository_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="set_group_repository_permission",
        method="PUT",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
# Tags: Repositories
@mcp.tool()
async def delete_repository_group_permission(
    group_slug: str = Field(..., description="The slug identifier of the group whose repository permission should be deleted."),
    repo_slug: str = Field(..., description="The repository slug or UUID (surrounded by curly-braces) identifying the target repository."),
    workspace: str = Field(..., description="The workspace ID (slug) or UUID (surrounded by curly-braces) that owns the repository.")
) -> dict[str, Any]:
    """Removes the explicit group permission assigned to a specific repository, if one exists. Requires admin permission on the repository and authentication via app passwords."""

    # Construct request model with validation
    try:
        _request = _models.DeleteRepositoriesPermissionsConfigGroupsRequest(
            path=_models.DeleteRepositoriesPermissionsConfigGroupsRequestPath(group_slug=group_slug, repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for delete_repository_group_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("delete_repository_group_permission", "DELETE", "/repositories/{workspace}/{repo_slug}/permissions-config/groups/{group_slug}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_repository_group_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="delete_repository_group_permission",
        method="DELETE",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def list_repository_user_permissions(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_user_permissions", "GET", "/repositories/{workspace}/{repo_slug}/permissions-config/users", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_user_permissions")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_user_repository_permission(
    ...
    # Log invocation
    _log_tool_invocation("get_user_repository_permission", "GET", "/repositories/{workspace}/{repo_slug}/permissions-config/users/{selected_user_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_user_repository_permission")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Repositories
@mcp.tool()
async def set_repository_user_permission(
    repo_slug: str = Field(..., description="The repository slug or UUID identifying the target repository within the workspace."),
    selected_user_id: str = Field(..., description="The account identifier for the user whose permission is being updated, either as an Atlassian Account ID or a UUID wrapped in curly-braces."),
    workspace: str = Field(..., description="The workspace identifier, either as a slug or a UUID wrapped in curly-braces, that contains the repository.")
) -> dict[str, Any]:
    """Sets or updates the explicit permission level for a specific user on a repository. The user must be a workspace member (not the owner), and the caller must have admin access to the repository."""

    # Construct request model with validation
    try:
        _request = _models.PutRepositoriesPermissionsConfigUsersRequest(
            path=_models.PutRepositoriesPermissionsConfigUsersRequestPath(repo_slug=repo_slug, selected_user_id=selected_user_id, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for set_repository_user_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("set_repository_user_permission", "PUT", "/repositories/{workspace}/{repo_slug}/permissions-config/users/{selected_user_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("set_repository_user_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="set_repository_user_permission",
        method="PUT",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def delete_user_repository_permission(
    ...
    # Log invocation
    _log_tool_invocation("delete_user_repository_permission", "DELETE", "/repositories/{workspace}/{repo_slug}/permissions-config/users/{selected_user_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_user_repository_permission")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pipelines(
    workspace: str = Field(..., description="The workspace identifier, either the slug (short name) or the UUID surrounded by curly braces."),
    repo_slug: str = Field(..., description="The repository slug or identifier within the specified workspace."),
    creator_uuid: str | None = Field(None, alias="creator.uuid", description="Filter pipelines to only those created by the user with this UUID.", json_schema_extra={'format': 'uuid'}),
    ...
    created_on: str | None = Field(None, description="Filter pipelines by their creation date and time, provided in ISO 8601 date-time format.", json_schema_extra={'format': 'date-time'}),
    ...
    page: int | None = Field(1, description="The page number to retrieve in the paginated result set, starting at 1.", ge=1, json_schema_extra={'format': 'int32'}),
    pagelen: int | None = Field(10, description="The maximum number of pipeline results to return per page, between 1 and 100.", ge=1, le=100, json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_pipelines", "GET", "/repositories/{workspace}/{repo_slug}/pipelines", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipelines")
    _http_headers.update(_auth.get("headers", {}))
...
async def trigger_pipeline(
    ...
    # Log invocation
    _log_tool_invocation("trigger_pipeline", "POST", "/repositories/{workspace}/{repo_slug}/pipelines", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("trigger_pipeline")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pipeline_caches(
    ...
    # Log invocation
    _log_tool_invocation("list_pipeline_caches", "GET", "/repositories/{workspace}/{repo_slug}/pipelines-config/caches", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipeline_caches")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_pipeline_caches(
    ...
    # Log invocation
    _log_tool_invocation("delete_pipeline_caches", "DELETE", "/repositories/{workspace}/{repo_slug}/pipelines-config/caches", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_pipeline_caches")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_pipeline_cache(
    ...
    # Log invocation
    _log_tool_invocation("delete_pipeline_cache", "DELETE", "/repositories/{workspace}/{repo_slug}/pipelines-config/caches/{cache_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_pipeline_cache")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline_cache_content_uri(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_cache_content_uri", "GET", "/repositories/{workspace}/{repo_slug}/pipelines-config/caches/{cache_uuid}/content-uri", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_cache_content_uri")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_runners(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_runners", "GET", "/repositories/{workspace}/{repo_slug}/pipelines-config/runners", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_runners")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_repository_runner(
    ...
    # Log invocation
    _log_tool_invocation("create_repository_runner", "POST", "/repositories/{workspace}/{repo_slug}/pipelines-config/runners", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_repository_runner")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository_runner(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_runner", "GET", "/repositories/{workspace}/{repo_slug}/pipelines-config/runners/{runner_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_runner")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_repository_runner(
    ...
    # Log invocation
    _log_tool_invocation("update_repository_runner", "PUT", "/repositories/{workspace}/{repo_slug}/pipelines-config/runners/{runner_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_repository_runner")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_repository_runner(
    ...
    # Log invocation
    _log_tool_invocation("delete_repository_runner", "DELETE", "/repositories/{workspace}/{repo_slug}/pipelines-config/runners/{runner_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_repository_runner")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline", "GET", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pipeline_steps(
    ...
    # Log invocation
    _log_tool_invocation("list_pipeline_steps", "GET", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}/steps", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipeline_steps")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline_step(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_step", "GET", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}/steps/{step_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_step")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline_step_log(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_step_log", "GET", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}/steps/{step_uuid}/log", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_step_log")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline_step_log_container(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_step_log_container", "GET", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}/steps/{step_uuid}/logs/{log_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_step_log_container")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline_step_test_report(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_step_test_report", "GET", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}/steps/{step_uuid}/test_reports", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_step_test_report")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pipeline_step_test_cases(
    ...
    # Log invocation
    _log_tool_invocation("list_pipeline_step_test_cases", "GET", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}/steps/{step_uuid}/test_reports/test_cases", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipeline_step_test_cases")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_test_case_reasons(
    ...
    # Log invocation
    _log_tool_invocation("list_test_case_reasons", "GET", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}/steps/{step_uuid}/test_reports/test_cases/{test_case_uuid}/test_case_reasons", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_test_case_reasons")
    _http_headers.update(_auth.get("headers", {}))
...
async def stop_pipeline(
    ...
    # Log invocation
    _log_tool_invocation("stop_pipeline", "POST", "/repositories/{workspace}/{repo_slug}/pipelines/{pipeline_uuid}/stopPipeline", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("stop_pipeline")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipelines_config(
    ...
    # Log invocation
    _log_tool_invocation("get_pipelines_config", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipelines_config")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Pipelines
@mcp.tool()
async def update_pipelines_config(
    workspace: str = Field(..., description="The workspace identifier, either as a slug (short name) or as a UUID surrounded by curly braces."),
    repo_slug: str = Field(..., description="The repository slug (URL-friendly name) that uniquely identifies the repository within the workspace."),
    body: _models.PutRepositoriesPipelinesConfigBody | None = Field(None, description="The pipelines configuration object containing the settings to apply to the repository.")
) -> dict[str, Any]:
    """Update the Pipelines configuration settings for a specific repository. Use this to enable, disable, or modify pipeline behavior for the given workspace and repository."""

    # Construct request model with validation
    try:
        _request = _models.PutRepositoriesPipelinesConfigRequest(
            path=_models.PutRepositoriesPipelinesConfigRequestPath(workspace=workspace, repo_slug=repo_slug),
            body=_models.PutRepositoriesPipelinesConfigRequestBody(body=body)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for update_pipelines_config: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("update_pipelines_config", "PUT", "/repositories/{workspace}/{repo_slug}/pipelines_config", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_pipelines_config")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="update_pipelines_config",
        method="PUT",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def set_pipeline_next_build_number(
    ...
    # Log invocation
    _log_tool_invocation("set_pipeline_next_build_number", "PUT", "/repositories/{workspace}/{repo_slug}/pipelines_config/build_number", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("set_pipeline_next_build_number")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pipeline_schedules(
    ...
    # Log invocation
    _log_tool_invocation("list_pipeline_schedules", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config/schedules", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipeline_schedules")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_pipeline_schedule(
    ...
    # Log invocation
    _log_tool_invocation("create_pipeline_schedule", "POST", "/repositories/{workspace}/{repo_slug}/pipelines_config/schedules", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_pipeline_schedule")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline_schedule(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_schedule", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config/schedules/{schedule_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_schedule")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_pipeline_schedule(
    ...
    # Log invocation
    _log_tool_invocation("update_pipeline_schedule", "PUT", "/repositories/{workspace}/{repo_slug}/pipelines_config/schedules/{schedule_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_pipeline_schedule")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_pipeline_schedule(
    ...
    # Log invocation
    _log_tool_invocation("delete_pipeline_schedule", "DELETE", "/repositories/{workspace}/{repo_slug}/pipelines_config/schedules/{schedule_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_pipeline_schedule")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_schedule_executions(
    ...
    # Log invocation
    _log_tool_invocation("list_schedule_executions", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config/schedules/{schedule_uuid}/executions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_schedule_executions")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository_ssh_key_pair(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_ssh_key_pair", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config/ssh/key_pair", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_ssh_key_pair")
    _http_headers.update(_auth.get("headers", {}))
...
async def set_repository_ssh_key_pair(
    ...
    # Log invocation
    _log_tool_invocation("set_repository_ssh_key_pair", "PUT", "/repositories/{workspace}/{repo_slug}/pipelines_config/ssh/key_pair", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("set_repository_ssh_key_pair")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_repository_ssh_key_pair(
    ...
    # Log invocation
    _log_tool_invocation("delete_repository_ssh_key_pair", "DELETE", "/repositories/{workspace}/{repo_slug}/pipelines_config/ssh/key_pair", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_repository_ssh_key_pair")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pipeline_ssh_known_hosts(
    ...
    # Log invocation
    _log_tool_invocation("list_pipeline_ssh_known_hosts", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config/ssh/known_hosts", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipeline_ssh_known_hosts")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_ssh_known_host(
    ...
    # Log invocation
    _log_tool_invocation("create_ssh_known_host", "POST", "/repositories/{workspace}/{repo_slug}/pipelines_config/ssh/known_hosts", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_ssh_known_host")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline_ssh_known_host(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_ssh_known_host", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config/ssh/known_hosts/{known_host_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_ssh_known_host")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_pipeline_known_host(
    ...
    # Log invocation
    _log_tool_invocation("update_pipeline_known_host", "PUT", "/repositories/{workspace}/{repo_slug}/pipelines_config/ssh/known_hosts/{known_host_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_pipeline_known_host")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_known_host(
    ...
    # Log invocation
    _log_tool_invocation("delete_known_host", "DELETE", "/repositories/{workspace}/{repo_slug}/pipelines_config/ssh/known_hosts/{known_host_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_known_host")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_pipeline_variables(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_pipeline_variables", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config/variables", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_pipeline_variables")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_repository_pipeline_variable(
    ...
    # Log invocation
    _log_tool_invocation("create_repository_pipeline_variable", "POST", "/repositories/{workspace}/{repo_slug}/pipelines_config/variables", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_repository_pipeline_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pipeline_variable(
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_variable", "GET", "/repositories/{workspace}/{repo_slug}/pipelines_config/variables/{variable_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_repository_pipeline_variable(
    ...
    # Log invocation
    _log_tool_invocation("update_repository_pipeline_variable", "PUT", "/repositories/{workspace}/{repo_slug}/pipelines_config/variables/{variable_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_repository_pipeline_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_repository_pipeline_variable(
    ...
    # Log invocation
    _log_tool_invocation("delete_repository_pipeline_variable", "DELETE", "/repositories/{workspace}/{repo_slug}/pipelines_config/variables/{variable_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_repository_pipeline_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository_property(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_property", "GET", "/repositories/{workspace}/{repo_slug}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_repository_property(
    ...
    # Log invocation
    _log_tool_invocation("update_repository_property", "PUT", "/repositories/{workspace}/{repo_slug}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_repository_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_repository_property(
    ...
    # Log invocation
    _log_tool_invocation("delete_repository_property", "DELETE", "/repositories/{workspace}/{repo_slug}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_repository_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pull_requests(
    ...
    # Log invocation
    _log_tool_invocation("list_pull_requests", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests", _request_id)
...
async def create_pull_request(
    ...
    # Log invocation
    _log_tool_invocation("create_pull_request", "POST", "/repositories/{workspace}/{repo_slug}/pullrequests", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_pull_request")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pull_request_activity(
    ...
    # Log invocation
    _log_tool_invocation("list_pull_request_activity", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/activity", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pull_request_activity")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pull_request(
    ...
    # Log invocation
    _log_tool_invocation("get_pull_request", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}", _request_id)
...
async def update_pull_request(
    ...
    # Log invocation
    _log_tool_invocation("update_pull_request", "PUT", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_pull_request")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pull_request_activity_by_id(
    ...
    # Log invocation
    _log_tool_invocation("list_pull_request_activity_by_id", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/activity", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pull_request_activity_by_id")
    _http_headers.update(_auth.get("headers", {}))
...
async def approve_pull_request(
    ...
    # Log invocation
    _log_tool_invocation("approve_pull_request", "POST", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/approve", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("approve_pull_request")
    _http_headers.update(_auth.get("headers", {}))
...
async def unapprove_pull_request(
    ...
    # Log invocation
    _log_tool_invocation("unapprove_pull_request", "DELETE", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/approve", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("unapprove_pull_request")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pull_request_comments(
    ...
    # Log invocation
    _log_tool_invocation("list_pull_request_comments", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/comments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pull_request_comments")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_pull_request_comment(
    ...
    # Log invocation
    _log_tool_invocation("create_pull_request_comment", "POST", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/comments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_pull_request_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pull_request_comment(
    comment_id: int = Field(..., description="The unique numeric identifier of the comment to retrieve.", json_schema_extra={'format': 'int64'}),
    ...
    try:
        _request = _models.GetRepositoriesByWorkspaceByRepoSlugPullrequestsByPullRequestIdCommentsByCommentIdRequest(
            path=_models.GetRepositoriesByWorkspaceByRepoSlugPullrequestsByPullRequestIdCommentsByCommentIdRequestPath(comment_id=comment_id, pull_request_id=pull_request_id, repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("get_pull_request_comment", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pull_request_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_pull_request_comment(
    ...
    try:
        _request = _models.PutRepositoriesPullrequestsCommentsRequest(
            path=_models.PutRepositoriesPullrequestsCommentsRequestPath(comment_id=comment_id, pull_request_id=pull_request_id, repo_slug=repo_slug, workspace=workspace),
    ...
    # Log invocation
    _log_tool_invocation("update_pull_request_comment", "PUT", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_pull_request_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_pull_request_comment(
    comment_id: int = Field(..., description="The unique numeric identifier of the comment to delete.", json_schema_extra={'format': 'int64'}),
    pull_request_id: int = Field(..., description="The unique numeric identifier of the pull request containing the comment."),
    repo_slug: str = Field(..., description="The repository slug (URL-friendly name) or the repository UUID surrounded by curly-braces."),
    workspace: str = Field(..., description="The workspace slug (URL-friendly name) or the workspace UUID surrounded by curly-braces.")
) -> dict[str, Any]:
    ...
    try:
        _request = _models.DeleteRepositoriesPullrequestsCommentsRequest(
            path=_models.DeleteRepositoriesPullrequestsCommentsRequestPath(comment_id=comment_id, pull_request_id=pull_request_id, repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("delete_pull_request_comment", "DELETE", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_pull_request_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def resolve_pull_request_comment(
    comment_id: int = Field(..., description="The unique numeric identifier of the comment thread to resolve.", json_schema_extra={'format': 'int64'}),
    ...
    try:
        _request = _models.PostRepositoriesPullrequestsCommentsResolveRequest(
            path=_models.PostRepositoriesPullrequestsCommentsResolveRequestPath(comment_id=comment_id, pull_request_id=pull_request_id, repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("resolve_pull_request_comment", "POST", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/comments/{comment_id}/resolve", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("resolve_pull_request_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def reopen_pull_request_comment_thread(
    comment_id: int = Field(..., description="The unique numeric identifier of the comment whose resolved status should be removed.", json_schema_extra={'format': 'int64'}),
    pull_request_id: int = Field(..., description="The unique numeric identifier of the pull request containing the comment thread."),
    repo_slug: str = Field(..., description="The repository slug or UUID identifying the repository. UUIDs must be surrounded by curly-braces."),
    workspace: str = Field(..., description="The workspace slug or UUID identifying the workspace. UUIDs must be surrounded by curly-braces.")
) -> dict[str, Any]:
    ...
    try:
        _request = _models.DeleteRepositoriesPullrequestsCommentsResolveRequest(
            path=_models.DeleteRepositoriesPullrequestsCommentsResolveRequestPath(comment_id=comment_id, pull_request_id=pull_request_id, repo_slug=repo_slug, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("reopen_pull_request_comment_thread", "DELETE", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/comments/{comment_id}/resolve", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("reopen_pull_request_comment_thread")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pull_request_commits(
    ...
    # Log invocation
    _log_tool_invocation("list_pull_request_commits", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/commits", _request_id)
...
async def decline_pull_request(
    ...
    # Log invocation
    _log_tool_invocation("decline_pull_request", "POST", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/decline", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("decline_pull_request")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pull_request_diff(
    ...
    # Log invocation
    _log_tool_invocation("get_pull_request_diff", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/diff", _request_id)
...
async def get_pull_request_diffstat(
    ...
    # Log invocation
    _log_tool_invocation("get_pull_request_diffstat", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/diffstat", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pull_request_diffstat")
    _http_headers.update(_auth.get("headers", {}))
...
async def merge_pull_request(
    ...
    merge_strategy: Literal["merge_commit", "squash", "fast_forward", "squash_fast_forward", "rebase_fast_forward", "rebase_merge"] | None = Field('merge_commit', description="The strategy used to merge the pull request into the target branch; controls how commits are combined or rewritten.")
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("merge_pull_request", "POST", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/merge", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("merge_pull_request")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pull_request_merge_task_status(
    ...
    # Log invocation
    _log_tool_invocation("get_pull_request_merge_task_status", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/merge/task-status/{task_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pull_request_merge_task_status")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pull_request_patch(
    ...
    # Log invocation
    _log_tool_invocation("get_pull_request_patch", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/patch", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pull_request_patch")
    _http_headers.update(_auth.get("headers", {}))
...
async def request_pull_request_changes(
    ...
    # Log invocation
    _log_tool_invocation("request_pull_request_changes", "POST", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/request-changes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("request_pull_request_changes")
    _http_headers.update(_auth.get("headers", {}))
...
async def remove_pull_request_change_request(
    ...
    # Log invocation
    _log_tool_invocation("remove_pull_request_change_request", "DELETE", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/request-changes", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("remove_pull_request_change_request")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pull_request_statuses(
    ...
    # Log invocation
    _log_tool_invocation("list_pull_request_statuses", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/statuses", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pull_request_statuses")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_pull_request_tasks(
    ...
    # Log invocation
    _log_tool_invocation("list_pull_request_tasks", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/tasks", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pull_request_tasks")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_pull_request_task(
    ...
    # Log invocation
    _log_tool_invocation("create_pull_request_task", "POST", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/tasks", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_pull_request_task")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pull_request_task(
    ...
    repo_slug: str = Field(..., description="The repository slug or UUID identifying the repository. UUIDs must be surrounded by curly-braces."),
    task_id: int = Field(..., description="The unique numeric ID of the task to retrieve.", json_schema_extra={'format': 'int64'}),
    workspace: str = Field(..., description="The workspace slug or UUID identifying the workspace. UUIDs must be surrounded by curly-braces.")
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetRepositoriesByWorkspaceByRepoSlugPullrequestsByPullRequestIdTasksByTaskIdRequest(
            path=_models.GetRepositoriesByWorkspaceByRepoSlugPullrequestsByPullRequestIdTasksByTaskIdRequestPath(pull_request_id=pull_request_id, repo_slug=repo_slug, task_id=task_id, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("get_pull_request_task", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/tasks/{task_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pull_request_task")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_pull_request_task(
    ...
    repo_slug: str = Field(..., description="The repository slug or UUID (surrounded by curly-braces) identifying the repository containing the pull request."),
    task_id: int = Field(..., description="The unique numeric ID of the task to update.", json_schema_extra={'format': 'int64'}),
    workspace: str = Field(..., description="The workspace slug or UUID (surrounded by curly-braces) identifying the workspace that owns the repository."),
    ...
    try:
        _request = _models.PutRepositoriesPullrequestsTasksRequest(
            path=_models.PutRepositoriesPullrequestsTasksRequestPath(pull_request_id=pull_request_id, repo_slug=repo_slug, task_id=task_id, workspace=workspace),
    ...
    # Log invocation
    _log_tool_invocation("update_pull_request_task", "PUT", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/tasks/{task_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_pull_request_task")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_pull_request_task(
    ...
    repo_slug: str = Field(..., description="The repository slug or UUID identifying the repository. UUIDs must be surrounded by curly-braces."),
    task_id: int = Field(..., description="The unique numeric ID of the task to delete.", json_schema_extra={'format': 'int64'}),
    workspace: str = Field(..., description="The workspace slug or UUID identifying the workspace. UUIDs must be surrounded by curly-braces.")
) -> dict[str, Any]:
    ...
    try:
        _request = _models.DeleteRepositoriesPullrequestsTasksRequest(
            path=_models.DeleteRepositoriesPullrequestsTasksRequestPath(pull_request_id=pull_request_id, repo_slug=repo_slug, task_id=task_id, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("delete_pull_request_task", "DELETE", "/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/tasks/{task_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_pull_request_task")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_pull_request_property(
    ...
    # Log invocation
    _log_tool_invocation("get_pull_request_property", "GET", "/repositories/{workspace}/{repo_slug}/pullrequests/{pullrequest_id}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pull_request_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_pull_request_property(
    ...
    # Log invocation
    _log_tool_invocation("update_pull_request_property", "PUT", "/repositories/{workspace}/{repo_slug}/pullrequests/{pullrequest_id}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_pull_request_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_pull_request_property(
    ...
    # Log invocation
    _log_tool_invocation("delete_pull_request_property", "DELETE", "/repositories/{workspace}/{repo_slug}/pullrequests/{pullrequest_id}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_pull_request_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_refs(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_refs", "GET", "/repositories/{workspace}/{repo_slug}/refs", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_refs")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_branches(
    ...
    # Log invocation
    _log_tool_invocation("list_branches", "GET", "/repositories/{workspace}/{repo_slug}/refs/branches", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_branches")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_branch(
    ...
    # Log invocation
    _log_tool_invocation("create_branch", "POST", "/repositories/{workspace}/{repo_slug}/refs/branches", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_branch")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_branch(
    ...
    # Log invocation
    _log_tool_invocation("get_branch", "GET", "/repositories/{workspace}/{repo_slug}/refs/branches/{name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_branch")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_branch(
    ...
    # Log invocation
    _log_tool_invocation("delete_branch", "DELETE", "/repositories/{workspace}/{repo_slug}/refs/branches/{name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_branch")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_tags(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_tags", "GET", "/repositories/{workspace}/{repo_slug}/refs/tags", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_tags")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_tag(
    ...
    # Log invocation
    _log_tool_invocation("create_tag", "POST", "/repositories/{workspace}/{repo_slug}/refs/tags", _request_id)
...
async def get_repository_tag(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_tag", "GET", "/repositories/{workspace}/{repo_slug}/refs/tags/{name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_tag")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_tag(
    ...
    # Log invocation
    _log_tool_invocation("delete_tag", "DELETE", "/repositories/{workspace}/{repo_slug}/refs/tags/{name}", _request_id)
...
async def get_repository_root_src(
    ...
    # Log invocation
    _log_tool_invocation("get_repository_root_src", "GET", "/repositories/{workspace}/{repo_slug}/src", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_root_src")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_commit_with_files(
    ...
    # Log invocation
    _log_tool_invocation("create_commit_with_files", "POST", "/repositories/{workspace}/{repo_slug}/src", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_commit_with_files")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_repository_src(
    ...
    max_depth: int | None = Field(1, description="Maximum directory depth to recurse into when listing directory contents. Performs a breadth-first traversal; very large values may cause the request to time out with a 555 error. Defaults to 1 (non-recursive, direct children only).")
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_repository_src", "GET", "/repositories/{workspace}/{repo_slug}/src/{commit}/{path}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_repository_src")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_watchers(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_watchers", "GET", "/repositories/{workspace}/{repo_slug}/watchers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_watchers")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_snippet() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_snippet", "POST", "/snippets", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_snippet")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspace_snippets(
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_snippets", "GET", "/snippets/{workspace}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_snippets")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_workspace_snippet(workspace: str = Field(..., description="The workspace in which to create the snippet, identified by either its slug (human-readable ID) or its UUID surrounded by curly-braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_workspace_snippet", "POST", "/snippets/{workspace}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_workspace_snippet")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_snippet(
    ...
    # Log invocation
    _log_tool_invocation("get_snippet", "GET", "/snippets/{workspace}/{encoded_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_snippet")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_snippet(
    ...
    # Log invocation
    _log_tool_invocation("update_snippet", "PUT", "/snippets/{workspace}/{encoded_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_snippet")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_snippet(
    ...
    # Log invocation
    _log_tool_invocation("delete_snippet", "DELETE", "/snippets/{workspace}/{encoded_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_snippet")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_snippet_comments(
    ...
    # Log invocation
    _log_tool_invocation("list_snippet_comments", "GET", "/snippets/{workspace}/{encoded_id}/comments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_snippet_comments")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_snippet_comment(
    ...
    # Log invocation
    _log_tool_invocation("create_snippet_comment", "POST", "/snippets/{workspace}/{encoded_id}/comments", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_snippet_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_snippet_comment(
    ...
    try:
        _request = _models.GetSnippetsByWorkspaceByEncodedIdCommentsByCommentIdRequest(
            path=_models.GetSnippetsByWorkspaceByEncodedIdCommentsByCommentIdRequestPath(comment_id=comment_id, encoded_id=encoded_id, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("get_snippet_comment", "GET", "/snippets/{workspace}/{encoded_id}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_snippet_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_snippet_comment(
    ...
    try:
        _request = _models.PutSnippetsCommentsRequest(
            path=_models.PutSnippetsCommentsRequestPath(comment_id=comment_id, encoded_id=encoded_id, workspace=workspace),
    ...
    # Log invocation
    _log_tool_invocation("update_snippet_comment", "PUT", "/snippets/{workspace}/{encoded_id}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_snippet_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_snippet_comment(
    ...
    try:
        _request = _models.DeleteSnippetsCommentsRequest(
            path=_models.DeleteSnippetsCommentsRequestPath(comment_id=comment_id, encoded_id=encoded_id, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("delete_snippet_comment", "DELETE", "/snippets/{workspace}/{encoded_id}/comments/{comment_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_snippet_comment")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_snippet_commits(
    ...
    # Log invocation
    _log_tool_invocation("list_snippet_commits", "GET", "/snippets/{workspace}/{encoded_id}/commits", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_snippet_commits")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_snippet_commit_changes(
    ...
    # Log invocation
    _log_tool_invocation("get_snippet_commit_changes", "GET", "/snippets/{workspace}/{encoded_id}/commits/{revision}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_snippet_commit_changes")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_snippet_file_content(
    ...
    # Log invocation
    _log_tool_invocation("get_snippet_file_content", "GET", "/snippets/{workspace}/{encoded_id}/files/{path}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_snippet_file_content")
    _http_headers.update(_auth.get("headers", {}))
...
async def check_snippet_watch_status(
    ...
    # Log invocation
    _log_tool_invocation("check_snippet_watch_status", "GET", "/snippets/{workspace}/{encoded_id}/watch", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("check_snippet_watch_status")
    _http_headers.update(_auth.get("headers", {}))
...
async def watch_snippet(
    ...
    # Log invocation
    _log_tool_invocation("watch_snippet", "PUT", "/snippets/{workspace}/{encoded_id}/watch", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("watch_snippet")
    _http_headers.update(_auth.get("headers", {}))
...
async def unwatch_snippet(
    ...
    # Log invocation
    _log_tool_invocation("unwatch_snippet", "DELETE", "/snippets/{workspace}/{encoded_id}/watch", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("unwatch_snippet")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_snippet_revision(
    ...
    # Log invocation
    _log_tool_invocation("get_snippet_revision", "GET", "/snippets/{workspace}/{encoded_id}/{node_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_snippet_revision")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_snippet_at_revision(
    ...
    # Log invocation
    _log_tool_invocation("update_snippet_at_revision", "PUT", "/snippets/{workspace}/{encoded_id}/{node_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_snippet_at_revision")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_snippet_revision(
    ...
    # Log invocation
    _log_tool_invocation("delete_snippet_revision", "DELETE", "/snippets/{workspace}/{encoded_id}/{node_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_snippet_revision")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_snippet_file_contents(
    ...
    # Log invocation
    _log_tool_invocation("get_snippet_file_contents", "GET", "/snippets/{workspace}/{encoded_id}/{node_id}/files/{path}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_snippet_file_contents")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_snippet_diff(
    ...
    # Log invocation
    _log_tool_invocation("get_snippet_diff", "GET", "/snippets/{workspace}/{encoded_id}/{revision}/diff", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_snippet_diff")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_snippet_patch(
    ...
    # Log invocation
    _log_tool_invocation("get_snippet_patch", "GET", "/snippets/{workspace}/{encoded_id}/{revision}/patch", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_snippet_patch")
    _http_headers.update(_auth.get("headers", {}))
...
async def search_team_code(
    ...
    page: int | None = Field(1, description="The page number of search results to retrieve, starting at 1 for the first page.", json_schema_extra={'format': 'int32'}),
    pagelen: int | None = Field(10, description="The number of search results to return per page; controls pagination granularity.", json_schema_extra={'format': 'int32'})
) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetTeamsSearchCodeRequest(
            ...
            query=_models.GetTeamsSearchCodeRequestQuery(search_query=search_query, page=page, pagelen=pagelen)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("search_team_code", "GET", "/teams/{username}/search/code", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("search_team_code")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_current_user() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_current_user", "GET", "/user", _request_id)
...
async def list_user_emails() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_user_emails", "GET", "/user/emails", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_user_emails")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_email(email: str = Field(..., description="The full email address to look up among the authenticated user's registered email addresses.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_email", "GET", "/user/emails/{email}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_email")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspaces(
    ...
    # Log invocation
    _log_tool_invocation("list_workspaces", "GET", "/user/workspaces", _request_id)
...
async def get_workspace_permission(workspace: str = Field(..., description="The workspace identifier, either as a slug (short name) or as a UUID surrounded by curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_workspace_permission", "GET", "/user/workspaces/{workspace}/permission", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_workspace_permission")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspace_repository_permissions_for_user(
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_repository_permissions_for_user", "GET", "/user/workspaces/{workspace}/permissions/repositories", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_repository_permissions_for_user")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_user(selected_user: str = Field(..., description="The identifier of the user to retrieve, accepted as either an Atlassian Account ID or a UUID wrapped in curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_user", "GET", "/users/{selected_user}", _request_id)
...
async def list_user_gpg_keys(selected_user: str = Field(..., description="The identifier of the user whose GPG keys will be listed, accepted as either an Atlassian Account ID or an account UUID wrapped in curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_user_gpg_keys", "GET", "/users/{selected_user}/gpg-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_user_gpg_keys")
    _http_headers.update(_auth.get("headers", {}))
...
async def add_gpg_key(
    ...
    # Log invocation
    _log_tool_invocation("add_gpg_key", "POST", "/users/{selected_user}/gpg-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("add_gpg_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_user_gpg_key(
    ...
    # Log invocation
    _log_tool_invocation("get_user_gpg_key", "GET", "/users/{selected_user}/gpg-keys/{fingerprint}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_user_gpg_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_user_gpg_key(
    ...
    # Log invocation
    _log_tool_invocation("delete_user_gpg_key", "DELETE", "/users/{selected_user}/gpg-keys/{fingerprint}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_user_gpg_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_user_app_property(
    ...
    # Log invocation
    _log_tool_invocation("get_user_app_property", "GET", "/users/{selected_user}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_user_app_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_user_app_property(
    ...
    # Log invocation
    _log_tool_invocation("update_user_app_property", "PUT", "/users/{selected_user}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_user_app_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_user_app_property(
    ...
    # Log invocation
    _log_tool_invocation("delete_user_app_property", "DELETE", "/users/{selected_user}/properties/{app_key}/{property_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_user_app_property")
    _http_headers.update(_auth.get("headers", {}))
...
async def search_user_code(
    ...
    try:
        _request = _models.GetUsersSearchCodeRequest(
            ...
            query=_models.GetUsersSearchCodeRequestQuery(search_query=search_query, page=page, pagelen=pagelen)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("search_user_code", "GET", "/users/{selected_user}/search/code", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("search_user_code")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_user_ssh_keys(selected_user: str = Field(..., description="The identifier of the user whose SSH keys will be listed — either an Atlassian Account ID or an account UUID wrapped in curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_user_ssh_keys", "GET", "/users/{selected_user}/ssh-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_user_ssh_keys")
    _http_headers.update(_auth.get("headers", {}))
...
async def add_ssh_key(
    ...
    # Log invocation
    _log_tool_invocation("add_ssh_key", "POST", "/users/{selected_user}/ssh-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("add_ssh_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_user_ssh_key(
    ...
    # Log invocation
    _log_tool_invocation("get_user_ssh_key", "GET", "/users/{selected_user}/ssh-keys/{key_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_user_ssh_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_ssh_key(
    ...
    # Log invocation
    _log_tool_invocation("update_ssh_key", "PUT", "/users/{selected_user}/ssh-keys/{key_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_ssh_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_user_ssh_key(
    ...
    # Log invocation
    _log_tool_invocation("delete_user_ssh_key", "DELETE", "/users/{selected_user}/ssh-keys/{key_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_user_ssh_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_workspace(workspace: str = Field(..., description="The unique identifier for the workspace, accepted as either a slug (short name) or a UUID wrapped in curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_workspace", "GET", "/workspaces/{workspace}", _request_id)
...
async def list_workspace_webhooks(workspace: str = Field(..., description="The unique identifier for the workspace, accepted as either a slug (short name) or a UUID surrounded by curly-braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_webhooks", "GET", "/workspaces/{workspace}/hooks", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_webhooks")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Workspaces, Webhooks
@mcp.tool()
async def create_workspace_webhook(workspace: str = Field(..., description="The workspace identifier, either the workspace slug (e.g. my-workspace) or the workspace UUID surrounded by curly braces.")) -> dict[str, Any]:
    """Creates a new webhook on the specified workspace, enabling event notifications for all repositories within that workspace. Requires the webhook scope plus any scopes relevant to the subscribed events, and can only be performed by workspace owners."""

    # Construct request model with validation
    try:
        _request = _models.PostWorkspacesHooksRequest(
            path=_models.PostWorkspacesHooksRequestPath(workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for create_workspace_webhook: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("create_workspace_webhook", "POST", "/workspaces/{workspace}/hooks", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_workspace_webhook")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="create_workspace_webhook",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def get_workspace_webhook(
    ...
    # Log invocation
    _log_tool_invocation("get_workspace_webhook", "GET", "/workspaces/{workspace}/hooks/{uid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_workspace_webhook")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_workspace_webhook(
    ...
    # Log invocation
    _log_tool_invocation("update_workspace_webhook", "PUT", "/workspaces/{workspace}/hooks/{uid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_workspace_webhook")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_workspace_webhook(
    ...
    # Log invocation
    _log_tool_invocation("delete_workspace_webhook", "DELETE", "/workspaces/{workspace}/hooks/{uid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_workspace_webhook")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspace_members(workspace: str = Field(..., description="The unique identifier of the workspace, either as a slug (e.g., a short name/alias) or as a UUID surrounded by curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_members", "GET", "/workspaces/{workspace}/members", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_members")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_workspace_member(
    ...
    # Log invocation
    _log_tool_invocation("get_workspace_member", "GET", "/workspaces/{workspace}/members/{member}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_workspace_member")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspace_permissions(
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_permissions", "GET", "/workspaces/{workspace}/permissions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_permissions")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspace_repository_permissions(
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_repository_permissions", "GET", "/workspaces/{workspace}/permissions/repositories", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_repository_permissions")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_repository_user_permissions_workspace(
    ...
    # Log invocation
    _log_tool_invocation("list_repository_user_permissions_workspace", "GET", "/workspaces/{workspace}/permissions/repositories/{repo_slug}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_repository_user_permissions_workspace")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Pipelines
@mcp.tool()
async def get_oidc_openid_configuration(workspace: str = Field(..., description="The workspace identifier, either as a slug (human-readable short name) or as a UUID surrounded by curly braces.")) -> dict[str, Any]:
    """Retrieves the OpenID Connect (OIDC) discovery configuration for a workspace's Pipelines identity provider. This well-known configuration document enables resource servers to validate OIDC tokens issued by Bitbucket Pipelines."""

    # Construct request model with validation
    try:
        _request = _models.GetWorkspacesPipelinesConfigIdentityOidcWellKnownOpenidConfigurationRequest(
            path=_models.GetWorkspacesPipelinesConfigIdentityOidcWellKnownOpenidConfigurationRequestPath(workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_oidc_openid_configuration: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("get_oidc_openid_configuration", "GET", "/workspaces/{workspace}/pipelines-config/identity/oidc/.well-known/openid-configuration", _request_id)

    # Extract parameters for API call
    _http_path = _build_path("/workspaces/{workspace}/pipelines-config/identity/oidc/.well-known/openid-configuration", _request.path.model_dump(by_alias=True)) if _request.path else "/workspaces/{workspace}/pipelines-config/identity/oidc/.well-known/openid-configuration"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_oidc_openid_configuration")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_oidc_openid_configuration",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
# Tags: Pipelines
@mcp.tool()
async def get_oidc_public_keys(workspace: str = Field(..., description="The workspace identifier, either as a slug (human-readable short name) or as a UUID surrounded by curly-braces.")) -> dict[str, Any]:
    """Retrieves the public JWKS (JSON Web Key Set) used to verify OIDC tokens issued by Bitbucket Pipelines for a given workspace, enabling resource servers to validate pipeline identity tokens."""

    # Construct request model with validation
    try:
        _request = _models.GetWorkspacesPipelinesConfigIdentityOidcKeysJsonRequest(
            path=_models.GetWorkspacesPipelinesConfigIdentityOidcKeysJsonRequestPath(workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_oidc_public_keys: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("get_oidc_public_keys", "GET", "/workspaces/{workspace}/pipelines-config/identity/oidc/keys.json", _request_id)

    # Extract parameters for API call
    _http_path = _build_path("/workspaces/{workspace}/pipelines-config/identity/oidc/keys.json", _request.path.model_dump(by_alias=True)) if _request.path else "/workspaces/{workspace}/pipelines-config/identity/oidc/keys.json"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_oidc_public_keys")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_oidc_public_keys",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def list_workspace_runners(workspace: str = Field(..., description="The unique identifier for the workspace, accepted as either a slug (short name) or a UUID enclosed in curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_runners", "GET", "/workspaces/{workspace}/pipelines-config/runners", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_runners")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_workspace_runner(workspace: str = Field(..., description="The workspace identifier, either the workspace slug (human-readable short name) or the workspace UUID enclosed in curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_workspace_runner", "POST", "/workspaces/{workspace}/pipelines-config/runners", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_workspace_runner")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_workspace_runner(
    ...
    # Log invocation
    _log_tool_invocation("get_workspace_runner", "GET", "/workspaces/{workspace}/pipelines-config/runners/{runner_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_workspace_runner")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_workspace_runner(
    ...
    # Log invocation
    _log_tool_invocation("update_workspace_runner", "PUT", "/workspaces/{workspace}/pipelines-config/runners/{runner_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_workspace_runner")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_workspace_runner(
    ...
    # Log invocation
    _log_tool_invocation("delete_workspace_runner", "DELETE", "/workspaces/{workspace}/pipelines-config/runners/{runner_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_workspace_runner")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspace_pipeline_variables(workspace: str = Field(..., description="The unique identifier for the workspace, accepted as either a slug (human-readable short name) or a UUID wrapped in curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_pipeline_variables", "GET", "/workspaces/{workspace}/pipelines-config/variables", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workspace_pipeline_variables")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_pipeline_variable(workspace: str = Field(..., description="The workspace identifier, either as a slug (short name) or as a UUID enclosed in curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_pipeline_variable", "POST", "/workspaces/{workspace}/pipelines-config/variables", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_pipeline_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_workspace_pipeline_variable(
    ...
    # Log invocation
    _log_tool_invocation("get_workspace_pipeline_variable", "GET", "/workspaces/{workspace}/pipelines-config/variables/{variable_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_workspace_pipeline_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def update_workspace_pipeline_variable(
    ...
    # Log invocation
    _log_tool_invocation("update_workspace_pipeline_variable", "PUT", "/workspaces/{workspace}/pipelines-config/variables/{variable_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_workspace_pipeline_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_workspace_pipeline_variable(
    ...
    # Log invocation
    _log_tool_invocation("delete_workspace_pipeline_variable", "DELETE", "/workspaces/{workspace}/pipelines-config/variables/{variable_uuid}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_workspace_pipeline_variable")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_workspace_projects(workspace: str = Field(..., description="The workspace identifier, either as a slug (short name) or as a UUID surrounded by curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workspace_projects", "GET", "/workspaces/{workspace}/projects", _request_id)
...
async def create_project(workspace: str = Field(..., description="The workspace in which to create the project, specified as either the workspace slug (human-readable ID) or the workspace UUID surrounded by curly braces.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_project", "POST", "/workspaces/{workspace}/projects", _request_id)
...
async def get_project(
    ...
    # Log invocation
    _log_tool_invocation("get_project", "GET", "/workspaces/{workspace}/projects/{project_key}", _request_id)
...
async def update_project(
    ...
    # Log invocation
    _log_tool_invocation("update_project", "PUT", "/workspaces/{workspace}/projects/{project_key}", _request_id)
...
async def delete_project(
    ...
    # Log invocation
    _log_tool_invocation("delete_project", "DELETE", "/workspaces/{workspace}/projects/{project_key}", _request_id)
...
async def get_project_branching_model(
    ...
    # Log invocation
    _log_tool_invocation("get_project_branching_model", "GET", "/workspaces/{workspace}/projects/{project_key}/branching-model", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_project_branching_model")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_project_branching_model_settings(
    ...
    # Log invocation
    _log_tool_invocation("get_project_branching_model_settings", "GET", "/workspaces/{workspace}/projects/{project_key}/branching-model/settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_project_branching_model_settings")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Branching model
@mcp.tool()
async def update_project_branching_model_settings(
    project_key: str = Field(..., description="The unique key assigned to the project within the workspace, used to identify which project's branching model settings to update."),
    workspace: str = Field(..., description="The workspace identifier, which can be either the workspace slug or the workspace UUID enclosed in curly braces.")
) -> dict[str, Any]:
    """Updates the branching model configuration for a specific project, including development and production branch settings, branch type prefixes, and default branch deletion behavior. Only the properties explicitly passed in the request body will be modified; all others remain unchanged."""

    # Construct request model with validation
    try:
        _request = _models.PutWorkspacesProjectsBranchingModelSettingsRequest(
            path=_models.PutWorkspacesProjectsBranchingModelSettingsRequestPath(project_key=project_key, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for update_project_branching_model_settings: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("update_project_branching_model_settings", "PUT", "/workspaces/{workspace}/projects/{project_key}/branching-model/settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_project_branching_model_settings")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="update_project_branching_model_settings",
        method="PUT",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def list_project_default_reviewers(
    ...
    # Log invocation
    _log_tool_invocation("list_project_default_reviewers", "GET", "/workspaces/{workspace}/projects/{project_key}/default-reviewers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_project_default_reviewers")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_project_default_reviewer(
    ...
    # Log invocation
    _log_tool_invocation("get_project_default_reviewer", "GET", "/workspaces/{workspace}/projects/{project_key}/default-reviewers/{selected_user}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_project_default_reviewer")
    _http_headers.update(_auth.get("headers", {}))
...
async def add_project_default_reviewer(
    ...
    # Log invocation
    _log_tool_invocation("add_project_default_reviewer", "PUT", "/workspaces/{workspace}/projects/{project_key}/default-reviewers/{selected_user}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("add_project_default_reviewer")
    _http_headers.update(_auth.get("headers", {}))
...
async def remove_project_default_reviewer(
    ...
    # Log invocation
    _log_tool_invocation("remove_project_default_reviewer", "DELETE", "/workspaces/{workspace}/projects/{project_key}/default-reviewers/{selected_user}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("remove_project_default_reviewer")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_project_deploy_keys(
    ...
    # Log invocation
    _log_tool_invocation("list_project_deploy_keys", "GET", "/workspaces/{workspace}/projects/{project_key}/deploy-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_project_deploy_keys")
    _http_headers.update(_auth.get("headers", {}))
...
async def create_project_deploy_key(
    ...
    # Log invocation
    _log_tool_invocation("create_project_deploy_key", "POST", "/workspaces/{workspace}/projects/{project_key}/deploy-keys", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_project_deploy_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def get_project_deploy_key(
    ...
    # Log invocation
    _log_tool_invocation("get_project_deploy_key", "GET", "/workspaces/{workspace}/projects/{project_key}/deploy-keys/{key_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_project_deploy_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_project_deploy_key(
    ...
    # Log invocation
    _log_tool_invocation("delete_project_deploy_key", "DELETE", "/workspaces/{workspace}/projects/{project_key}/deploy-keys/{key_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_project_deploy_key")
    _http_headers.update(_auth.get("headers", {}))
...
async def list_project_group_permissions(
    ...
    # Log invocation
    _log_tool_invocation("list_project_group_permissions", "GET", "/workspaces/{workspace}/projects/{project_key}/permissions-config/groups", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_project_group_permissions")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Projects
@mcp.tool()
async def get_project_group_permission(
    group_slug: str = Field(..., description="The slug identifier of the group whose project permission is being retrieved."),
    project_key: str = Field(..., description="The unique key assigned to the project within the workspace."),
    workspace: str = Field(..., description="The workspace identifier, either as a slug or as a UUID surrounded by curly-braces.")
) -> dict[str, Any]:
    """Retrieves the explicit permission level assigned to a specific group for a given project. Requires admin permission on the project; returns one of: admin, create-repo, write, read, or none."""

    # Construct request model with validation
    try:
        _request = _models.GetWorkspacesByWorkspaceProjectsByProjectKeyPermissionsConfigGroupsByGroupSlugRequest(
            path=_models.GetWorkspacesByWorkspaceProjectsByProjectKeyPermissionsConfigGroupsByGroupSlugRequestPath(group_slug=group_slug, project_key=project_key, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_project_group_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("get_project_group_permission", "GET", "/workspaces/{workspace}/projects/{project_key}/permissions-config/groups/{group_slug}", _request_id)

    # Extract parameters for API call
    _http_path = _build_path("/workspaces/{workspace}/projects/{project_key}/permissions-config/groups/{group_slug}", _request.path.model_dump(by_alias=True)) if _request.path else "/workspaces/{workspace}/projects/{project_key}/permissions-config/groups/{group_slug}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_project_group_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_project_group_permission",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
# Tags: Projects
@mcp.tool()
async def set_project_group_permission(
    group_slug: str = Field(..., description="The slug identifier of the group whose project permission is being set or updated."),
    project_key: str = Field(..., description="The unique key assigned to the project within the workspace."),
    workspace: str = Field(..., description="The workspace identifier, either as a slug or as a UUID surrounded by curly-braces.")
) -> dict[str, Any]:
    """Grants or updates an explicit permission for a group on a specific project. Requires admin permission on the project; JWT and OAuth authentication are not supported for this endpoint."""

    # Construct request model with validation
    try:
        _request = _models.PutWorkspacesProjectsPermissionsConfigGroupsRequest(
            path=_models.PutWorkspacesProjectsPermissionsConfigGroupsRequestPath(group_slug=group_slug, project_key=project_key, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for set_project_group_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("set_project_group_permission", "PUT", "/workspaces/{workspace}/projects/{project_key}/permissions-config/groups/{group_slug}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("set_project_group_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="set_project_group_permission",
        method="PUT",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
# Tags: Projects
@mcp.tool()
async def delete_project_group_permission(
    group_slug: str = Field(..., description="The slug identifier of the group whose project permission should be deleted."),
    project_key: str = Field(..., description="The unique key assigned to the project whose group permission is being removed."),
    workspace: str = Field(..., description="The workspace identifier, either as a slug or as a UUID surrounded by curly-braces.")
) -> dict[str, Any]:
    """Removes the explicit group permission assigned to a specific project, if one exists. Only users with admin permission for the project may perform this action."""

    # Construct request model with validation
    try:
        _request = _models.DeleteWorkspacesProjectsPermissionsConfigGroupsRequest(
            path=_models.DeleteWorkspacesProjectsPermissionsConfigGroupsRequestPath(group_slug=group_slug, project_key=project_key, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for delete_project_group_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("delete_project_group_permission", "DELETE", "/workspaces/{workspace}/projects/{project_key}/permissions-config/groups/{group_slug}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_project_group_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="delete_project_group_permission",
        method="DELETE",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def list_project_user_permissions(
    ...
    # Log invocation
    _log_tool_invocation("list_project_user_permissions", "GET", "/workspaces/{workspace}/projects/{project_key}/permissions-config/users", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_project_user_permissions")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Projects
@mcp.tool()
async def get_project_user_permission(
    project_key: str = Field(..., description="The unique key assigned to the project, used to identify the project within the workspace."),
    selected_user_id: str = Field(..., description="The identifier for the user whose permission is being retrieved; accepts a username, a UUID surrounded by curly-braces, or an Atlassian ID."),
    workspace: str = Field(..., description="The identifier for the workspace containing the project; accepts either the workspace slug or a UUID surrounded by curly-braces.")
) -> dict[str, Any]:
    """Retrieves the explicit permission level assigned to a specific user for a given project within a workspace. Requires admin permission on the project to access."""

    # Construct request model with validation
    try:
        _request = _models.GetWorkspacesByWorkspaceProjectsByProjectKeyPermissionsConfigUsersBySelectedUserIdRequest(
            path=_models.GetWorkspacesByWorkspaceProjectsByProjectKeyPermissionsConfigUsersBySelectedUserIdRequestPath(project_key=project_key, selected_user_id=selected_user_id, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_project_user_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("get_project_user_permission", "GET", "/workspaces/{workspace}/projects/{project_key}/permissions-config/users/{selected_user_id}", _request_id)

    # Extract parameters for API call
    _http_path = _build_path("/workspaces/{workspace}/projects/{project_key}/permissions-config/users/{selected_user_id}", _request.path.model_dump(by_alias=True)) if _request.path else "/workspaces/{workspace}/projects/{project_key}/permissions-config/users/{selected_user_id}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_project_user_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_project_user_permission",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
# Tags: Projects
@mcp.tool()
async def update_project_user_permission(
    project_key: str = Field(..., description="The unique key assigned to the project whose user permission is being updated."),
    selected_user_id: str = Field(..., description="The identifier of the user whose project permission is being updated; accepts a username, a UUID surrounded by curly-braces, or an Atlassian ID."),
    workspace: str = Field(..., description="The workspace containing the project; accepts either the workspace slug or a UUID surrounded by curly-braces.")
) -> dict[str, Any]:
    """Updates the explicit permission level for a specific user on a given project within a workspace. Only project admins may perform this action; the target user must be a workspace member and cannot be the workspace owner."""

    # Construct request model with validation
    try:
        _request = _models.PutWorkspacesProjectsPermissionsConfigUsersRequest(
            path=_models.PutWorkspacesProjectsPermissionsConfigUsersRequestPath(project_key=project_key, selected_user_id=selected_user_id, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for update_project_user_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("update_project_user_permission", "PUT", "/workspaces/{workspace}/projects/{project_key}/permissions-config/users/{selected_user_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_project_user_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="update_project_user_permission",
        method="PUT",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
# Tags: Projects
@mcp.tool()
async def delete_project_user_permission(
    project_key: str = Field(..., description="The unique key assigned to the project whose user permission is being deleted."),
    selected_user_id: str = Field(..., description="The identifier of the user whose project permission will be removed; accepts a username, a UUID surrounded by curly-braces, or an Atlassian account ID."),
    workspace: str = Field(..., description="The workspace containing the project, specified as either the workspace slug or a UUID surrounded by curly-braces.")
) -> dict[str, Any]:
    """Removes an explicit user-level permission from a project, if one exists. Requires admin permission on the project; JWT and OAuth authentication are not supported for this operation."""

    # Construct request model with validation
    try:
        _request = _models.DeleteWorkspacesProjectsPermissionsConfigUsersRequest(
            path=_models.DeleteWorkspacesProjectsPermissionsConfigUsersRequestPath(project_key=project_key, selected_user_id=selected_user_id, workspace=workspace)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for delete_project_user_permission: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("delete_project_user_permission", "DELETE", "/workspaces/{workspace}/projects/{project_key}/permissions-config/users/{selected_user_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_project_user_permission")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="delete_project_user_permission",
        method="DELETE",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def list_user_pull_requests_in_workspace(
    ...
    # Log invocation
    _log_tool_invocation("list_user_pull_requests_in_workspace", "GET", "/workspaces/{workspace}/pullrequests/{selected_user}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_user_pull_requests_in_workspace")
    _http_headers.update(_auth.get("headers", {}))
...
async def search_workspace_code(
    ...
    try:
        _request = _models.GetWorkspacesSearchCodeRequest(
            ...
            query=_models.GetWorkspacesSearchCodeRequestQuery(search_query=search_query, page=page, pagelen=pagelen)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("search_workspace_code", "GET", "/workspaces/{workspace}/search/code", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("search_workspace_code")
    _http_headers.update(_auth.get("headers", {}))
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python bitbucket_api_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
