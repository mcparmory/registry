...
uvx mcparmory-globalping
```

### With pip

```bash
pip install mcparmory-globalping
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
OAUTH2_SCOPES=YOUR_OAUTH2_SCOPES \
BEARER_TOKEN=YOUR_BEARER_TOKEN \
...
Example (if server is at `/home/user/mcp-servers/globalping`):
```json
{
  "mcpServers": {
    "globalping": {
      "command": "python",
      "args": ["/home/user/mcp-servers/globalping/server.py"]
    }
  }
}
```
...
```bash
docker build -t globalping .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env globalping
```
