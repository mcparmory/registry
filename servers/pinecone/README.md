...
uvx mcparmory-pinecone
```

### With pip

```bash
pip install mcparmory-pinecone
API_KEY=YOUR_API_KEY \
mcparmory-pinecone
```
...
Example (if server is at `/home/user/mcp-servers/pinecone`):
```json
{
  "mcpServers": {
    "pinecone": {
      "command": "python",
      "args": ["/home/user/mcp-servers/pinecone/server.py"]
    }
  }
}
```
