...
uvx mcparmory-brightdata
```

### With pip

```bash
pip install mcparmory-brightdata
BEARER_TOKEN=YOUR_BEARER_TOKEN \
mcparmory-brightdata
```
...
Example (if server is at `/home/user/mcp-servers/brightdata`):
```json
{
  "mcpServers": {
    "brightdata": {
      "command": "python",
      "args": ["/home/user/mcp-servers/brightdata/server.py"]
    }
  }
}
```
