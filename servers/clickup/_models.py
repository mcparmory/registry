"""
Clickup Api V2 Reference MCP Server - Pydantic Models
...
class GetAccessTokenRequestBody(StrictModel):
    client_id: str = Field(default=..., description="The unique client identifier issued to your OAuth application upon registration.")
    client_secret: str = Field(default=..., description="The confidential client secret issued to your OAuth application, used alongside the client ID to authenticate your app.")
    code: str = Field(default=..., description="The short-lived authorization code received in the redirect URL after the user grants access during the OAuth flow.")
class GetAccessTokenRequest(StrictModel):
...
class GetAuthorizedUserRequest(StrictModel):
...
class GetAuthorizedTeamsRequest(StrictModel):
...
class CreateWebhookRequestPath(StrictModel):
    team_id: float = Field(default=..., description="The unique identifier of the workspace in which to create the webhook.", json_schema_extra={'format': 'double', 'contentEncoding': 'double'}, examples=[123])
class CreateWebhookRequestBody(StrictModel):
    ...
    endpoint: str = Field(default=..., description="The publicly accessible URL that will receive HTTP POST requests when subscribed events occur.")
    events: list[str] = Field(default=..., description="List of event types to subscribe to; each item is an event name string. Use the wildcard value `*` to subscribe to all available events. Refer to the webhooks documentation for the full list of supported event names.")
    space_id: int | None = Field(default=None, description="Optional space ID to scope the webhook so it only fires for events within that specific space.")
    folder_id: int | None = Field(default=None, description="Optional folder ID to scope the webhook so it only fires for events within that specific folder.")
    list_id: int | None = Field(default=None, description="Optional list ID to scope the webhook so it only fires for events within that specific list.")
    task_id: str | None = Field(default=None, description="Optional task ID to scope the webhook so it only fires for events related to that specific task.")
class CreateWebhookRequest(StrictModel):
    ...
    path: CreateWebhookRequestPath
    body: CreateWebhookRequestBody

# Operation: update_webhook
class UpdateWebhookRequestPath(StrictModel):
    webhook_id: str = Field(default=..., description="The unique UUID identifier of the webhook to update.", examples=['4b67ac88'])
...
If you don't specify any events, all events will be sent to the webhook."""
    endpoint: str = Field(default=..., description="The destination URL where webhook event payloads will be sent when triggered.")
    events: str = Field(default=..., description="The event type or types that this webhook should monitor and trigger on.")
    status: str = Field(default=..., description="The current operational status of the webhook, such as active or inactive.")
