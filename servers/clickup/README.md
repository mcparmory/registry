# ClickUp MCP Server
<!-- mcp-name: com.mcparmory/clickup -->
...
uvx mcparmory-clickup
```

### With pip

```bash
pip install mcparmory-clickup
OAUTH2_CLIENT_ID=YOUR_OAUTH2_CLIENT_ID \
OAUTH2_CLIENT_SECRET=YOUR_OAUTH2_CLIENT_SECRET \
API_KEY=YOUR_API_KEY \
mcparmory-clickup
...
Example (if server is at `/home/user/mcp-servers/clickup`):
```json
{
  "mcpServers": {
    "clickup": {
      "command": "python",
      "args": ["/home/user/mcp-servers/clickup/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens \
  ...
  ghcr.io/mcparmory/clickup:latest
```
...
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env clickup
```
