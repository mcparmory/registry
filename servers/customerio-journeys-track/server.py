...
async def get_account_region() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_account_region", "GET", "/api/v1/accounts/region", _request_id)
    ...
    _auth = _get_auth_for_operation("get_account_region")
...
async def upsert_customer(
    identifier: str = Field(..., description="The unique identifier for the customer, such as an id, email address, or cio_id (prefixed with 'cio_'). The identifier type depends on your workspace configuration."),
    update: bool | None = Field(None, alias="_update", description="Set to true to prevent duplicate profile creation when making rapid successive requests. If enabled and the identifier is not found, the request will fail with a Failed Attribute Change event rather than creating a new profile."),
    action: Literal["add_relationships", "delete_relationships"] | None = Field(None, description="Specifies whether to add new relationships to the customer or remove existing relationships."),
    relationships: list[_models.IdentifyBodyCioRelationshipsRelationshipsItem] | None = Field(None, description="An array of relationship objects to add to or remove from the customer, determined by the action parameter."),
    unsubscribed: bool | None = Field(None, description="Set to true to unsubscribe the customer from all messages. When false or omitted, the customer's subscription status is determined by their subscription preferences. Accepts true, TRUE, 1, or '1' for unsubscribed; all other values are treated as subscribed."),
    topics: dict[str, bool] | None = Field(None, description="An object containing topic subscription states for active topics in your workspace, with keys formatted as 'topic_<id>'.")
) -> dict[str, Any]:
    ...
    try:
        _request = _models.IdentifyRequest(
            ...
            body=_models.IdentifyRequestBody(update=update, unsubscribed=unsubscribed,
    ...
    # Log invocation
    _log_tool_invocation("upsert_customer", "PUT", "/api/v1/customers/{identifier}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("upsert_customer")
    _http_headers.update(_auth.get("headers", {}))
...
async def delete_customer(identifier: str = Field(..., description="The unique identifier for the customer to delete. This can be a customer ID, email address, or cio_id (prefixed with 'cio_') depending on your workspace configuration.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_customer", "DELETE", "/api/v1/customers/{identifier}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_customer")
    _http_headers.update(_auth.get("headers", {}))
...
async def register_device(
    identifier: str = Field(..., description="The unique identifier for the customer. This can be their user ID, email address, or CIO ID (prefixed with `cio_`) depending on your workspace configuration."),
    device: _models.DeviceObject | None = Field(None, description="An object containing the device properties to register or update. Properties are automatically collected by SDKs unless disabled, and can be referenced in segments and Liquid templates.")
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("register_device", "PUT", "/api/v1/customers/{identifier}/devices", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("register_device")
    _http_headers.update(_auth.get("headers", {}))
...
async def remove_device(
    identifier: str = Field(..., description="The unique identifier for the customer. This can be their user ID, email address, or Customer.io ID (prefixed with `cio_`)."),
    ...
    # Log invocation
    _log_tool_invocation("remove_device", "DELETE", "/api/v1/customers/{identifier}/devices/{device_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("remove_device")
    _http_headers.update(_auth.get("headers", {}))
...
async def suppress_customer(identifier: str = Field(..., description="The unique identifier for the customer to suppress. This can be an email address, customer ID, or CIO ID (prefixed with `cio_`), depending on your workspace configuration. The suppressed identifier cannot be re-added until explicitly unsuppressed.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("suppress_customer", "POST", "/api/v1/customers/{identifier}/suppress", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("suppress_customer")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Track Customers
@mcp.tool()
async def unsuppress_customer(identifier: str = Field(..., description="The unique identifier for the customer, which can be their ID, email address, or cio_id (prefixed with 'cio_'). The accepted identifier type depends on your workspace configuration.")) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("unsuppress_customer", "POST", "/api/v1/customers/{identifier}/unsuppress", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("unsuppress_customer")
    _http_headers.update(_auth.get("headers", {}))
...
async def mark_delivery_unsubscribed(
    ...
    unsubscribe: bool | None = Field(None, description="Set to true to mark the person as unsubscribed and attribute the unsubscribe action to this delivery.", examples=[True])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("mark_delivery_unsubscribed", "POST", "/unsubscribe/{delivery_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("mark_delivery_unsubscribed")
    _http_headers.update(_auth.get("headers", {}))
...
async def track_customer_event(
    identifier: str = Field(..., description="The unique identifier for the customer. Can be the customer's ID, email address, or CIO ID depending on workspace configuration."),
    body: _models.TrackBodyV0 | _models.TrackBodyV1 | _models.TrackBodyV2 | None = Field(None, description="Event payload containing the event type (page, screen, or event), event name, and optional properties. Reserved properties (from_address, recipient, reply_to) will override campaign settings if present.")
) -> dict[str, Any]:
    ...
    try:
        _request = _models.TrackRequest(
            ...
            body=_models.TrackRequestBody(body=body)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("track_customer_event", "POST", "/api/v1/customers/{identifier}/events", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("track_customer_event")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Track Events
@mcp.tool()
async def log_anonymous_event(body: _models.TrackAnonymousBodyV0 | _models.TrackAnonymousBodyV1 | _models.TrackAnonymousBodyV2 | None = Field(None, description="Event data for an anonymous user. Include an `anonymous_id` to enable later association with an identified person. Supports three event types: `page` (website views), `screen` (mobile app views), or `event` (custom events). Event names should not have leading or trailing spaces.")) -> dict[str, Any]:
    ...
    try:
        _request = _models.TrackAnonymousRequest(
            body=_models.TrackAnonymousRequestBody(body=body)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("log_anonymous_event", "POST", "/api/v1/events", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("log_anonymous_event")
    _http_headers.update(_auth.get("headers", {}))
...
async def submit_form(
    form_id: str = Field(..., description="Unique identifier for the form. Use a value that is meaningful to your system and traceable to your backend. If Customer.io does not recognize this identifier, a new form connection will be created automatically."),
    data: _models.SubmitFormBodyDataV0 | _models.SubmitFormBodyDataV1 | None = Field(None, description="Form submission data containing respondent information and form field responses. Must include at least one identifier (id, email, or a field mapped to an identifier) to identify or create the respondent. All form field values must be formatted as strings. Reserved keys (form_id, form_name, form_type, form_url, form_url_param) are ignored if included.")
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("submit_form", "POST", "/api/v1/forms/{form_id}/submit", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("submit_form")
    _http_headers.update(_auth.get("headers", {}))
...
async def merge_customers(
    ...
    # Log invocation
    _log_tool_invocation("merge_customers", "POST", "/api/v1/merge_customers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("merge_customers")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: Track Events
@mcp.tool()
async def report_metrics(body: _models.MetricsBodyV0 | _models.MetricsBodyV1 | _models.MetricsBodyV2 | _models.MetricsBodyV3 | _models.MetricsBodyV4 | _models.MetricsBodyV5 | None = Field(None, description="Metric payload containing the delivery_id (from CIO-Delivery-ID header) and associated metric data to report for tracking message performance and user interactions.")) -> dict[str, Any]:
    ...
    try:
        _request = _models.MetricsRequest(
            body=_models.MetricsRequestBody(body=body)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for report_metrics: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("report_metrics", "POST", "/api/v1/metrics", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("report_metrics")
    _http_headers.update(_auth.get("headers", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="report_metrics",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def add_customers_to_segment(
    segment_id: int = Field(..., description="The unique identifier of the manual segment. Find this ID in the dashboard under Segments > [Your Segment] > Usage, or retrieve it using the App API.", json_schema_extra={'format': 'int32'}),
    ids: list[str] | None = Field(None, description="Array of customer identifiers to add to the segment. The identifier type (ID, email, or CIO ID) is determined by the id_type query parameter. All identifiers in the array must be of the same type; mismatched entries are ignored.", min_length=1, max_length=1000)
) -> dict[str, Any]:
    ...
    try:
        _request = _models.AddToSegmentRequest(
            path=_models.AddToSegmentRequestPath(segment_id=segment_id),
    ...
    # Log invocation
    _log_tool_invocation("add_customers_to_segment", "POST", "/api/v1/segments/{segment_id}/add_customers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("add_customers_to_segment")
    _http_headers.update(_auth.get("headers", {}))
...
async def remove_customers_from_segment(
    segment_id: int = Field(..., description="The unique identifier of the segment. You can find this ID in the Segments dashboard under Usage, or retrieve it using the Segments API.", json_schema_extra={'format': 'int32'}),
    ids: list[str] | None = Field(None, description="An array of customer IDs to remove from the segment. Each ID must correspond to a customer with an `id` attribute in your workspace.", min_length=1, max_length=1000)
) -> dict[str, Any]:
    ...
    try:
        _request = _models.RemoveFromSegmentRequest(
            path=_models.RemoveFromSegmentRequestPath(segment_id=segment_id),
    ...
    # Log invocation
    _log_tool_invocation("remove_customers_from_segment", "POST", "/api/v1/segments/{segment_id}/remove_customers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("remove_customers_from_segment")
    _http_headers.update(_auth.get("headers", {}))
...
# Tags: track_v2
@mcp.tool()
async def manage_entity(body: _models.PersonOperations | _models.ObjectOperations | _models.DeliveryOperations | None = Field(None, description="Request payload containing entity data and operation details. Maximum payload size is 32KB.")) -> dict[str, Any]:
    ...
    try:
        _request = _models.EntityRequest(
            body=_models.EntityRequestBody(body=body)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("manage_entity", "POST", "/api/v2/entity", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("manage_entity")
    _http_headers.update(_auth.get("headers", {}))
...
async def batch_entities(batch: str | None = Field(None, description="Array of entity operations to process. Each object represents a single person or object modification. The batch payload must not exceed 500kb total, and each individual entity operation must not exceed 32kb. Pass as a JSON-encoded string with a \"type\" discriminator field, e.g. '{\"type\": \"value\", ...}'.")) -> dict[str, Any]:
    ...
    _batch: Any = json.loads(batch) if isinstance(batch, str) else batch

    # Construct request model with validation
    try:
        _request = _models.BatchRequest(
            body=_models.BatchRequestBody(batch=_batch)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("batch_entities", "POST", "/api/v2/batch", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("batch_entities")
    _http_headers.update(_auth.get("headers", {}))
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python journeys_track_api_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Journeys Track Api MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting Journeys Track Api MCP Server")
