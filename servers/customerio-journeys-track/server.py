...
Generated: 2026-03-29 11:29:24 UTC
...
async def upsert_customer(
    ...
    email: str | None = Field(None, description="The email address of the customer. Case-insensitive for workspaces using email as an identifier.", json_schema_extra={'format': 'email'}),
...
async def track_customer_event(
    ...
    id_: str | None = Field(None, alias="id", description="A unique identifier for deduplicating events. If an event with this ID was previously received, it will not be processed again.", json_schema_extra={'format': 'ulid'}),
    ...
    timestamp: int | None = Field(None, description="The Unix timestamp indicating when the event occurred. If omitted, the server timestamp at receipt is used. Only events from the past 72 hours can trigger campaigns.", json_schema_extra={'format': 'unix timestamp'}),
    ...
    try:
        _request = _models.TrackRequest(
            ...
            body=_models.TrackRequestBody(name=name, id_=id_, type_=type_, timestamp=timestamp, data=data, anonymous_id=anonymous_id)
        )
    except pydantic.ValidationError as _validation_err:
...
async def log_anonymous_event(
    ...
    id_: str | None = Field(None, alias="id", description="A unique identifier for deduplicating this event. If an event with the same ID was previously received, it will not be processed again.", json_schema_extra={'format': 'ulid'}),
    ...
    timestamp: int | None = Field(None, description="The Unix timestamp indicating when the event occurred. If not provided, the server timestamp at receipt time is used."),
    ...
    try:
        _request = _models.TrackAnonymousRequest(
            body=_models.TrackAnonymousRequestBody(anonymous_id=anonymous_id, name=name, id_=id_, type_=type_, timestamp=timestamp, data=data)
        )
    except pydantic.ValidationError as _validation_err:
...
async def report_metric(
    delivery_id: str = Field(..., description="The CIO-Delivery-ID header value from the notification you want to associate this metric with. This ID links the reported event back to the original message delivery.", examples=['RPILAgUBcRhIBqSfeiIwdIYJKxTY']),
    ...
    timestamp: int | None = Field(None, description="The Unix timestamp indicating when the event occurred. If omitted, the current time is used.", examples=[1613063089], json_schema_extra={'format': 'unix timestamp'}),
    ...
    try:
        _request = _models.MetricsRequest(
            body=_models.MetricsRequestBody(delivery_id=delivery_id, timestamp=timestamp, metric=metric, recipient=recipient, reason=reason, href=href)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("report_metric", "POST", "/api/v1/metrics", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("report_metric")
    _http_headers.update(_auth.get("headers", {}))
...
async def add_customers_to_segment(
    segment_id: int = Field(..., description="The unique identifier of the manual segment. Find this ID in the segment's dashboard page under Usage, or retrieve it using the segments API.", json_schema_extra={'format': 'int32'}),
    ...
    id_type: Literal["id", "email", "cio_id"] | None = Field('id', description="The identifier type for all values in the ids array. All customer identifiers must be of the same type. Defaults to customer ID if not specified.")
) -> dict[str, Any]:
...
async def remove_customers_from_segment(
    segment_id: int = Field(..., description="The unique identifier of the segment from which to remove customers. You can find this ID in the Segments dashboard under Usage, or retrieve it via the Segments API.", json_schema_extra={'format': 'int32'}),
    ...
    id_type: Literal["id", "email", "cio_id"] | None = Field('id', description="The identifier type for the customers being removed. All values in the ids array must match this type. Defaults to id if not specified.")
) -> dict[str, Any]:
...
async def manage_entity(
    ...
    timestamp: int | None = Field(None, description="Unix timestamp indicating when the attribute update occurred. Use this to control the order of updates when multiple requests are sent in rapid succession.", examples=[1772013598]),
    ...
    id_: str | None = Field(None, alias="id", description="A unique ULID used to deduplicate events and prevent duplicate processing.", json_schema_extra={'format': 'ULID'}),
