...
uvx mcparmory-customerio-journeys-track
```

### With pip

```bash
pip install mcparmory-customerio-journeys-track
BASIC_AUTH_USERNAME=YOUR_BASIC_AUTH_USERNAME \
BASIC_AUTH_PASSWORD=YOUR_BASIC_AUTH_PASSWORD \
...
Example (if server is at `/home/user/mcp-servers/customerio-journeys-track`):
```json
{
  "mcpServers": {
    "customerio-journeys-track": {
      "command": "python",
      "args": ["/home/user/mcp-servers/customerio-journeys-track/server.py"]
    }
  }
}
```
...
```bash
docker build -t customerio-journeys-track .
