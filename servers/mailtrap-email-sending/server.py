#!/usr/bin/env python3
"""
Mailtrap Email Sending MCP Server
...
SERVER_NAME = "Mailtrap Email Sending"
...
mcp = FastMCP("Mailtrap Email Sending", middleware=[_JsonCoercionMiddleware()])
...
async def list_sending_domains(account_id: int = Field(..., description="The unique identifier for the account. Must be a positive integer.")) -> dict[str, Any]:
    ...
    try:
        _request = _models.GetSendingDomainsRequest(
            path=_models.GetSendingDomainsRequestPath(account_id=account_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_sending_domains: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/sending_domains", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/sending_domains"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_sending_domains")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_sending_domains", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_sending_domains",
        method="GET",
        path=_http_path,
...
async def create_sending_domain(
    ...
    # Construct request model with validation
    try:
        _request = _models.CreateSendingDomainRequest(
            path=_models.CreateSendingDomainRequestPath(account_id=account_id),
            body=_models.CreateSendingDomainRequestBody(sending_domain=_models.CreateSendingDomainRequestBodySendingDomain(domain_name=domain_name))
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for create_sending_domain: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("create_sending_domain")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("create_sending_domain", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="create_sending_domain",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def get_sending_domain(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetSendingDomainRequest(
            path=_models.GetSendingDomainRequestPath(account_id=account_id, sending_domain_id=sending_domain_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_sending_domain: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/sending_domains/{sending_domain_id}", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/sending_domains/{sending_domain_id}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_sending_domain")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_sending_domain", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_sending_domain",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def delete_sending_domain(
    ...
    # Construct request model with validation
    try:
        _request = _models.DeleteSendingDomainRequest(
            path=_models.DeleteSendingDomainRequestPath(account_id=account_id, sending_domain_id=sending_domain_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for delete_sending_domain: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("delete_sending_domain")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("delete_sending_domain", "DELETE", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="delete_sending_domain",
        method="DELETE",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def send_sending_domain_setup_instructions(
    ...
    # Construct request model with validation
    try:
        _request = _models.SendSendingDomainSetupInstructionsRequest(
            path=_models.SendSendingDomainSetupInstructionsRequestPath(account_id=account_id, sending_domain_id=sending_domain_id),
            body=_models.SendSendingDomainSetupInstructionsRequestBody(email=email)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for send_sending_domain_setup_instructions: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/sending_domains/{sending_domain_id}/send_setup_instructions", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/sending_domains/{sending_domain_id}/send_setup_instructions"
    _http_body = _request.body.model_dump(by_alias=True, exclude_none=True) if _request.body else None
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("send_sending_domain_setup_instructions")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("send_sending_domain_setup_instructions", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="send_sending_domain_setup_instructions",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def list_suppressions(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetSuppressionsRequest(
            path=_models.GetSuppressionsRequestPath(account_id=account_id),
            query=_models.GetSuppressionsRequestQuery(email=email, start_time=start_time)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_suppressions: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/suppressions", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/suppressions"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_suppressions")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_suppressions", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_suppressions",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def delete_suppression(
    ...
    # Construct request model with validation
    try:
        _request = _models.DeleteSuppressionRequest(
            path=_models.DeleteSuppressionRequestPath(account_id=account_id, suppression_id=suppression_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for delete_suppression: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/suppressions/{suppression_id}", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/suppressions/{suppression_id}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("delete_suppression")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("delete_suppression", "DELETE", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="delete_suppression",
        method="DELETE",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def get_account_sending_stats(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetAccountSendingStatsRequest(
            path=_models.GetAccountSendingStatsRequestPath(account_id=account_id),
            query=_models.GetAccountSendingStatsRequestQuery(start_date=start_date, end_date=end_date, sending_domain_ids=sending_domain_ids, sending_streams=sending_streams, categories=categories, email_service_providers=email_service_providers)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_account_sending_stats: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/stats", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/stats"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_account_sending_stats")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_account_sending_stats", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_account_sending_stats",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_account_sending_stats_by_domains(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetAccountSendingStatsByDomainsRequest(
            path=_models.GetAccountSendingStatsByDomainsRequestPath(account_id=account_id),
            query=_models.GetAccountSendingStatsByDomainsRequestQuery(start_date=start_date, end_date=end_date, sending_domain_ids=sending_domain_ids, sending_streams=sending_streams, categories=categories, email_service_providers=email_service_providers)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_account_sending_stats_by_domains: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/stats/domains", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/stats/domains"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_account_sending_stats_by_domains")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_account_sending_stats_by_domains", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_account_sending_stats_by_domains",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_sending_stats_by_categories(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetAccountSendingStatsByCategoriesRequest(
            path=_models.GetAccountSendingStatsByCategoriesRequestPath(account_id=account_id),
            query=_models.GetAccountSendingStatsByCategoriesRequestQuery(start_date=start_date, end_date=end_date, sending_domain_ids=sending_domain_ids, sending_streams=sending_streams, categories=categories, email_service_providers=email_service_providers)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_sending_stats_by_categories: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/stats/categories", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/stats/categories"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_sending_stats_by_categories")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_sending_stats_by_categories", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_sending_stats_by_categories",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_account_sending_stats_by_email_service_providers(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetAccountSendingStatsByEmailServiceProvidersRequest(
            path=_models.GetAccountSendingStatsByEmailServiceProvidersRequestPath(account_id=account_id),
            query=_models.GetAccountSendingStatsByEmailServiceProvidersRequestQuery(start_date=start_date, end_date=end_date, sending_domain_ids=sending_domain_ids, sending_streams=sending_streams, categories=categories, email_service_providers=email_service_providers)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_account_sending_stats_by_email_service_providers: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/stats/email_service_providers", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/stats/email_service_providers"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_account_sending_stats_by_email_service_providers")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_account_sending_stats_by_email_service_providers", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_account_sending_stats_by_email_service_providers",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_account_sending_stats_by_date(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetAccountSendingStatsByDateRequest(
            path=_models.GetAccountSendingStatsByDateRequestPath(account_id=account_id),
            query=_models.GetAccountSendingStatsByDateRequestQuery(start_date=start_date, end_date=end_date, sending_domain_ids=sending_domain_ids, sending_streams=sending_streams, categories=categories, email_service_providers=email_service_providers)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_account_sending_stats_by_date: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/stats/date", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/stats/date"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_account_sending_stats_by_date")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_account_sending_stats_by_date", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_account_sending_stats_by_date",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_email_logs(
    ...
    # Construct request model with validation
    try:
        _request = _models.ListEmailLogsRequest(
            path=_models.ListEmailLogsRequestPath(account_id=account_id),
            query=_models.ListEmailLogsRequestQuery(search_after=search_after, filters=filters)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_email_logs: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/email_logs", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/email_logs"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_query = _serialize_query(_http_query, {
        "filters": ("deepObject", True),
    })
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_email_logs")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_email_logs", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_email_logs",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_email_log_message(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetEmailLogMessageRequest(
            path=_models.GetEmailLogMessageRequestPath(account_id=account_id, sending_message_id=sending_message_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_email_log_message: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/accounts/{account_id}/email_logs/{sending_message_id}", _request.path.model_dump(by_alias=True)) if _request.path else "/api/accounts/{account_id}/email_logs/{sending_message_id}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_email_log_message")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_email_log_message", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_email_log_message",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python mailtrap_email_sending_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Mailtrap Email Sending MCP Server")
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting Mailtrap Email Sending MCP Server")
