...
uvx mcparmory-mailtrap-email-sending
```

### With pip

```bash
pip install mcparmory-mailtrap-email-sending
API_KEY=YOUR_API_KEY \
BEARER_TOKEN=YOUR_BEARER_TOKEN \
...
```json
{
  "mcpServers": {
    "mailtrap-email-sending": {
      "command": "uvx",
      "args": ["mcparmory-mailtrap-email-sending"],
      "env": {
        "API_KEY": "YOUR_API_KEY",
        "BEARER_TOKEN": "YOUR_BEARER_TOKEN"
      }
...
Example (if server is at `/home/user/mcp-servers/mailtrap-email-sending`):
```json
{
  "mcpServers": {
    "mailtrap-email-sending": {
      "command": "python",
      "args": ["/home/user/mcp-servers/mailtrap-email-sending/server.py"]
    }
  }
}
```
...
```bash
docker build -t mailtrap-email-sending .
docker run -p 8000:8000 --env-file .env mailtrap-email-sending
```
