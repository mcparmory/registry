...
Example (if server is at `/home/user/mcp-servers/close`):
```json
{
  "mcpServers": {
    "close": {
      "command": "python",
      "args": ["/home/user/mcp-servers/close/server.py"]
    }
  }
}
```
...
```bash
docker build -t close .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env close
```
