...
Example (if server is at `/home/user/mcp-servers/ramp`):
```json
{
  "mcpServers": {
    "ramp": {
      "command": "python",
      "args": ["/home/user/mcp-servers/ramp/server.py"]
    }
  }
}
```
...
docker build -t ramp .
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -v ./tokens:/app/tokens --env-file .env ramp
```
