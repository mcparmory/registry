...
ORG_OAUTH2_CLIENT_ID=YOUR_ORG_OAUTH2_CLIENT_ID \
ORG_OAUTH2_CLIENT_SECRET=YOUR_ORG_OAUTH2_CLIENT_SECRET \
ORG_OAUTH2_SCOPES=YOUR_ORG_OAUTH2_SCOPES \
...
- `ORG_OAUTH2_CLIENT_ID` — OAuth2 client ID
...
Example (if server is at `/home/user/mcp-servers/figma`):
```json
{
  "mcpServers": {
    "figma": {
      "command": "python",
      "args": ["/home/user/mcp-servers/figma/server.py"]
    }
  }
}
```
...
mkdir -p tokens
docker run -p 8000:8000 -p 9400:9400 -p 9401:9401 -v ./tokens:/app/tokens --env-file .env figma
```
