...
<!-- mcp-name: com.mcparmory/alchemy-nft -->
...
uvx mcparmory-alchemy-nft
```

### With pip

```bash
pip install mcparmory-alchemy-nft
API_KEY=YOUR_API_KEY \
...
Example (if server is at `/home/user/mcp-servers/alchemy-nft`):
```json
{
  "mcpServers": {
    "alchemy-nft": {
      "command": "python",
      "args": ["/home/user/mcp-servers/alchemy-nft/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 \
  ...
  ghcr.io/mcparmory/alchemy-nft:latest
```
...
```bash
docker build -t alchemy-nft .
docker run -p 8000:8000 --env-file .env alchemy-nft
```
