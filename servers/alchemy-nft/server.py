...
async def list_nfts_by_owner(
    ...
    contract_addresses: list[str] | None = Field(None, alias="contractAddresses", description="Optional list of NFT contract addresses to filter results. Specify up to 45 contract addresses to narrow the response to specific collections."),
    with_metadata: bool | None = Field(None, alias="withMetadata", description="Include full NFT metadata in the response. Set to false to reduce payload size and improve response speed if metadata is not needed."),
    ...
    try:
        _request = _models.GetNfTsForOwnerV3Request(
            query=_models.GetNfTsForOwnerV3RequestQuery(owner=owner, contract_addresses=contract_addresses, with_metadata=with_metadata, order_by=order_by, spam_confidence_level=spam_confidence_level, page_size=page_size)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nfts_by_owner: {_validation_err}")
...
async def list_nfts_for_contract(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetNfTsForContractV3Request(
            query=_models.GetNfTsForContractV3RequestQuery(contract_address=contract_address, with_metadata=with_metadata, start_token=start_token, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nfts_for_contract: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_nfts_for_contract",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_nfts_for_collection(
    ...
    start_token: str | None = Field(None, alias="startToken", description="Token ID offset for pagination, specified as either a hexadecimal or decimal string. Use this to resume from a previous position or fetch multiple token ranges in parallel."),
    ...
    # Construct request model with validation
    try:
        _request = _models.GetNfTsForCollectionV3Request(
            query=_models.GetNfTsForCollectionV3RequestQuery(with_metadata=with_metadata, start_token=start_token, limit=limit, contract_address=contract_address, collection_slug=collection_slug)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nfts_for_collection: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_nfts_for_collection",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_nft_metadata(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetNftMetadataV3Request(
            query=_models.GetNftMetadataV3RequestQuery(contract_address=contract_address, token_id=token_id, token_type=token_type)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_nft_metadata: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def batch_retrieve_nft_metadata(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetNftMetadataBatchV3Request(
            body=_models.GetNftMetadataBatchV3RequestBody(tokens=tokens)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for batch_retrieve_nft_metadata: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="batch_retrieve_nft_metadata",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def get_nft_contract_metadata(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetContractMetadataV3Request(
            query=_models.GetContractMetadataV3RequestQuery(contract_address=contract_address)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_nft_contract_metadata: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_nft_contract_metadata",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_collection_metadata(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetCollectionMetadataV3Request(
            query=_models.GetCollectionMetadataV3RequestQuery(collection_slug=collection_slug)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_collection_metadata: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def batch_get_contract_metadata(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetContractMetadataBatchV3Request(
            body=_models.GetContractMetadataBatchV3RequestBody(contract_addresses=contract_addresses)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for batch_get_contract_metadata: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="batch_get_contract_metadata",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def get_nft_owners(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetOwnersForNftV3Request(
            query=_models.GetOwnersForNftV3RequestQuery(contract_address=contract_address, token_id=token_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_nft_owners: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def list_nft_contract_owners(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetOwnersForContractV3Request(
            query=_models.GetOwnersForContractV3RequestQuery(contract_address=contract_address, with_token_balances=with_token_balances)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nft_contract_owners: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_nft_contract_owners",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def check_spam_contract(
    ...
    # Construct request model with validation
    try:
        _request = _models.IsSpamContractV3Request(
            query=_models.IsSpamContractV3RequestQuery(contract_address=contract_address)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for check_spam_contract: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def check_nft_airdrop(
    ...
    # Construct request model with validation
    try:
        _request = _models.IsAirdropNftV3Request(
            query=_models.IsAirdropNftV3RequestQuery(contract_address=contract_address, token_id=token_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for check_nft_airdrop: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def get_nft_collection_attribute_summary(
    ...
    # Construct request model with validation
    try:
        _request = _models.SummarizeNftAttributesV3Request(
            query=_models.SummarizeNftAttributesV3RequestQuery(contract_address=contract_address)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_nft_collection_attribute_summary: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_nft_collection_attribute_summary",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_nft_collection_floor_prices(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetFloorPriceV3Request(
            query=_models.GetFloorPriceV3RequestQuery(contract_address=contract_address)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_nft_collection_floor_prices: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_nft_collection_floor_prices",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def search_contract_metadata(
    ...
    # Construct request model with validation
    try:
        _request = _models.SearchContractMetadataV3Request(
            query=_models.SearchContractMetadataV3RequestQuery(query=query)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for search_contract_metadata: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="search_contract_metadata",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def check_nft_holder(
    ...
    # Construct request model with validation
    try:
        _request = _models.IsHolderOfContractV3Request(
            query=_models.IsHolderOfContractV3RequestQuery(wallet=wallet, contract_address=contract_address)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for check_nft_holder: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def calculate_nft_attribute_rarity(
    ...
    # Construct request model with validation
    try:
        _request = _models.ComputeRarityV3Request(
            query=_models.ComputeRarityV3RequestQuery(contract_address=contract_address, token_id=token_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for calculate_nft_attribute_rarity: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="calculate_nft_attribute_rarity",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_nft_sales(
    ...
    to_block: str | None = Field(None, alias="toBlock", description="The ending block number for fetching NFT sales data. Accepts decimal integers, hexadecimal integers (prefixed with 0x), or 'latest'. Defaults to the latest block."),
    order: Literal["asc", "desc"] | None = Field(None, description="Sort order for results relative to the block range. Choose 'asc' for ascending order from the start block or 'desc' for descending order. Defaults to descending."),
    marketplace: Literal["seaport", "looksrare", "x2y2", "wyvern", "blur", "cryptopunks"] | None = Field(None, description="Filter sales by a specific NFT marketplace. Supported values are seaport, looksrare, x2y2, wyvern, blur, and cryptopunks. Omit to return sales from all supported marketplaces."),
    ...
    # Construct request model with validation
    try:
        _request = _models.GetNftSalesV3Request(
            query=_models.GetNftSalesV3RequestQuery(from_block=from_block, to_block=to_block, order=order, marketplace=marketplace, token_id=token_id, taker=taker, limit=limit)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nft_sales: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def list_nft_contracts_by_owner(
    ...
    page_size: int | None = Field(None, alias="pageSize", description="Number of contracts to return per page, up to a maximum of 100. Defaults to 100."),
    ...
    # Construct request model with validation
    try:
        _request = _models.GetContractsForOwnerV3Request(
            query=_models.GetContractsForOwnerV3RequestQuery(owner=owner, page_size=page_size, with_metadata=with_metadata, order_by=order_by, spam_confidence_level=spam_confidence_level)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nft_contracts_by_owner: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_nft_contracts_by_owner",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def list_nft_collections_by_owner(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetCollectionsForOwnerV3Request(
            query=_models.GetCollectionsForOwnerV3RequestQuery(owner=owner, page_size=page_size, with_metadata=with_metadata)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nft_collections_by_owner: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_nft_collections_by_owner",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def report_spam_address(
    ...
    # Construct request model with validation
    try:
        _request = _models.ReportSpamV3Request(
            query=_models.ReportSpamV3RequestQuery(address=address, is_spam=is_spam)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for report_spam_address: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def refresh_nft_metadata(
    ...
    # Construct request model with validation
    try:
        _request = _models.RefreshNftMetadataV3Request(
            body=_models.RefreshNftMetadataV3RequestBody(contract_address=contract_address, token_id=token_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for refresh_nft_metadata: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="refresh_nft_metadata",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
async def list_nfts(
    ...
    contract_addresses: list[str] | None = Field(None, alias="contractAddresses", description="Optional list of contract addresses to filter results. Specify up to 45 contract addresses to narrow the response to specific NFT collections."),
    with_metadata: bool | None = Field(None, alias="withMetadata", description="Whether to include full NFT metadata in the response. Set to false to reduce payload size and improve response speed; defaults to true."),
    ...
    # Construct request model with validation
    try:
        _request = _models.GetNfTsRequest(
            query=_models.GetNfTsRequestQuery(owner=owner, contract_addresses=contract_addresses, with_metadata=with_metadata, order_by=order_by, spam_confidence_level=spam_confidence_level, page_size=page_size)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nfts: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def get_token_owners(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetOwnersForTokenRequest(
            query=_models.GetOwnersForTokenRequestQuery(contract_address=contract_address, token_id=token_id, page_size=page_size)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_token_owners: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def list_nft_collection_owners(
    ...
    # Construct request model with validation
    try:
        _request = _models.GetOwnersForCollectionRequest(
            query=_models.GetOwnersForCollectionRequestQuery(contract_address=contract_address, with_token_balances=with_token_balances)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_nft_collection_owners: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_nft_collection_owners",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def check_token_airdrop(
    ...
    # Construct request model with validation
    try:
        _request = _models.IsAirdropRequest(
            query=_models.IsAirdropRequestQuery(contract_address=contract_address, token_id=token_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for check_token_airdrop: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
...
async def check_wallet_nft_collection_ownership(
    ...
    # Construct request model with validation
    try:
        _request = _models.IsHolderOfCollectionRequest(
            query=_models.IsHolderOfCollectionRequestQuery(wallet=wallet, contract_address=contract_address)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for check_wallet_nft_collection_ownership: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="check_wallet_nft_collection_ownership",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
