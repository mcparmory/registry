...
try:
    _auth_handlers["api_key_header"] = _auth.APIKeyAuth(env_var="API_KEY_HEADER_API_KEY", location="header", param_name="Circle-Token")
...
try:
    _auth_handlers["api_key_query"] = _auth.APIKeyAuth(env_var="API_KEY_QUERY_API_KEY", location="query", param_name="circle-token")
...
async def get_project_workflow_summary(
    project_slug: str = Field(..., alias="project-slug", description="The project slug identifying the target project, formed as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID as org-name, and the project ID as repo-name. Forward slashes may be URL-escaped.", examples=['gh/CircleCI-Public/api-preview-docs']),
    reporting_window: Literal["last-7-days", "last-90-days", "last-24-hours", "last-30-days", "last-60-days"] | None = Field(None, alias="reporting-window", description="The time window over which summary metrics are calculated. Trends are only supported for windows up to 30 days; defaults to last-90-days if omitted.", examples=['last-90-days']),
    branches: dict[str, Any] | None = Field(None, description="One or more VCS branch names to filter branch-level workflow metrics. Multiple branches can be specified by repeating the query parameter.", examples=['A single branch: ?branches=main or for multiple branches: ?branches=main&branches=feature&branches=dev']),
    workflow_names: dict[str, Any] | None = Field(None, alias="workflow-names", description="One or more workflow names to filter workflow-level metrics. Multiple workflow names can be specified by repeating the query parameter.", examples=['A single workflow name: ?workflow-names=build-test-deploy or\n    for multiple workflow names: ?workflow-names=build&workflow-names=test-and-deploy.'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_project_workflow_summary", "GET", "/insights/pages/{project-slug}/summary", _request_id)
    ...
    _auth = _get_auth_for_operation("get_project_workflow_summary")
...
async def list_job_timeseries(
    ...
    granularity: Literal["daily", "hourly"] | None = Field(None, description="The time resolution for the returned timeseries data, either per hour or per day. Hourly data is only available for the past 48 hours; daily data is available for up to 90 days.", examples=['hourly']),
    start_date: str | None = Field(None, alias="start-date", description="The inclusive start of the time range filter, in ISO 8601 date-time format. Must be provided if an end-date is specified.", examples=['2020-08-21T13:26:29Z'], json_schema_extra={'format': 'date-time'}),
    end_date: str | None = Field(None, alias="end-date", description="The exclusive end of the time range filter, in ISO 8601 date-time format. Must be no more than 90 days after the start-date.", examples=['2020-09-04T13:26:29Z'], json_schema_extra={'format': 'date-time'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_job_timeseries", "GET", "/insights/time-series/{project-slug}/workflows/{workflow-name}/jobs", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_job_timeseries")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_org_summary(
    org_slug: str = Field(..., alias="org-slug", description="The organization slug identifying the target org, combining the VCS provider slug and organization name separated by a forward slash (which may be URL-encoded).", examples=['gh/CircleCI-Public']),
    reporting_window: Literal["last-7-days", "last-90-days", "last-24-hours", "last-30-days", "last-60-days"] | None = Field(None, alias="reporting-window", description="The time window over which summary metrics are calculated and trends are derived. Defaults to the last 90 days if not specified.", examples=['last-90-days']),
    project_names: dict[str, Any] | None = Field(None, alias="project-names", description="An optional list of project names used to filter results to specific projects within the org. Provide the parameter multiple times to include multiple projects.", examples=['For a single project: ?project-names=some-project or for multiple projects: ?project-names=some-project1&project-names=some-project2'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_org_summary", "GET", "/insights/{org-slug}/summary", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_org_summary")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_project_branches(
    project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project, formatted as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID as org-name, and the project ID as repo-name. Forward slashes may be URL-escaped.", examples=['gh/CircleCI-Public/api-preview-docs']),
    workflow_name: str | None = Field(None, alias="workflow-name", description="Filters the returned branches to only those associated with the specified workflow name. When omitted, branches are scoped to the entire project across all workflows.", examples=['build-and-test'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_project_branches", "GET", "/insights/{project-slug}/branches", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_project_branches")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_flaky_tests(project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project, formatted as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID (from Organization Settings) as org-name, and the project ID (from Project Settings) as repo-name. Forward slashes may be URL-escaped.", examples=['gh/CircleCI-Public/api-preview-docs'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_flaky_tests", "GET", "/insights/{project-slug}/flaky-tests", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_flaky_tests")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_workflow_metrics(
    project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project in the format `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the Organization ID (from Organization Settings) as org-name, and the Project ID (from Project Settings) as repo-name. Forward slashes may be URL-encoded.", examples=['gh/CircleCI-Public/api-preview-docs']),
    ...
    reporting_window: Literal["last-7-days", "last-90-days", "last-24-hours", "last-30-days", "last-60-days"] | None = Field(None, alias="reporting-window", description="Defines the time window over which summary metrics are calculated. Accepts predefined window values; if omitted, defaults to the last 90 days.", examples=['last-90-days'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workflow_metrics", "GET", "/insights/{project-slug}/workflows", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workflow_metrics")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_workflow_runs(
    project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project in the format `vcs-slug/org-name/repo-name`, where slashes may be URL-escaped. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the Organization ID as org-name, and the Project ID as repo-name.", examples=['gh/CircleCI-Public/api-preview-docs']),
    workflow_name: str = Field(..., alias="workflow-name", description="The exact name of the workflow whose runs you want to retrieve, as defined in the project's pipeline configuration.", examples=['build-and-test']),
    ...
    start_date: str | None = Field(None, alias="start-date", description="Filters results to only include workflow runs that started at or after this ISO 8601 datetime. Required when an end-date is specified.", examples=['2020-08-21T13:26:29Z'], json_schema_extra={'format': 'date-time'}),
    end_date: str | None = Field(None, alias="end-date", description="Filters results to only include workflow runs that started before this ISO 8601 datetime. Must be no more than 90 days after the start-date.", examples=['2020-09-04T13:26:29Z'], json_schema_extra={'format': 'date-time'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workflow_runs", "GET", "/insights/{project-slug}/workflows/{workflow-name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workflow_runs")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_workflow_job_metrics(
    ...
    workflow_name: str = Field(..., alias="workflow-name", description="The exact name of the workflow whose job metrics you want to retrieve.", examples=['build-and-test']),
    ...
    reporting_window: Literal["last-7-days", "last-90-days", "last-24-hours", "last-30-days", "last-60-days"] | None = Field(None, alias="reporting-window", description="Defines the time window over which summary metrics are calculated. Defaults to the last 90 days if not specified.", examples=['last-90-days']),
    job_name: str | None = Field(None, alias="job-name", description="Filters results to jobs whose names match the provided string, either as a full job name or a substring. Returns metrics for all jobs in the workflow if omitted.", examples=['lint'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workflow_job_metrics", "GET", "/insights/{project-slug}/workflows/{workflow-name}/jobs", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workflow_job_metrics")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_workflow_summary(
    project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project, formatted as vcs-slug/org-name/repo-name. For GitLab or GitHub App projects, use 'circleci' as the vcs-slug, the organization ID as org-name, and the project ID as repo-name.", examples=['gh/CircleCI-Public/api-preview-docs']),
    workflow_name: str = Field(..., alias="workflow-name", description="The exact name of the workflow for which to retrieve summary metrics.", examples=['build-and-test']),
    ...
    # Log invocation
    _log_tool_invocation("get_workflow_summary", "GET", "/insights/{project-slug}/workflows/{workflow-name}/summary", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_workflow_summary")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_workflow_test_metrics(
    project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project in the format `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID (from Organization Settings) as org-name, and the project ID (from Project Settings) as repo-name. Forward slashes may be URL-escaped.", examples=['gh/CircleCI-Public/api-preview-docs']),
    workflow_name: str = Field(..., alias="workflow-name", description="The exact name of the workflow for which test metrics should be retrieved.", examples=['build-and-test']),
    ...
    # Log invocation
    _log_tool_invocation("get_workflow_test_metrics", "GET", "/insights/{project-slug}/workflows/{workflow-name}/test-metrics", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_workflow_test_metrics")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Job
@mcp.tool()
async def cancel_job(job_id: str = Field(..., alias="job-id", description="The unique identifier of the job to cancel, in UUID format.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("cancel_job", "POST", "/jobs/{job-id}/cancel", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("cancel_job")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_current_user() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_current_user", "GET", "/me", _request_id)
...
async def list_collaborations() -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_collaborations", "GET", "/me/collaborations", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_collaborations")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_organization(
    ...
    vcs_type: Literal["github", "bitbucket", "circleci"] | None = Field(None, description="The version control system associated with the organization, or 'circleci' for a standalone organization not tied to a VCS provider.", examples=['github'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_organization", "POST", "/organization", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_organization")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_organization(org_slug_or_id: str = Field(..., alias="org-slug-or-id", description="The organization identifier, either as a UUID or a VCS slug in the format `vcs-slug/org-name` (e.g., `gh/` for GitHub, `bb/` for Bitbucket). For GitLab or GitHub App integrations, use `circleci` as the VCS slug and provide the numeric organization ID (found in Organization Settings) in place of the org name.", examples=['gh/CircleCI-Public'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_organization", "GET", "/organization/{org-slug-or-id}", _request_id)
...
async def delete_organization(org_slug_or_id: str = Field(..., alias="org-slug-or-id", description="The unique identifier for the organization, either as a UUID or a VCS-prefixed slug in the format `vcs-slug/org-name`. For organizations using GitLab or GitHub App, use `circleci` as the VCS slug and the organization ID (found in Organization Settings) as the org name.", examples=['gh/CircleCI-Public'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_organization", "DELETE", "/organization/{org-slug-or-id}", _request_id)
...
async def create_project(
    ...
    # Log invocation
    _log_tool_invocation("create_project", "POST", "/organization/{org-slug-or-id}/project", _request_id)
...
async def list_url_orb_allow_list_entries(org_slug_or_id: str = Field(..., alias="org-slug-or-id", description="The organization identifier, either as a UUID or a slug in the format `vcs-slug/org-name`. For organizations using GitLab or GitHub App, use `circleci` as the VCS slug and provide the organization ID (available in Organization Settings) in place of the org name.", examples=['gh/CircleCI-Public'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_url_orb_allow_list_entries", "GET", "/organization/{org-slug-or-id}/url-orb-allow-list", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_url_orb_allow_list_entries")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_url_orb_allow_list_entry(
    org_slug_or_id: str = Field(..., alias="org-slug-or-id", description="The organization identifier, either as a UUID or a slug in the form `vcs-slug/org-name`. For organizations using GitLab or GitHub App, use `circleci` as the vcs-slug and the organization ID (found in Organization Settings) as the org-name.", examples=['gh/CircleCI-Public']),
    name: str | None = Field(None, description="A human-readable label for this allow-list entry to help identify its purpose within the organization.", examples=['Allow URL orbs from raw.githubusercontent.com/CircleCI-Public']),
    prefix: Any | None = Field(None, description="The URL prefix that defines which URL orb references are permitted; any orb reference URL beginning with this prefix will be allowed by this entry.", examples=['https://raw.githubusercontent.com/CircleCI-Public/orbs/refs/heads/main/'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_url_orb_allow_list_entry", "POST", "/organization/{org-slug-or-id}/url-orb-allow-list", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_url_orb_allow_list_entry")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_url_orb_allow_list_entry(
    org_slug_or_id: str = Field(..., alias="org-slug-or-id", description="The organization identifier, either as a UUID or a slug in the format `vcs-slug/org-name`. For GitLab or GitHub App projects, use `circleci` as the `vcs-slug` and provide the organization ID (found in Organization Settings) as the `org-name`.", examples=['gh/CircleCI-Public']),
    allow_list_entry_id: str = Field(..., alias="allow-list-entry-id", description="The UUID of the URL orb allow-list entry to remove. This uniquely identifies the specific allow-list entry to be deleted.", examples=['ba98990a-5a00-4cad-b55e-b44117b92e0c'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_url_orb_allow_list_entry", "DELETE", "/organization/{org-slug-or-id}/url-orb-allow-list/{allow-list-entry-id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_url_orb_allow_list_entry")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_pipelines(
    org_slug: str | None = Field(None, alias="org-slug", description="The organization slug identifying the target organization, formatted as vcs-slug/org-name. For GitLab or GitHub App projects, use 'circleci' as the vcs-slug and supply the organization ID (found in Organization Settings) as the org-name.", examples=['gh/CircleCI-Public']),
    ...
    # Log invocation
    _log_tool_invocation("list_pipelines", "GET", "/pipeline", _request_id)
...
async def continue_pipeline(
    ...
    parameters: dict[str, int | str | bool] | None = Field(None, description="A key-value map of pipeline parameter names to their values, subject to limits of 100 max entries, 128-character maximum key length, and 512-character maximum value length.", examples=[{'deploy_prod': True}])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("continue_pipeline", "POST", "/pipeline/continue", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("continue_pipeline")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Pipeline
@mcp.tool()
async def get_pipeline(pipeline_id: str = Field(..., alias="pipeline-id", description="The unique identifier of the pipeline to retrieve, provided as a UUID.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline", "GET", "/pipeline/{pipeline-id}", _request_id)
...
# Tags: Pipeline
@mcp.tool()
async def get_pipeline_config(pipeline_id: str = Field(..., alias="pipeline-id", description="The unique identifier of the pipeline whose configuration you want to retrieve. Must be a valid UUID corresponding to an existing pipeline.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_config", "GET", "/pipeline/{pipeline-id}/config", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_config")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Pipeline
@mcp.tool()
async def get_pipeline_values(pipeline_id: str = Field(..., alias="pipeline-id", description="The unique identifier of the pipeline whose values you want to retrieve, in UUID format.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_values", "GET", "/pipeline/{pipeline-id}/values", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_values")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Pipeline
@mcp.tool()
async def list_pipeline_workflows(pipeline_id: str = Field(..., alias="pipeline-id", description="The unique identifier of the pipeline whose workflows you want to retrieve. Must be a valid UUID.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_pipeline_workflows", "GET", "/pipeline/{pipeline-id}/workflow", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipeline_workflows")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_project(project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project in the format `vcs-slug/org-name/repo-name`, where slashes may be URL-escaped. For GitLab or GitHub App projects, use `circleci` as the VCS slug, the organization ID (from Organization Settings) as the org name, and the project ID (from Project Settings) as the repo name.", examples=['gh/CircleCI-Public/api-preview-docs'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_project", "GET", "/project/{project-slug}", _request_id)
...
async def delete_project(project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project in the format `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID (from Organization Settings) as org-name, and the project ID (from Project Settings) as repo-name. Forward slashes may be URL-encoded.", examples=['gh/CircleCI-Public/api-preview-docs'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_project", "DELETE", "/project/{project-slug}", _request_id)
...
async def list_checkout_keys(
    ...
    # Log invocation
    _log_tool_invocation("list_checkout_keys", "GET", "/project/{project-slug}/checkout-key", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_checkout_keys")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_checkout_key(
    project_slug: str = Field(..., alias="project-slug", description="The project slug identifying the target project, formed as vcs-slug/org-name/repo-name. For GitLab or GitHub App projects, use 'circleci' as the vcs-slug with the organization ID and project ID in place of org-name and repo-name respectively.", examples=['gh/CircleCI-Public/api-preview-docs']),
    type_: Literal["user-key", "deploy-key"] | None = Field(None, alias="type", description="The type of checkout key to create: 'deploy-key' grants read-only repository access for deployments, while 'user-key' grants access tied to a specific GitHub user account.", examples=['deploy-key'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_checkout_key", "POST", "/project/{project-slug}/checkout-key", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_checkout_key")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_checkout_key(
    ...
    fingerprint: str = Field(..., description="The SSH key fingerprint used to uniquely identify the checkout key, accepted in either MD5 or SHA256 format. SHA256 fingerprints must be URL-encoded.", examples=['c9:0b:1c:4f:d5:65:56:b9:ad:88:f9:81:2b:37:74:2f'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_checkout_key", "GET", "/project/{project-slug}/checkout-key/{fingerprint}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_checkout_key")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_checkout_key(
    project_slug: str = Field(..., alias="project-slug", description="The project slug identifying the target project, formed as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID (from Organization Settings) as org-name, and the project ID (from Project Settings) as repo-name. Forward slashes may be URL-escaped.", examples=['gh/CircleCI-Public/api-preview-docs']),
    fingerprint: str = Field(..., description="The MD5 or SHA256 fingerprint of the SSH checkout key to delete. SHA256 fingerprints must be URL-encoded.", examples=['c9:0b:1c:4f:d5:65:56:b9:ad:88:f9:81:2b:37:74:2f'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_checkout_key", "DELETE", "/project/{project-slug}/checkout-key/{fingerprint}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_checkout_key")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_env_vars(project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project, formatted as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID (from Organization Settings) as org-name, and the project ID (from Project Settings) as repo-name. Forward slashes may be URL-encoded.", examples=['gh/CircleCI-Public/api-preview-docs'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_env_vars", "GET", "/project/{project-slug}/envvar", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_env_vars")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_env_var(
    ...
    name: str | None = Field(None, description="The name of the environment variable to create, which must be unique within the project and will be used to reference the variable in pipeline configurations.", examples=['foo']),
    value: str | None = Field(None, description="The value to assign to the environment variable; once stored, this value will be masked and not returned in plaintext by the API.", examples=['xxxx1234'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_env_var", "POST", "/project/{project-slug}/envvar", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_env_var")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_env_var(
    project_slug: str = Field(..., alias="project-slug", description="The project slug identifying the target project, formed as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID as org-name, and the project ID as repo-name. Forward slashes may be URL-escaped.", examples=['gh/CircleCI-Public/api-preview-docs']),
    name: str = Field(..., description="The exact name of the environment variable to retrieve, matching the name as defined in the project's environment variable settings.", examples=['foo'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_env_var", "GET", "/project/{project-slug}/envvar/{name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_env_var")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_env_var(
    project_slug: str = Field(..., alias="project-slug", description="The project slug uniquely identifying the target project, formatted as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID (from Organization Settings) as org-name, and the project ID (from Project Settings) as repo-name; forward slashes may be URL-encoded.", examples=['gh/CircleCI-Public/api-preview-docs']),
    name: str = Field(..., description="The exact name of the environment variable to delete, matching the variable's name as it appears in the project's environment variable settings.", examples=['foo'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_env_var", "DELETE", "/project/{project-slug}/envvar/{name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_env_var")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_job_details(
    job_number: Any = Field(..., alias="job-number", description="The unique numeric identifier of the job to retrieve details for.", examples=['123']),
    ...
    # Log invocation
    _log_tool_invocation("get_job_details", "GET", "/project/{project-slug}/job/{job-number}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_job_details")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def cancel_job_by_number(
    job_number: Any = Field(..., alias="job-number", description="The unique numeric identifier of the job to cancel within the project.", examples=['123']),
    project_slug: str = Field(..., alias="project-slug", description="Project slug identifying the target project, formatted as vcs-slug/org-name/repo-name. For GitLab or GitHub App projects, use circleci as the vcs-slug, the organization ID as org-name, and the project ID as repo-name; forward slashes may be URL-escaped.", examples=['gh/CircleCI-Public/api-preview-docs'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("cancel_job_by_number", "POST", "/project/{project-slug}/job/{job-number}/cancel", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("cancel_job_by_number")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_project_pipelines(
    ...
    # Log invocation
    _log_tool_invocation("list_project_pipelines", "GET", "/project/{project-slug}/pipeline", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_project_pipelines")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def trigger_pipeline(
    project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project in the format `vcs-slug/org-name/repo-name`, where forward slashes may be URL-encoded. For GitLab or GitHub App projects, use `circleci` as the vcs-slug with the organization ID and project ID in place of org and repo names.", examples=['gh/CircleCI-Public/api-preview-docs']),
    branch: str | None = Field(None, description="The branch to run the pipeline against, using the HEAD commit of that branch. Mutually exclusive with `tag`; only one may be provided. To target a pull request, use `pull/<number>/head` for the PR ref or `pull/<number>/merge` for the merge ref (GitHub only).", examples=['feature/design-new-api']),
    tag: str | None = Field(None, description="A Git tag whose pointed-to commit will be used for the pipeline run. Mutually exclusive with `branch`; only one may be provided.", examples=['v3.1.4159']),
    parameters: dict[str, int | str | bool] | None = Field(None, description="A key-value map of pipeline parameter names to their values, used to customize the pipeline run. Limited to 100 entries, with keys up to 128 characters and values up to 512 characters each.", examples=[{'deploy_prod': True}])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("trigger_pipeline", "POST", "/project/{project-slug}/pipeline", _request_id)
...
async def list_my_pipelines(project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project, formatted as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID (from Organization Settings) as org-name, and the project ID (from Project Settings) as repo-name. Forward slashes may be URL-encoded.", examples=['gh/CircleCI-Public/api-preview-docs'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_my_pipelines", "GET", "/project/{project-slug}/pipeline/mine", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_my_pipelines")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_pipeline_by_number(
    project_slug: str = Field(..., alias="project-slug", description="The project slug identifying the target project, formatted as `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID as org-name, and the project ID as repo-name.", examples=['gh/CircleCI-Public/api-preview-docs']),
    pipeline_number: Any = Field(..., alias="pipeline-number", description="The sequential number assigned to the pipeline within the project, uniquely identifying it among all pipelines for that project.", examples=['123'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_by_number", "GET", "/project/{project-slug}/pipeline/{pipeline-number}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_by_number")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_project_schedules(project_slug: str = Field(..., alias="project-slug", description="Unique identifier for the project in the format `vcs-slug/org-name/repo-name`. For GitLab or GitHub App projects, use `circleci` as the vcs-slug, the organization ID (from Organization Settings) as org-name, and the project ID (from Project Settings) as repo-name. Forward slashes may be URL-encoded.", examples=['gh/CircleCI-Public/api-preview-docs'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_project_schedules", "GET", "/project/{project-slug}/schedule", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_project_schedules")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_schedule(
    ...
    attribution_actor: Literal["current", "system"] | None = Field(None, alias="attribution-actor", description="Determines which actor's permissions are used when the scheduled pipeline runs — `current` uses the token owner's permissions, while `system` uses neutral system-level permissions.", examples=['current']),
    parameters: dict[str, int | str | bool] | None = Field(None, description="Key-value pairs of pipeline parameters passed to each triggered pipeline run; must include at least a `branch` or `tag` key to specify the target ref.", examples=[{'deploy_prod': True, 'branch': 'feature/design-new-api'}]),
    ...
    # Log invocation
    _log_tool_invocation("create_schedule", "POST", "/project/{project-slug}/schedule", _request_id)
...
async def list_job_artifacts(
    job_number: Any = Field(..., alias="job-number", description="The unique number identifying the job within the project whose artifacts you want to retrieve.", examples=['123']),
    ...
    # Log invocation
    _log_tool_invocation("list_job_artifacts", "GET", "/project/{project-slug}/{job-number}/artifacts", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_job_artifacts")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_job_tests(
    job_number: Any = Field(..., alias="job-number", description="The unique numeric identifier of the job whose test metadata you want to retrieve.", examples=['123']),
    ...
    # Log invocation
    _log_tool_invocation("list_job_tests", "GET", "/project/{project-slug}/{job-number}/tests", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_job_tests")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Schedule
@mcp.tool()
async def get_schedule(schedule_id: str = Field(..., alias="schedule-id", description="The unique identifier of the schedule to retrieve, in UUID format.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_schedule", "GET", "/schedule/{schedule-id}", _request_id)
...
async def update_schedule(
    schedule_id: str = Field(..., alias="schedule-id", description="The unique UUID identifying the schedule to update.", json_schema_extra={'format': 'uuid'}),
    ...
    per_hour: int | None = Field(None, alias="per-hour", description="How many times the schedule triggers per hour; must be a whole number between 1 and 60. Mutually exclusive with hour-based scheduling fields.", json_schema_extra={'format': 'integer'}),
    ...
    attribution_actor: Literal["current", "system"] | None = Field(None, alias="attribution-actor", description="Determines whose permissions are used when the scheduled pipeline runs: 'current' uses the token owner's permissions, 'system' uses a neutral system actor.", examples=['current']),
    parameters: dict[str, int | str | bool] | None = Field(None, description="Key-value pairs of pipeline parameters to pass when the schedule triggers; must include either a branch or tag key to specify the target ref.", examples=[{'deploy_prod': True, 'branch': 'feature/design-new-api'}])
) -> dict[str, Any]:
    ...
    try:
        _request = _models.UpdateScheduleRequest(
            ...
            body=_models.UpdateScheduleRequestBody(description=description, name=name, attribution_actor=attribution_actor, parameters=parameters,
                timetable=_models.UpdateScheduleRequestBodyTimetable(per_hour=per_hour, hours_of_day=hours_of_day, days_of_week=days_of_week, days_of_month=days_of_month, months=months) if any(v is not None for v in [per_hour, hours_of_day, days_of_week, days_of_month, months]) else None)
        )
    except pydantic.ValidationError as _validation_err:
    ...
    # Log invocation
    _log_tool_invocation("update_schedule", "PATCH", "/schedule/{schedule-id}", _request_id)
...
# Tags: Schedule
@mcp.tool()
async def delete_schedule(schedule_id: str = Field(..., alias="schedule-id", description="The unique identifier of the schedule to delete, in UUID format.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_schedule", "DELETE", "/schedule/{schedule-id}", _request_id)
...
# Tags: User
@mcp.tool()
async def get_user(id_: str = Field(..., alias="id", description="The unique identifier of the user whose information should be retrieved, provided as a UUID.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_user", "GET", "/user/{id}", _request_id)
...
async def list_webhooks(
    scope_id: str = Field(..., alias="scope-id", description="The unique identifier of the scope entity to filter webhooks by. Currently only project IDs are supported.", json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("list_webhooks", "GET", "/webhook", _request_id)
...
async def create_webhook(
    ...
    id_: str | None = Field(None, alias="id", description="The UUID of the scope (e.g., a project) this webhook is associated with; currently only project IDs are supported.", json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("create_webhook", "POST", "/webhook", _request_id)
...
# Tags: Webhook
@mcp.tool()
async def get_webhook(webhook_id: str = Field(..., alias="webhook-id", description="The unique identifier of the outbound webhook to retrieve, provided as a UUID.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_webhook", "GET", "/webhook/{webhook-id}", _request_id)
...
async def update_webhook(
    webhook_id: str = Field(..., alias="webhook-id", description="The unique identifier of the webhook to update.", json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("update_webhook", "PUT", "/webhook/{webhook-id}", _request_id)
...
# Tags: Webhook
@mcp.tool()
async def delete_webhook(webhook_id: str = Field(..., alias="webhook-id", description="The unique identifier of the outbound webhook to delete, provided as a UUID.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_webhook", "DELETE", "/webhook/{webhook-id}", _request_id)
...
# Tags: Workflow
@mcp.tool()
async def get_workflow(id_: str = Field(..., alias="id", description="The unique identifier of the workflow to retrieve, in UUID format.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_workflow", "GET", "/workflow/{id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_workflow")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def approve_workflow_job(
    approval_request_id: str = Field(..., description="The unique identifier of the pending approval job to approve, in UUID format.", json_schema_extra={'format': 'uuid'}),
    id_: str = Field(..., alias="id", description="The unique identifier of the workflow containing the pending approval job, in UUID format.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("approve_workflow_job", "POST", "/workflow/{id}/approve/{approval_request_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("approve_workflow_job")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Workflow
@mcp.tool()
async def cancel_workflow(id_: str = Field(..., alias="id", description="The unique identifier of the workflow to cancel. Must correspond to an existing, currently running workflow.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("cancel_workflow", "POST", "/workflow/{id}/cancel", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("cancel_workflow")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Workflow
@mcp.tool()
async def list_workflow_jobs(id_: str = Field(..., alias="id", description="The unique identifier of the workflow whose jobs you want to retrieve.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_workflow_jobs", "GET", "/workflow/{id}/job", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_workflow_jobs")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def rerun_workflow(
    id_: str = Field(..., alias="id", description="The unique identifier of the workflow to rerun.", examples=['5034460f-c7c4-4c43-9457-de07e2029e7b'], json_schema_extra={'format': 'uuid'}),
    enable_ssh: bool | None = Field(None, description="When true, enables SSH access for the triggering user on the newly rerun job. Requires the jobs parameter to be specified and is mutually exclusive with from_failed.", examples=[False]),
    from_failed: bool | None = Field(None, description="When true, reruns the workflow starting from the first failed job rather than the beginning. Mutually exclusive with the jobs and sparse_tree parameters.", examples=[False]),
    jobs: list[str] | None = Field(None, description="A list of specific job IDs (UUIDs) to rerun within the workflow. Order is not significant. Mutually exclusive with from_failed.", examples=[['c65b68ef-e73b-4bf2-be9a-7a322a9df150', '5e957edd-5e8c-4985-9178-5d0d69561822']]),
    sparse_tree: bool | None = Field(None, description="When true, applies sparse tree optimization logic during the rerun, improving performance for workflows containing disconnected subgraphs. Requires the jobs parameter and is mutually exclusive with from_failed.", examples=[False])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("rerun_workflow", "POST", "/workflow/{id}/rerun", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("rerun_workflow")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
@mcp.tool()
async def list_org_oidc_custom_claims(org_id: str = Field(..., alias="orgID", description="The unique identifier of the organization whose OIDC custom claims should be retrieved.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_org_oidc_custom_claims", "GET", "/org/{orgID}/oidc-custom-claims", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_org_oidc_custom_claims")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def update_org_oidc_claims(
    org_id: str = Field(..., alias="orgID", description="The unique identifier of the organization whose OIDC custom claims will be updated.", json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("update_org_oidc_claims", "PATCH", "/org/{orgID}/oidc-custom-claims", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_org_oidc_claims")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_org_oidc_claims(
    org_id: str = Field(..., alias="orgID", description="The unique identifier of the organization whose custom OIDC claims will be deleted.", json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("delete_org_oidc_claims", "DELETE", "/org/{orgID}/oidc-custom-claims", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_org_oidc_claims")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_project_oidc_claims(
    org_id: str = Field(..., alias="orgID", description="The unique identifier of the organization that owns the project.", json_schema_extra={'format': 'uuid'}),
    project_id: str = Field(..., alias="projectID", description="The unique identifier of the project whose custom OIDC claims are being retrieved.", json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_project_oidc_claims", "GET", "/org/{orgID}/project/{projectID}/oidc-custom-claims", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_project_oidc_claims")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def update_project_oidc_claims(
    org_id: str = Field(..., alias="orgID", description="Unique identifier of the organization that owns the project.", json_schema_extra={'format': 'uuid'}),
    project_id: str = Field(..., alias="projectID", description="Unique identifier of the project whose OIDC custom claims are being created or updated.", json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("update_project_oidc_claims", "PATCH", "/org/{orgID}/project/{projectID}/oidc-custom-claims", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_project_oidc_claims")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_project_oidc_claims(
    org_id: str = Field(..., alias="orgID", description="Unique identifier of the organization that owns the project.", json_schema_extra={'format': 'uuid'}),
    project_id: str = Field(..., alias="projectID", description="Unique identifier of the project whose OIDC custom claims will be deleted.", json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("delete_project_oidc_claims", "DELETE", "/org/{orgID}/project/{projectID}/oidc-custom-claims", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_project_oidc_claims")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_decision_logs(
    ...
    after: str | None = Field(None, description="Filters results to only include decisions made after this timestamp. Must be a valid ISO 8601 date-time string.", json_schema_extra={'format': 'date-time'}),
    before: str | None = Field(None, description="Filters results to only include decisions made before this timestamp. Must be a valid ISO 8601 date-time string.", json_schema_extra={'format': 'date-time'}),
    ...
    # Log invocation
    _log_tool_invocation("list_decision_logs", "GET", "/owner/{ownerID}/context/{context}/decision", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_decision_logs")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def evaluate_policy_decision(
    owner_id: str = Field(..., alias="ownerID", description="Unique identifier of the owner whose policies will be evaluated against the input data."),
    context: str = Field(..., description="The policy context namespace under which the decision evaluation should be performed, scoping which policies are applicable."),
    input_: str | None = Field(None, alias="input", description="The configuration or input data string to be evaluated against the owner's policies in the specified context."),
    metadata: dict[str, Any] | None = Field(None, description="Additional key-value metadata object accompanying the input, providing supplementary context to influence or enrich the policy evaluation.")
) -> dict[str, Any]:
    """Evaluates input data against an owner's stored policies within a given context and returns a decision. Combines configuration input and metadata to determine the policy outcome."""

    # Construct request model with validation
    try:
        _request = _models.MakeDecisionRequest(
            path=_models.MakeDecisionRequestPath(owner_id=owner_id, context=context),
            body=_models.MakeDecisionRequestBody(input_=input_, metadata=metadata)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for evaluate_policy_decision: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("evaluate_policy_decision", "POST", "/owner/{ownerID}/context/{context}/decision", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("evaluate_policy_decision")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_decision_settings(
    owner_id: str = Field(..., alias="ownerID", description="The unique identifier of the owner whose decision settings are being retrieved."),
    context: str = Field(..., description="The context scope under which the decision settings apply, used to namespace or segment policy evaluation configuration.")
) -> dict[str, Any]:
    """Retrieves the current decision settings for a given owner and context, such as whether policy evaluation is enabled or disabled."""

    # Construct request model with validation
    try:
        _request = _models.GetDecisionSettingsRequest(
            path=_models.GetDecisionSettingsRequestPath(owner_id=owner_id, context=context)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_decision_settings: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("get_decision_settings", "GET", "/owner/{ownerID}/context/{context}/decision/settings", _request_id)

    # Extract parameters for API call
    _http_path = _build_path("/owner/{ownerID}/context/{context}/decision/settings", _request.path.model_dump(by_alias=True)) if _request.path else "/owner/{ownerID}/context/{context}/decision/settings"
    _http_query = {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_decision_settings")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_decision_settings",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def update_decision_settings(
    owner_id: str = Field(..., alias="ownerID", description="The unique identifier of the owner whose decision settings are being updated."),
    context: str = Field(..., description="The context scope under which the decision settings apply, used to namespace policy evaluation configurations."),
    enabled: bool | None = Field(None, description="Whether policy evaluation is enabled for the specified owner and context. Set to true to activate policy enforcement, or false to disable it.")
) -> dict[str, Any]:
    """Updates the policy decision settings for a specific owner and context, such as enabling or disabling policy evaluation. Use this to control whether policy enforcement is active for a given context."""

    # Construct request model with validation
    try:
        _request = _models.SetDecisionSettingsRequest(
            path=_models.SetDecisionSettingsRequestPath(owner_id=owner_id, context=context),
            body=_models.SetDecisionSettingsRequestBody(enabled=enabled)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for update_decision_settings: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("update_decision_settings", "PATCH", "/owner/{ownerID}/context/{context}/decision/settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_decision_settings")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="update_decision_settings",
        method="PATCH",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_decision_log(
    ...
    # Log invocation
    _log_tool_invocation("get_decision_log", "GET", "/owner/{ownerID}/context/{context}/decision/{decisionID}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_decision_log")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_decision_policy_bundle(
    ...
    # Log invocation
    _log_tool_invocation("get_decision_policy_bundle", "GET", "/owner/{ownerID}/context/{context}/decision/{decisionID}/policy-bundle", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_decision_policy_bundle")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_policy_bundle(
    ...
    # Log invocation
    _log_tool_invocation("get_policy_bundle", "GET", "/owner/{ownerID}/context/{context}/policy-bundle", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_policy_bundle")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_policy_bundle(
    owner_id: str = Field(..., alias="ownerID", description="The unique identifier of the owner (organization or user) whose policy bundle is being updated."),
    context: str = Field(..., description="The context scope under which the policy bundle applies, used to namespace and isolate policies by environment or purpose."),
    dry: bool | None = Field(None, description="When set to true, validates and previews the policy bundle replacement without persisting any changes."),
    policies: dict[str, str] | None = Field(None, description="A collection of policy definitions that will fully replace the existing policy bundle for the specified owner and context.")
) -> dict[str, Any]:
    """Replaces the current policy bundle for a given owner and context with the provided set of policies. Use the dry-run option to validate the bundle without applying changes."""

    # Construct request model with validation
    try:
        _request = _models.CreatePolicyBundleRequest(
            path=_models.CreatePolicyBundleRequestPath(owner_id=owner_id, context=context),
            query=_models.CreatePolicyBundleRequestQuery(dry=dry),
            body=_models.CreatePolicyBundleRequestBody(policies=policies)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for create_policy_bundle: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("create_policy_bundle", "POST", "/owner/{ownerID}/context/{context}/policy-bundle", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_policy_bundle")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="create_policy_bundle",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_policy_document(
    owner_id: str = Field(..., alias="ownerID", description="The unique identifier of the owner (e.g., organization or user) whose policy documents are being accessed."),
    context: str = Field(..., description="The context scope under which the policy bundle is organized, used to namespace or categorize policies within an owner."),
    policy_name: str = Field(..., alias="policyName", description="The name of the policy to retrieve, as defined by the rego policy_name rule within the policy document.")
) -> dict[str, Any]:
    """Retrieves a specific policy document by name within a given owner context. Use this to inspect the full content of a named policy in a policy bundle."""

    # Construct request model with validation
    try:
        _request = _models.GetPolicyDocumentRequest(
            path=_models.GetPolicyDocumentRequestPath(owner_id=owner_id, context=context, policy_name=policy_name)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_policy_document: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Log invocation
    _log_tool_invocation("get_policy_document", "GET", "/owner/{ownerID}/context/{context}/policy-bundle/{policyName}", _request_id)

    # Extract parameters for API call
    _http_path = _build_path("/owner/{ownerID}/context/{context}/policy-bundle/{policyName}", _request.path.model_dump(by_alias=True)) if _request.path else "/owner/{ownerID}/context/{context}/policy-bundle/{policyName}"
    _http_query = {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_policy_document")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_contexts(
    owner_id: str | None = Field(None, alias="owner-id", description="The unique UUID of the organization that owns the contexts. Use this or owner-slug to identify the organization — find both in CircleCI web app under Organization Settings > Overview.", json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("list_contexts", "GET", "/context", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_contexts")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_context(
    ...
    # Log invocation
    _log_tool_invocation("create_context", "POST", "/context", _request_id)
...
# Tags: Context
@mcp.tool()
async def get_context(context_id: str = Field(..., description="The unique identifier of the context to retrieve, provided as a UUID.", examples=['be8bb2e3-c3d6-4098-89f4-572ff976ba9a'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_context", "GET", "/context/{context_id}", _request_id)
...
# Tags: Context
@mcp.tool()
async def delete_context(context_id: str = Field(..., description="The unique identifier of the context to delete. Deleting a context will also remove all environment variables stored within it.", examples=['be8bb2e3-c3d6-4098-89f4-572ff976ba9a'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_context", "DELETE", "/context/{context_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_context")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Context
@mcp.tool()
async def list_context_environment_variables(context_id: str = Field(..., description="The unique identifier of the context whose environment variables should be listed.", examples=['be8bb2e3-c3d6-4098-89f4-572ff976ba9a'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_context_environment_variables", "GET", "/context/{context_id}/environment-variable", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_context_environment_variables")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def set_context_environment_variable(
    context_id: str = Field(..., description="The unique identifier of the context in which to create or update the environment variable.", examples=['be8bb2e3-c3d6-4098-89f4-572ff976ba9a'], json_schema_extra={'format': 'uuid'}),
    env_var_name: str = Field(..., description="The name of the environment variable to create or update within the context.", examples=['POSTGRES_USER']),
    value: str | None = Field(None, description="The value to assign to the environment variable; treated as a secret and not returned in responses.", examples=['some-secret-value'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("set_context_environment_variable", "PUT", "/context/{context_id}/environment-variable/{env_var_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("set_context_environment_variable")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_context_environment_variable(
    context_id: str = Field(..., description="The unique identifier of the context from which the environment variable will be deleted.", examples=['be8bb2e3-c3d6-4098-89f4-572ff976ba9a'], json_schema_extra={'format': 'uuid'}),
    env_var_name: str = Field(..., description="The exact name of the environment variable to delete, matching the name as it was originally stored in the context.", examples=['POSTGRES_USER'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_context_environment_variable", "DELETE", "/context/{context_id}/environment-variable/{env_var_name}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_context_environment_variable")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Context
@mcp.tool()
async def list_context_restrictions(context_id: str = Field(..., description="The unique identifier of the context whose restrictions should be retrieved, provided as a UUID.", examples=['be8bb2e3-c3d6-4098-89f4-572ff976ba9a'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_context_restrictions", "GET", "/context/{context_id}/restrictions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_context_restrictions")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def add_context_restriction(
    context_id: str = Field(..., description="The unique identifier of the context to which the restriction will be added.", examples=['be8bb2e3-c3d6-4098-89f4-572ff976ba9a'], json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("add_context_restriction", "POST", "/context/{context_id}/restrictions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("add_context_restriction")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_context_restriction(
    context_id: str = Field(..., description="The unique identifier of the context from which the restriction will be deleted.", examples=['be8bb2e3-c3d6-4098-89f4-572ff976ba9a'], json_schema_extra={'format': 'uuid'}),
    restriction_id: str = Field(..., description="The unique identifier of the specific restriction to delete within the given context.", examples=['1c23d2cb-07b1-4a28-8af3-e369732050ed'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_context_restriction", "DELETE", "/context/{context_id}/restrictions/{restriction_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_context_restriction")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_project_settings(
    provider: Literal["github", "gh", "bitbucket", "bb", "circleci"] = Field(..., description="The version control or CI provider portion of the project slug, identifying which platform hosts the project.", examples=['gh']),
    organization: str = Field(..., description="The organization segment of the project slug, which may be an organization name or a unique organization ID depending on the account type.", examples=['CircleCI-Public']),
    project: str = Field(..., description="The project segment of the project slug, which may be a project name or a unique project ID depending on the account type.", examples=['api-preview-docs'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_project_settings", "GET", "/project/{provider}/{organization}/{project}/settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_project_settings")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def update_project_settings(
    provider: Literal["github", "gh", "bitbucket", "bb", "circleci"] = Field(..., description="The version control provider for the project, corresponding to the first segment of the project slug visible in Project Settings > Overview.", examples=['gh']),
    organization: str = Field(..., description="The organization identifier, corresponding to the second segment of the project slug visible in Project Settings > Overview. May be an org name or an org ID depending on the organization type.", examples=['CircleCI-Public']),
    project: str = Field(..., description="The project identifier, corresponding to the third segment of the project slug visible in Project Settings > Overview. May be a project name or a project ID depending on the organization type.", examples=['api-preview-docs']),
    ...
    # Log invocation
    _log_tool_invocation("update_project_settings", "PATCH", "/project/{provider}/{organization}/{project}/settings", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_project_settings")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_organization_groups(
    org_id: str = Field(..., description="The unique identifier of the organization whose groups you want to retrieve.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953']),
    ...
    # Log invocation
    _log_tool_invocation("list_organization_groups", "GET", "/organizations/{org_id}/groups", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_organization_groups")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_group(
    org_id: str = Field(..., description="The unique opaque identifier of the organization under which the group will be created.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953']),
    ...
    # Log invocation
    _log_tool_invocation("create_group", "POST", "/organizations/{org_id}/groups", _request_id)
...
@mcp.tool()
async def get_group(
    org_id: str = Field(..., description="The unique opaque identifier of the organization that contains the group.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953']),
    group_id: str = Field(..., description="The unique identifier of the group to retrieve, provided as a UUID.", examples=['39f660db-f49b-417e-ad79-2769ba29faf7'], json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_group", "GET", "/organizations/{org_id}/groups/{group_id}", _request_id)
...
async def delete_group(
    org_id: str = Field(..., description="The unique opaque identifier of the organization that contains the group to be deleted.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953']),
    group_id: str = Field(..., description="The unique UUID identifier of the group to delete. All members and associated role grants will be removed upon deletion.", examples=['39f660db-f49b-417e-ad79-2769ba29faf7'], json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_group", "DELETE", "/organizations/{org_id}/groups/{group_id}", _request_id)
...
async def create_usage_export(
    org_id: str = Field(..., description="The unique identifier of the organization for which the usage export will be generated.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953']),
    start: str = Field(..., description="The start date and time (inclusive) of the export range in ISO 8601 format. Must be no more than one year in the past.", json_schema_extra={'format': 'date-time'}),
    end: str = Field(..., description="The end date and time (inclusive) of the export range in ISO 8601 format. Must be no more than 31 days after the start date.", json_schema_extra={'format': 'date-time'}),
    ...
    # Log invocation
    _log_tool_invocation("create_usage_export", "POST", "/organizations/{org_id}/usage_export_job", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_usage_export")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_usage_export_job(
    org_id: str = Field(..., description="The unique opaque identifier of the organization whose usage export job is being retrieved.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953']),
    usage_export_job_id: str = Field(..., description="The unique UUID identifier of the usage export job to retrieve.", examples=['e8235eed-f121-4ae3-9c72-2719d6572818'], json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_usage_export_job", "GET", "/organizations/{org_id}/usage_export_job/{usage_export_job_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_usage_export_job")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def trigger_pipeline_run(
    provider: Literal["github", "gh", "bitbucket", "bb", "circleci"] = Field(..., description="The VCS or platform provider, corresponding to the first segment of the slash-separated project slug found in Project Settings > Overview.", examples=['gh']),
    organization: str = Field(..., description="The second segment of the slash-separated project slug, representing either a human-readable organization name or an opaque organization ID, as shown in Project Settings > Overview.", examples=['CircleCI-Public']),
    project: str = Field(..., description="The third segment of the slash-separated project slug, representing either a human-readable project name or an opaque project ID, as shown in Project Settings > Overview.", examples=['api-preview-docs']),
    definition_id: str | None = Field(None, description="The UUID of the pipeline definition to run, found in Project Settings > Pipelines. If omitted, the default pipeline definition is used.", examples=['2338d0ae-5541-4bbf-88a2-55e9f7281f80'], json_schema_extra={'format': 'uuid'}),
    config_branch: str | None = Field(None, alias="configBranch", description="The branch from which the pipeline config file should be fetched. Mutually exclusive with the config tag field. For GitHub PRs, use pull/<number>/head or pull/<number>/merge.", examples=['main']),
    checkout_branch: str | None = Field(None, alias="checkoutBranch", description="The branch to check out source code from during a checkout step. Mutually exclusive with the checkout tag field. For GitHub PRs, use pull/<number>/head or pull/<number>/merge.", examples=['main']),
    config_tag: str | None = Field(None, alias="configTag", description="The tag used to fetch the pipeline config file; the pipeline runs against the commit the tag points to. Mutually exclusive with the config branch field.", examples=['v2']),
    checkout_tag: str | None = Field(None, alias="checkoutTag", description="The tag used to check out source code during a checkout step; the pipeline runs against the commit the tag points to. Mutually exclusive with the checkout branch field.", examples=['v2']),
    parameters: dict[str, Any] | None = Field(None, description="A key-value map of pipeline parameter names to their values. Limited to 100 entries, with keys up to 128 characters and values up to 512 characters. Values may be strings, booleans, or integers.", examples=[{'example_param': 'my value', 'example_param2': True, 'example_param3': 3}])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("trigger_pipeline_run", "POST", "/project/{provider}/{organization}/{project}/pipeline/run", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("trigger_pipeline_run")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
@mcp.tool()
async def list_pipeline_definitions(project_id: str = Field(..., description="The unique identifier of the project whose pipeline definitions should be listed.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_pipeline_definitions", "GET", "/projects/{project_id}/pipeline-definitions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipeline_definitions")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_pipeline_definition(
    project_id: str = Field(..., description="The unique opaque identifier of the project under which the pipeline definition will be created.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    name: str | None = Field(None, description="A human-readable name for the pipeline definition to distinguish it within the project.", examples=['Some pipeline name']),
    description: str | None = Field(None, description="An optional description providing additional context or details about the purpose of this pipeline definition.", examples=['Some pipeline description']),
    ...
    provider: Literal["github_app"] | None = Field(None, description="The version control integration provider for the pipeline definition's configuration source. Currently only 'github_app' is supported.", examples=['github_app']),
    external_id: str | None = Field(None, description="The external identifier for the repository as defined by the version control provider, used to link the pipeline definition to the correct repository.", examples=['some-repo-id'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_pipeline_definition", "POST", "/projects/{project_id}/pipeline-definitions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_pipeline_definition")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_pipeline_definition(
    project_id: str = Field(..., description="The unique opaque identifier of the project containing the pipeline definition.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    pipeline_definition_id: str = Field(..., description="The unique opaque identifier of the pipeline definition to retrieve.", examples=['6cb29b00-8e02-4357-8bc0-313983bf1f46'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_pipeline_definition", "GET", "/projects/{project_id}/pipeline-definitions/{pipeline_definition_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_pipeline_definition")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def update_pipeline_definition(
    project_id: str = Field(..., description="The unique opaque identifier of the project containing the pipeline definition to update.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    pipeline_definition_id: str = Field(..., description="The unique opaque identifier of the pipeline definition to update.", examples=['6cb29b00-8e02-4357-8bc0-313983bf1f46']),
    name: str | None = Field(None, description="A human-readable display name for the pipeline definition.", examples=['Some pipeline name']),
    description: str | None = Field(None, description="A brief explanation of the pipeline definition's purpose or behavior.", examples=['Some pipeline description']),
    file_path: str | None = Field(None, description="The relative path within the repository to the CircleCI YAML configuration file that this pipeline definition should use.", examples=['.circleci/some-pipeline.yml']),
    provider: str | None = Field(None, description="The version control integration provider for the pipeline definition's config source. Currently only 'github_app' is supported.", examples=['github_app']),
    external_id: str | None = Field(None, description="The repository identifier as defined by the version control provider, used to associate the pipeline definition with a specific external repository.", examples=['some-repo-id'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("update_pipeline_definition", "PATCH", "/projects/{project_id}/pipeline-definitions/{pipeline_definition_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_pipeline_definition")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_pipeline_definition(
    project_id: str = Field(..., description="The unique opaque identifier of the project containing the pipeline definition to delete.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    pipeline_definition_id: str = Field(..., description="The unique opaque identifier of the pipeline definition to delete.", examples=['6cb29b00-8e02-4357-8bc0-313983bf1f46'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_pipeline_definition", "DELETE", "/projects/{project_id}/pipeline-definitions/{pipeline_definition_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_pipeline_definition")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_pipeline_definition_triggers(
    project_id: str = Field(..., description="The unique identifier of the project containing the pipeline definition.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    pipeline_definition_id: str = Field(..., description="The unique identifier of the pipeline definition whose triggers you want to list.", examples=['6cb29b00-8e02-4357-8bc0-313983bf1f46'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_pipeline_definition_triggers", "GET", "/projects/{project_id}/pipeline-definitions/{pipeline_definition_id}/triggers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_pipeline_definition_triggers")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_pipeline_trigger(
    project_id: str = Field(..., description="The unique opaque identifier of the project in which the pipeline definition resides.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    pipeline_definition_id: str = Field(..., description="The unique opaque identifier of the pipeline definition for which the trigger will be created.", examples=['6cb29b00-8e02-4357-8bc0-313983bf1f46']),
    provider: str | None = Field(None, description="The version control or integration provider for the trigger's event source. Accepted values are `github_app`, `github_oauth`, and `webhook`.", examples=['github_app']),
    external_id: str | None = Field(None, description="The repository identifier as defined by the version control provider, used to associate the trigger with a specific repository.", examples=['some-repo-id']),
    sender: str | None = Field(None, description="Identifies the entity sending the webhook event; only applicable when the provider is `webhook`.", examples=['some-webhook-sender']),
    event_preset: Literal["all-pushes", "only-tags", "default-branch-pushes", "only-build-prs", "only-open-prs", "only-labeled-prs", "only-merged-prs", "only-ready-for-review-prs", "only-branch-delete", "only-build-pushes-to-non-draft-prs", "only-merged-or-closed-prs", "pr-comment-equals-run-ci", "non-draft-pr-opened", "pushes-to-merge-queues"] | None = Field(None, description="Specifies which category of events will activate this trigger using a named preset. Only applicable when `event_source.provider` is `github_app`; choose from the supported preset values to control which push, PR, or branch events fire the trigger.", examples=['all-pushes']),
    checkout_ref: str | None = Field(None, description="The Git ref used to check out source code for pipeline runs created by this trigger. Required when the provider is `webhook`; for `github_app`, only provide this if the event source repository differs from the pipeline definition's checkout source repository.", examples=['some-checkout-ref']),
    config_ref: str | None = Field(None, description="The Git ref used to fetch the pipeline configuration for runs created by this trigger. Required when the provider is `webhook`; for `github_app`, only provide this if the event source repository differs from the pipeline definition's config source repository.", examples=['some-config-ref']),
    event_name: str | None = Field(None, description="The name of the event that activates this trigger. Should only be set when the provider is `webhook`.", examples=['some-event-name']),
    disabled: bool | None = Field(None, description="When set to `true`, the trigger is created in a disabled state and will not fire until explicitly enabled. Not supported for pipeline definitions using `github_oauth` as the config source provider.", examples=[False])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("create_pipeline_trigger", "POST", "/projects/{project_id}/pipeline-definitions/{pipeline_definition_id}/triggers", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_pipeline_trigger")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def get_trigger(
    project_id: str = Field(..., description="The unique opaque identifier of the project that owns the trigger.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    trigger_id: str = Field(..., description="The unique opaque identifier of the trigger to retrieve.", examples=['bbea3a3d-4686-48b6-8d2d-2a14ace3913c'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_trigger", "GET", "/projects/{project_id}/triggers/{trigger_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("get_trigger")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def update_trigger(
    project_id: str = Field(..., description="The unique opaque identifier of the project that owns the trigger.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    trigger_id: str = Field(..., description="The unique opaque identifier of the trigger to update.", examples=['bbea3a3d-4686-48b6-8d2d-2a14ace3913c']),
    event_preset: Literal["all-pushes", "only-tags", "default-branch-pushes", "only-build-prs", "only-open-prs", "only-labeled-prs", "only-merged-prs", "only-ready-for-review-prs", "only-branch-delete", "only-build-pushes-to-non-draft-prs", "only-merged-or-closed-prs", "pr-comment-equals-run-ci", "non-draft-pr-opened", "pushes-to-merge-queues"] | None = Field(None, description="A predefined event filtering preset that determines which GitHub events activate this trigger. Only applicable when the trigger's provider is `github_app`.", examples=['all-pushes']),
    checkout_ref: str | None = Field(None, description="The Git ref (branch, tag, or commit SHA) used to check out source code when pipeline runs are created from this trigger.", examples=['some-checkout-ref']),
    config_ref: str | None = Field(None, description="The Git ref used to fetch the pipeline configuration file when pipeline runs are created from this trigger.", examples=['some-config-ref']),
    event_name: str | None = Field(None, description="The name of the event that activates this trigger. Only settable for triggers where the provider is `webhook`.", examples=['some event name']),
    disabled: bool | None = Field(None, description="Whether the trigger is disabled and should not create pipeline runs when events occur. Only settable for triggers where the provider is `github_oauth`, `github_app`, or `webhook`.", examples=[False]),
    sender: str | None = Field(None, description="The identity of the entity sending the webhook payload. Only settable for triggers where the provider is `webhook`.", examples=['some sender'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("update_trigger", "PATCH", "/projects/{project_id}/triggers/{trigger_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("update_trigger")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def delete_trigger(
    project_id: str = Field(..., description="The unique opaque identifier of the project from which the trigger will be deleted.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    trigger_id: str = Field(..., description="The unique opaque identifier of the trigger to be deleted.", examples=['bbea3a3d-4686-48b6-8d2d-2a14ace3913c'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_trigger", "DELETE", "/projects/{project_id}/triggers/{trigger_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_trigger")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def rollback_project(
    project_id: str = Field(..., description="The unique opaque identifier of the project in which the rollback will be performed.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8']),
    component_name: str = Field(..., description="The name of the component within the project to be rolled back.", examples=['frontend']),
    current_version: str = Field(..., description="The version of the component currently deployed, which will be replaced by the rollback.", examples=['1.0.0']),
    environment_name: str = Field(..., description="The name of the environment in which the rollback will be executed (e.g., production, staging).", examples=['production']),
    target_version: str = Field(..., description="The version to roll back to, which should be a previously stable and deployed version of the component.", examples=['1.0.1']),
    namespace: str | None = Field(None, description="The Kubernetes or deployment namespace where the component resides. Defaults to the project's default namespace if not specified.", examples=['default']),
    parameters: dict[str, Any] | None = Field(None, description="A key-value map of additional parameters to pass to the rollback pipeline, allowing customization of pipeline behavior beyond standard inputs.", examples=[{'param1': 'value1', 'param2': 'value2'}]),
    reason: str | None = Field(None, description="A human-readable explanation for why the rollback is being performed, useful for audit trails and incident tracking.", examples=['The component is down'])
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("rollback_project", "POST", "/projects/{project_id}/rollback", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("rollback_project")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def list_environments(
    org_id: str = Field(..., alias="org-id", description="The unique identifier of the organization whose environments you want to list, provided as a UUID.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953'], json_schema_extra={'format': 'uuid'}),
    ...
    # Log invocation
    _log_tool_invocation("list_environments", "GET", "/deploy/environments", _request_id)
...
# Tags: Deploy
@mcp.tool()
async def get_environment(environment_id: str = Field(..., description="The unique UUID identifying the deployment environment to retrieve.", examples=['1c23d2cb-07b1-4a28-8af3-e369732050ed'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_environment", "GET", "/deploy/environments/{environment_id}", _request_id)
...
async def list_components(
    org_id: str = Field(..., alias="org-id", description="The unique identifier of the organization whose components will be listed.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953'], json_schema_extra={'format': 'uuid'}),
    ...
    project_id: str | None = Field(None, alias="project-id", description="The unique identifier of a project used to filter components to only those belonging to that project.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8'], json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_components", "GET", "/deploy/components", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_components")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: Deploy
@mcp.tool()
async def get_component(component_id: str = Field(..., description="The unique opaque identifier of the component to retrieve, as returned when the component was created or listed.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953'])) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("get_component", "GET", "/deploy/components/{component_id}", _request_id)
...
async def list_component_versions(
    component_id: str = Field(..., description="The unique identifier of the component whose versions you want to retrieve.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953']),
    environment_id: str | None = Field(None, alias="environment-id", description="The unique identifier of an environment to filter component versions by, returning only versions relevant to that environment. Must be a valid UUID.", examples=['39723015-b399-4601-9ff6-bd1bfbed8fa8'], json_schema_extra={'format': 'uuid'})
) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_component_versions", "GET", "/deploy/components/{component_id}/versions", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_component_versions")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: OTel
@mcp.tool()
async def list_otel_exporters(org_id: str = Field(..., alias="org-id", description="The unique identifier of the organization whose OTLP exporter configurations should be retrieved, provided as a UUID.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953'], json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("list_otel_exporters", "GET", "/otel/exporters", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("list_otel_exporters")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
async def create_otlp_exporter(
    org_id: str = Field(..., description="The unique identifier of the organization for which the OTLP exporter will be created.", examples=['b9291e0d-a11e-41fb-8517-c545388b5953'], json_schema_extra={'format': 'uuid'}),
    endpoint: str = Field(..., description="The destination OTLP collector endpoint, specified as hostname and port only — omit any scheme prefix such as https:// or grpc://.", examples=['otel.example.com:4317']),
    protocol: Literal["grpc", "http"] = Field(..., description="The transport protocol used when exporting telemetry data; choose grpc for gRPC transport or http for HTTP/protobuf transport.", examples=['grpc']),
    insecure: bool | None = Field(None, description="When set to true, the exporter connects to the endpoint without TLS encryption; defaults to false for secure connections.", examples=[False]),
    ...
    # Log invocation
    _log_tool_invocation("create_otlp_exporter", "POST", "/otel/exporters", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("create_otlp_exporter")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
# Tags: OTel
@mcp.tool()
async def delete_otlp_exporter(otel_exporter_id: str = Field(..., description="The unique identifier of the OTLP exporter configuration to delete.", json_schema_extra={'format': 'uuid'})) -> dict[str, Any]:
    ...
    # Log invocation
    _log_tool_invocation("delete_otlp_exporter", "DELETE", "/otel/exporters/{otel_exporter_id}", _request_id)
    ...
    # Inject per-operation authentication
    _auth = _get_auth_for_operation("delete_otlp_exporter")
    _http_headers.update(_auth.get("headers", {}))
    _http_query.update(_auth.get("params", {}))
...
def validate_environment() -> None:
    ...
    if errors:
        ...
        print("  python circle_ci_api_server.py", file=sys.stderr)
        print("=" * 70, file=sys.stderr)
        sys.exit(1)
