...
<!-- mcp-name: com.mcparmory/launchdarkly -->
...
uvx mcparmory-launchdarkly
```

### With pip

```bash
pip install mcparmory-launchdarkly
API_KEY=YOUR_API_KEY \
...
Example (if server is at `/home/user/mcp-servers/launchdarkly`):
```json
{
  "mcpServers": {
    "launchdarkly": {
      "command": "python",
      "args": ["/home/user/mcp-servers/launchdarkly/server.py"]
    }
  }
}
```
...
docker run -p 8000:8000 \
  ...
  ghcr.io/mcparmory/launchdarkly:latest
```
...
```bash
docker build -t launchdarkly .
docker run -p 8000:8000 --env-file .env launchdarkly
```
