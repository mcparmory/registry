...
async def list_holdouts(
    project_key: str = Field(..., alias="projectKey", description="The unique identifier for the project containing the holdouts."),
    environment_key: str = Field(..., alias="environmentKey", description="The unique identifier for the environment within the project from which to retrieve holdouts."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetAllHoldoutsRequest(
            path=_models.GetAllHoldoutsRequestPath(project_key=project_key, environment_key=environment_key)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_holdouts: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/v2/projects/{projectKey}/environments/{environmentKey}/holdouts", _request.path.model_dump(by_alias=True)) if _request.path else "/api/v2/projects/{projectKey}/environments/{environmentKey}/holdouts"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_holdouts")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_holdouts", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_holdouts",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def create_holdout(
    project_key: str = Field(..., alias="projectKey", description="The unique identifier for the project where the holdout will be created."),
    environment_key: str = Field(..., alias="environmentKey", description="The unique identifier for the environment within the project where the holdout will be created."),
    name: str | None = Field(None, description="A human-readable name for the holdout to help identify it in the UI and reports."),
    key: str | None = Field(None, description="A unique identifier for the holdout used in API calls and programmatic references."),
    randomizationunit: str | None = Field(None, description="The unit used to randomly assign users to the holdout, such as 'user' for individual users or other entity types."),
    attributes: list[str] | None = Field(None, description="A list of attribute names (e.g., 'country', 'device', 'os') that can be used to segment and analyze the holdout's results."),
    holdoutamount: str | None = Field(None, description="The percentage of the audience allocated to the holdout, specified as a numeric string (e.g., '10' for 10%)."),
    primarymetrickey: str | None = Field(None, description="The identifier of the primary metric used to evaluate the holdout's effectiveness."),
    metrics: list[_models.MetricInput] | None = Field(None, description="An array of metric objects containing detailed configuration for secondary metrics to track alongside the primary metric."),
    prerequisiteflagkey: str | None = Field(None, description="The identifier of a feature flag that must be enabled for users to be included in the holdout, creating a dependency between the flag and holdout."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.PostHoldoutRequest(
            path=_models.PostHoldoutRequestPath(project_key=project_key, environment_key=environment_key),
            body=_models.PostHoldoutRequestBody(name=name, key=key, randomizationunit=randomizationunit, attributes=attributes, holdoutamount=holdoutamount, primarymetrickey=primarymetrickey, metrics=metrics, prerequisiteflagkey=prerequisiteflagkey)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for create_holdout: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("create_holdout")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("create_holdout", "POST", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="create_holdout",
        method="POST",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
# Tags: Holdouts (beta)
@mcp.tool()
async def get_holdout_by_id(
    project_key: str = Field(..., alias="projectKey", description="The unique identifier for the project containing the holdout experiment."),
    environment_key: str = Field(..., alias="environmentKey", description="The unique identifier for the environment within the project where the holdout experiment exists."),
    holdout_id: str = Field(..., alias="holdoutId", description="The unique identifier of the holdout experiment to retrieve."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetHoldoutByIdRequest(
            path=_models.GetHoldoutByIdRequestPath(project_key=project_key, environment_key=environment_key, holdout_id=holdout_id)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_holdout_by_id: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/v2/projects/{projectKey}/environments/{environmentKey}/holdouts/id/{holdoutId}", _request.path.model_dump(by_alias=True)) if _request.path else "/api/v2/projects/{projectKey}/environments/{environmentKey}/holdouts/id/{holdoutId}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_holdout_by_id")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_holdout_by_id", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_holdout_by_id",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def get_holdout(
    project_key: str = Field(..., alias="projectKey", description="The unique identifier for the LaunchDarkly project containing the holdout."),
    environment_key: str = Field(..., alias="environmentKey", description="The unique identifier for the environment within the project where the holdout is configured."),
    holdout_key: str = Field(..., alias="holdoutKey", description="The unique identifier for the holdout experiment to retrieve."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetHoldoutRequest(
            path=_models.GetHoldoutRequestPath(project_key=project_key, environment_key=environment_key, holdout_key=holdout_key)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_holdout: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/v2/projects/{projectKey}/environments/{environmentKey}/holdouts/{holdoutKey}", _request.path.model_dump(by_alias=True)) if _request.path else "/api/v2/projects/{projectKey}/environments/{environmentKey}/holdouts/{holdoutKey}"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_holdout")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_holdout", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_holdout",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def update_holdout(
    project_key: str = Field(..., alias="projectKey", description="The project key that contains the holdout. Used to identify which project the holdout belongs to."),
    environment_key: str = Field(..., alias="environmentKey", description="The environment key where the holdout exists. Used to identify which environment the holdout is configured in."),
    holdout_key: str = Field(..., alias="holdoutKey", description="The holdout key that uniquely identifies the holdout to update within the project and environment."),
    instructions: list[_models.Instruction] = Field(..., description="An array of semantic patch instruction objects that define the updates to apply. Each instruction object must include a `kind` field specifying the operation (endHoldout, removeExperiment, updateDescription, or updateName), and some operations require an additional `value` field. An optional `comment` field can be included at the root level to describe the change."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.PatchHoldoutRequest(
            path=_models.PatchHoldoutRequestPath(project_key=project_key, environment_key=environment_key, holdout_key=holdout_key),
            body=_models.PatchHoldoutRequestBody(instructions=instructions)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for update_holdout: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("update_holdout")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("update_holdout", "PATCH", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="update_holdout",
        method="PATCH",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
# Tags: Approvals (beta)
@mcp.tool()
async def update_flag_approval_request(
    project_key: str = Field(..., alias="projectKey", description="The project key that contains the feature flag."),
    feature_flag_key: str = Field(..., alias="featureFlagKey", description="The feature flag key associated with the approval request."),
    environment_key: str = Field(..., alias="environmentKey", description="The environment key where the flag approval request exists."),
    id_: str = Field(..., alias="id", description="The unique identifier of the approval request to update."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.PatchFlagConfigApprovalRequest(
            path=_models.PatchFlagConfigApprovalRequestPath(project_key=project_key, feature_flag_key=feature_flag_key, environment_key=environment_key, id_=id_)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for update_flag_approval_request: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("update_flag_approval_request")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("update_flag_approval_request", "PATCH", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="update_flag_approval_request",
        method="PATCH",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
async def update_teams(instructions: list[_models.Instruction] = Field(..., description="Array of instruction objects that define the update operations. Each instruction must include a `kind` field specifying the action (addMembersToTeams or addAllMembersToTeams) and relevant parameters. For addMembersToTeams, provide memberIDs and teamKeys. For addAllMembersToTeams, provide teamKeys and optional filters (filterLastSeen, filterQuery, filterRoles, filterTeamKey, ignoredMemberIDs).")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.PatchTeamsRequest(
            body=_models.PatchTeamsRequestBody(instructions=instructions)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for update_teams: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err
    ...
    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("update_teams")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("update_teams", "PATCH", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="update_teams",
        method="PATCH",
        path=_http_path,
        request_id=_request_id,
        body=_http_body,
...
@mcp.tool()
async def get_clientside_context_usage(
    from_: str | None = Field(None, alias="from", description="Start timestamp for the data series in Unix milliseconds. Defaults to the beginning of the current month if not specified."),
    to: str | None = Field(None, description="End timestamp for the data series in Unix milliseconds. Defaults to the current time if not specified."),
    project_key: str | None = Field(None, alias="projectKey", description="Filter results by one or more project keys. Specify multiple times to include multiple projects."),
    environment_key: str | None = Field(None, alias="environmentKey", description="Filter results by one or more environment keys. Requires exactly one `projectKey` to be specified. Specify multiple times to include multiple environments."),
    sdk_name: str | None = Field(None, alias="sdkName", description="Filter results by one or more SDK names. Specify multiple times to include multiple SDKs."),
    anonymous: str | None = Field(None, description="Filter results by anonymous context status. Accepts `true` or `false`. Specify multiple times to include both values."),
    group_by: str | None = Field(None, alias="groupBy", description="Group results by one or more dimensions. Valid options are `projectId`, `environmentId`, `sdkName`, `sdkAppId`, or `anonymousV2`. Context kind is always included as a grouping dimension. Specify multiple times to group by multiple dimensions."),
    aggregation_type: str | None = Field(None, alias="aggregationType", description="Aggregation method for the time series data. Defaults to `month_to_date`. Choose from `month_to_date`, `incremental`, or `rolling_30d`."),
    granularity: str | None = Field(None, description="Data granularity for the time series. Defaults to `daily`. Supported values depend on aggregation type: `month_to_date` supports `daily` and `monthly`; `incremental` and `rolling_30d` support `daily` only."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetContextsClientsideUsageRequest(
            query=_models.GetContextsClientsideUsageRequestQuery(from_=from_, to=to, project_key=project_key, environment_key=environment_key, sdk_name=sdk_name, anonymous=anonymous, group_by=group_by, aggregation_type=aggregation_type, granularity=granularity)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_clientside_context_usage: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/api/v2/usage/clientside-contexts"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_clientside_context_usage")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_clientside_context_usage", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_clientside_context_usage",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
# Tags: Account usage (beta)
@mcp.tool()
async def get_evaluations_usage_for_flag(
    project_key: str = Field(..., alias="projectKey", description="The unique identifier for the project containing the feature flag."),
    environment_key: str = Field(..., alias="environmentKey", description="The unique identifier for the environment where the flag is being evaluated."),
    feature_flag_key: str = Field(..., alias="featureFlagKey", description="The unique identifier for the feature flag to retrieve usage metrics for."),
    from_: str | None = Field(None, alias="from", description="The start timestamp for the data series in ISO 8601 format. Defaults to 30 days before the current time if not specified."),
    to: str | None = Field(None, description="The end timestamp for the data series in ISO 8601 format. Defaults to the current time if not specified."),
    tz: str | None = Field(None, description="The timezone identifier (e.g., UTC, America/New_York) used to determine day boundaries when returning daily-granularity data."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetEvaluationsUsageRequest(
            path=_models.GetEvaluationsUsageRequestPath(project_key=project_key, environment_key=environment_key, feature_flag_key=feature_flag_key),
            query=_models.GetEvaluationsUsageRequestQuery(from_=from_, to=to, tz=tz)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_evaluations_usage_for_flag: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/v2/usage/evaluations/{projectKey}/{environmentKey}/{featureFlagKey}", _request.path.model_dump(by_alias=True)) if _request.path else "/api/v2/usage/evaluations/{projectKey}/{environmentKey}/{featureFlagKey}"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_evaluations_usage_for_flag")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_evaluations_usage_for_flag", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_evaluations_usage_for_flag",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
# Tags: Account usage (beta)
@mcp.tool()
async def list_sdk_versions_details() -> dict[str, Any]:
    """Retrieve detailed SDK version usage data for your account, including version information, end-of-life status, and relay proxy metadata. Data is refreshed hourly and shows the maximum version for each SDK observed in the last 24 hours."""

    # Extract parameters for API call
    _http_path = "/api/v2/usage/sdk-versions/details"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_sdk_versions_details")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_sdk_versions_details", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_sdk_versions_details",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
# Tags: Account usage (beta)
@mcp.tool()
async def get_serverside_context_usage(
    from_: str | None = Field(None, alias="from", description="Start timestamp for the usage data series in Unix seconds. Defaults to the beginning of the current month if not specified."),
    to: str | None = Field(None, description="End timestamp for the usage data series in Unix seconds. Defaults to the current time if not specified."),
    project_key: str | None = Field(None, alias="projectKey", description="Filter results by one or more project keys. Specify multiple times to include multiple projects."),
    environment_key: str | None = Field(None, alias="environmentKey", description="Filter results by one or more environment keys. Requires exactly one `projectKey` to be specified. Specify multiple times to include multiple environments."),
    sdk_name: str | None = Field(None, alias="sdkName", description="Filter results by one or more SDK names. Specify multiple times to include multiple SDKs."),
    anonymous: str | None = Field(None, description="Filter results by anonymous context status. Accepts `true` or `false`. Specify multiple times to include both values."),
    group_by: str | None = Field(None, alias="groupBy", description="Group results by one or more dimensions (contextKind is always included). Valid options: `projectId`, `environmentId`, `sdkName`, `sdkAppId`, `anonymousV2`. Specify multiple times to group by multiple dimensions."),
    aggregation_type: str | None = Field(None, alias="aggregationType", description="Aggregation method for the time series data. Defaults to `month_to_date`. Options: `month_to_date` (cumulative within month), `incremental` (period-over-period), `rolling_30d` (30-day rolling window)."),
    granularity: str | None = Field(None, description="Data granularity for the time series. Defaults to `daily`. Supported values depend on aggregationType: `month_to_date` and `incremental` support `daily` and `monthly`; `rolling_30d` supports `daily` only."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetContextsServersideUsageRequest(
            query=_models.GetContextsServersideUsageRequestQuery(from_=from_, to=to, project_key=project_key, environment_key=environment_key, sdk_name=sdk_name, anonymous=anonymous, group_by=group_by, aggregation_type=aggregation_type, granularity=granularity)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_serverside_context_usage: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/api/v2/usage/serverside-contexts"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_serverside_context_usage")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_serverside_context_usage", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_serverside_context_usage",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
# Tags: Account usage (beta)
@mcp.tool()
async def get_stream_usage_by_sdk_version(
    source: str = Field(..., description="The source type for streaming connections. Must be either 'client' (for client-side SDKs) or 'server' (for server-side SDKs)."),
    from_: str | None = Field(None, alias="from", description="The start timestamp for the data series. If omitted, defaults to 24 hours before the 'to' timestamp. Use ISO 8601 format."),
    to: str | None = Field(None, description="The end timestamp for the data series. If omitted, defaults to the current time. Use ISO 8601 format."),
    tz: str | None = Field(None, description="The timezone to apply when determining day boundaries in daily data responses. Specify using standard timezone identifiers (e.g., 'America/New_York', 'UTC')."),
    sdk: str | None = Field(None, description="Optional filter to return only usage data for a specific SDK name. If provided, only series matching this SDK identifier will be included in the response."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetStreamUsageBySdkVersionRequest(
            path=_models.GetStreamUsageBySdkVersionRequestPath(source=source),
            query=_models.GetStreamUsageBySdkVersionRequestQuery(from_=from_, to=to, tz=tz, sdk=sdk)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_stream_usage_by_sdk_version: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/v2/usage/streams/{source}/bysdkversion", _request.path.model_dump(by_alias=True)) if _request.path else "/api/v2/usage/streams/{source}/bysdkversion"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_stream_usage_by_sdk_version")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_stream_usage_by_sdk_version", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_stream_usage_by_sdk_version",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
# Tags: Account usage (beta)
@mcp.tool()
async def list_stream_sdk_versions(source: str = Field(..., description="The type of streaming connection source to query. Must be either 'client' for client-side SDKs or 'server' for server-side SDKs.")) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetStreamUsageSdkversionRequest(
            path=_models.GetStreamUsageSdkversionRequestPath(source=source)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for list_stream_sdk_versions: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = _build_path("/api/v2/usage/streams/{source}/sdkversions", _request.path.model_dump(by_alias=True)) if _request.path else "/api/v2/usage/streams/{source}/sdkversions"
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("list_stream_sdk_versions")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("list_stream_sdk_versions", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="list_stream_sdk_versions",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        headers=_http_headers,
...
# Tags: Account usage (beta)
@mcp.tool()
async def get_contexts_total_usage(
    from_: str | None = Field(None, alias="from", description="Start timestamp for the usage data series in Unix milliseconds. Defaults to the beginning of the current month if not specified."),
    to: str | None = Field(None, description="End timestamp for the usage data series in Unix milliseconds. Defaults to the current time if not specified."),
    project_key: str | None = Field(None, alias="projectKey", description="Filter results by one or more project keys. Specify multiple times to include multiple projects."),
    environment_key: str | None = Field(None, alias="environmentKey", description="Filter results by one or more environment keys. Requires exactly one `projectKey` to be specified. Specify multiple times to include multiple environments."),
    sdk_name: str | None = Field(None, alias="sdkName", description="Filter results by one or more SDK names. Specify multiple times to include multiple SDKs."),
    sdk_type: str | None = Field(None, alias="sdkType", description="Filter results by one or more SDK types. Specify multiple times to include multiple SDK types."),
    anonymous: str | None = Field(None, description="Filter results by anonymity status. Accepts `true` or `false`. Specify multiple times to include both anonymous and non-anonymous contexts."),
    group_by: str | None = Field(None, alias="groupBy", description="Group results by one or more dimensions in addition to the always-included `contextKind`. Valid dimensions are `projectId`, `environmentId`, `sdkName`, `sdkType`, `sdkAppId`, or `anonymousV2`. Specify multiple times to group by multiple dimensions."),
    aggregation_type: str | None = Field(None, alias="aggregationType", description="Aggregation method for the time series data. Choose `month_to_date` (default) for data from the start of the current month, `incremental` for cumulative totals, or `rolling_30d` for a 30-day rolling window."),
    granularity: str | None = Field(None, description="Data granularity for the time series. Defaults to `daily`. Supported values depend on `aggregationType`: `month_to_date` supports `daily` and `monthly`; `incremental` and `rolling_30d` support `daily` only."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetContextsTotalUsageRequest(
            query=_models.GetContextsTotalUsageRequestQuery(from_=from_, to=to, project_key=project_key, environment_key=environment_key, sdk_name=sdk_name, sdk_type=sdk_type, anonymous=anonymous, group_by=group_by, aggregation_type=aggregation_type, granularity=granularity)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_contexts_total_usage: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/api/v2/usage/total-contexts"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_contexts_total_usage")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_contexts_total_usage", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_contexts_total_usage",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
...
async def get_mau_total_usage(
    ...
    sdk_name: str | None = Field(None, alias="sdkName", description="Filter results by one or more SDK names. Specify multiple times to include multiple SDKs."),
    sdk_type: str | None = Field(None, alias="sdkType", description="Filter results by one or more SDK types. Specify multiple times to include multiple SDK types."),
    anonymous: str | None = Field(None, description="Filter results by anonymity status. Valid values are `true` or `false`. Specify multiple times to include both anonymous and non-anonymous data."),
    group_by: str | None = Field(None, alias="groupBy", description="Group results by one or more dimensions: `projectId`, `environmentId`, `sdkName`, `sdkType`, `sdkAppId`, or `anonymousV2`. Specify multiple times to create multi-dimensional groupings."),
    aggregation_type: str | None = Field(None, alias="aggregationType", description="Aggregation method for the time series data. Valid values are `month_to_date` (default), `incremental`, or `rolling_30d`. Affects supported granularity options."),
    granularity: str | None = Field(None, description="Data granularity for the time series. Defaults to `daily`. Supported values depend on aggregation type: `month_to_date` supports `daily` and `monthly`; `incremental` and `rolling_30d` support `daily` only."),
) -> dict[str, Any]:
    ...
    # Construct request model with validation
    try:
        _request = _models.GetMauTotalUsageRequest(
            query=_models.GetMauTotalUsageRequestQuery(from_=from_, to=to, project_key=project_key, environment_key=environment_key, sdk_name=sdk_name, sdk_type=sdk_type, anonymous=anonymous, group_by=group_by, aggregation_type=aggregation_type, granularity=granularity)
        )
    except pydantic.ValidationError as _validation_err:
        logging.error(f"Parameter validation failed for get_mau_total_usage: {_validation_err}")
        raise ValueError(f"Invalid parameters: {_validation_err.errors()}") from _validation_err

    # Extract parameters for API call
    _http_path = "/api/v2/usage/total-mau"
    _http_query = _request.query.model_dump(by_alias=True, exclude_none=True) if _request.query else {}
    _http_headers = {}

    # Inject per-operation authentication
    _auth = await _get_auth_for_operation("get_mau_total_usage")
    _http_headers.update(_auth.get("headers", {}))

    _request_id = str(uuid.uuid4())
    _log_tool_invocation("get_mau_total_usage", "GET", _http_path, _request_id)

    # Execute request (returns normalized dict and status code)
    _response_data, _ = await _execute_tool_request(
        tool_name="get_mau_total_usage",
        method="GET",
        path=_http_path,
        request_id=_request_id,
        params=_http_query,
